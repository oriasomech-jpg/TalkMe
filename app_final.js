...
(() => {
  ...
  // ---------- AUTH (Local users; admin can manage users in Settings) ----------
  const USERS_STORAGE_KEY = 'gemel_users_v1';
  const DEFAULT_USERS = [
  ...
  function loadUsers_(){
    try{
      ...
      if(!Array.isArray(arr) || !arr.length) return DEFAULT_USERS.slice();
      // minimal sanitize
      return arr.map(u => ({
      ...
      })).filter(u => u.username && u.password && u.displayName);
    }catch(_){
      return DEFAULT_USERS.slice();
    }
  }
  function saveUsers_(users){
    try{ localStorage.setItem(USERS_STORAGE_KEY, JSON.stringify(users)); }catch(_){}
  }
  function getUsers_(){
    // always load fresh (so settings changes are immediate)
    return loadUsers_();
  }
  ...
  function deleteUser_(username){
    const users = loadUsers_().filter(u => u.username !== username);
    saveUsers_(users);
  }
  ...
  function tryLogin_(username, password){
    const u = getUsers_().find(x => x.username === username && x.password === password);
...
function renderKpisSettings(){
  ...
  function renderSettings(){
    viewEl.innerHTML = `
      <div class="grid2">
        ...
        <div class="sideCard">
          ...
          <div class="hint">פתיחת יוזרים חדשים לנציגים: שם תצוגה + שם משתמש + קוד.</div>

          <div class="grid2" style="margin-top:10px">
            <div class="field">
              <label>שם מלא (תצוגה)</label>
              <input id="um_displayName" placeholder="לדוגמה: סתיו כהן"/>
            </div>
            <div class="field">
              <label>שם משתמש</label>
              <input id="um_username" placeholder="לדוגמה: stav"/>
            </div>
          </div>

          <div class="grid2" style="margin-top:10px">
            <div class="field">
              <label>קוד / סיסמה</label>
              <input id="um_password" placeholder="4 ספרות / קוד" inputmode="numeric"/>
            </div>
            <div class="field">
              <label>תפקיד</label>
              <select id="um_role">
          ...
          <div style="height:10px"></div>
          <button class="btn btnPrimary" id="um_addBtn">הוסף משתמש</button>

          <div class="hr"></div>
          <div id="um_list"></div>
          <div class="sideHint">⚠️ דמו מקומי: נשמר בדפדפן (LocalStorage). חיבור ל־Sheets/DB נוכל להוסיף אחרי.</div>
        </div>
        ` : ``}
</div>
    ...
    if(isAdmin_()){
      const listHost = document.getElementById('um_list');
        ...
        listHost.innerHTML = users.map(u => `
          <div style="display:flex;align-items:center;justify-content:space-between;gap:10px;padding:8px 0;border-bottom:1px solid var(--line)">
            <div>
              ...
              <div class="sideHint" style="margin:0">${escapeHtml(u.username)} • ${u.role==='admin'?'מנהל':'נציג'}</div>
            </div>
            <div style="display:flex;gap:8px;flex-wrap:wrap">
              ${u.username === (state.currentUser?.username||'') ? `<span class="pill">אתה</span>` : `<button class="btn btnSoft btnTiny" data-um-del="${escapeAttr(u.username)}" type="button">מחק</button>`}
            </div>
          </div>
        `).join('') || `<div class="sideHint">אין משתמשים.</div>`;

        $$('#um_list [data-um-del]').forEach(b => b.addEventListener('click', () => {
          const uname = b.getAttribute('data-um-del') || '';
          if(!uname) return;
          deleteUser_(uname);
          toast('נמחק', 'המשתמש נמחק');
          renderUserList();
        }));
      };
      ...
      const addBtn = document.getElementById('um_addBtn');
      if(addBtn){
        addBtn.addEventListener('click', () => {
          const displayName = String(document.getElementById('um_displayName')?.value || '').trim();
          const username = String(document.getElementById('um_username')?.value || '').trim();
          const password = String(document.getElementById('um_password')?.value || '').trim();
          const role = String(document.getElementById('um_role')?.value || 'agent').trim() === 'admin' ? 'admin' : 'agent';

          if(!displayName || !username || !password){
            toast('חסר', 'מלא שם, שם משתמש וקוד');
            return;
          }

          // Basic username constraint
          if(!/^[a-zA-Z0-9._-]{3,}$/.test(username)){
            toast('שגיאה', 'שם משתמש צריך להיות באנגלית/ספרות (לפחות 3 תווים)');
            return;
          }

          upsertUser_({ username, password, role, displayName });
          toast('נוסף', 'המשתמש נשמר');
          // clear fields
          const ids = ['um_displayName','um_username','um_password'];
  ...
  function validateCurrentStep(){
    ...
    // כרגע אנחנו מאשרים מעבר חופשי בין שלבים (כמו שסיכמנו), אבל נשמור ולידציה בסיסית לפרטי משלם
    // כדי שלא נתקע לפני הסיכום.
    if(stepKey !== 'payer') return true;
    ...
    if(method === 'card'){
      ...
      if(!number || !exp){
        if(numEl && !number) numEl.classList.add('reqError');
        if(expEl && !exp) expEl.classList.add('reqError');
        toast('חסר מידע', 'יש למלא מספר כרטיס ותוקף');
        ( (!number && numEl) ? numEl : expEl )?.scrollIntoView?.({behavior:'smooth', block:'center'});
        try{ ((!number && numEl) ? numEl : expEl)?.focus?.(); }catch(_){}
        return false;
      }
      return true;
    }
    ...
    if(method === 'hok'){
      const bankEl = document.getElementById('payer_hok_bank');
      const branchEl = document.getElementById('payer_hok_branch');
      const accEl = document.getElementById('payer_hok_account');
      const bank = (bankEl && bankEl.value) ? String(bankEl.value).trim() : '';
      const branch = (branchEl && branchEl.value) ? String(branchEl.value).trim() : '';
      const account = (accEl && accEl.value) ? String(accEl.value).trim() : '';
      if(!bank || !branch || !account){
        if(bankEl && !bank) bankEl.classList.add('reqError');
        if(branchEl && !branch) branchEl.classList.add('reqError');
        if(accEl && !account) accEl.classList.add('reqError');
        toast('חסר מידע', 'יש למלא בנק + סניף + מספר חשבון');
        ( (!bank && bankEl) ? bankEl : (!branch && branchEl ? branchEl : accEl) )?.scrollIntoView?.({behavior:'smooth', block:'center'});
        try{ ( (!bank && bankEl) ? bankEl : (!branch && branchEl ? branchEl : accEl) )?.focus?.(); }catch(_){}
        return false;
      }
      // תוקף בהו״ק הוא אופציונלי
      return true;
    }
...
function gotoStep(i){
    ...
    // UX: when moving between steps (especially from long forms like "פרטי משלם"),
    // keep the user at the top so the next screen doesn't look blank.
    try{
      const modalBody = document.querySelector('.modalBody');
      if(modalBody) modalBody.scrollTo({ top: 0, behavior: 'smooth' });
      // also ensure the main content is in view
      if(wizardMain) wizardMain.scrollIntoView({ behavior: 'smooth', block: 'start' });
    }catch(_){/* ignore */}
  }
...
function renderNewPoliciesStep(){
  ...
  function computeCompletion(p){
    ...
    const okPayer = (method === 'card')
      ? !!(payer.card && payer.card.tz && payer.card.name && payer.card.last4 && payer.card.exp)
      : (method === 'hok')
