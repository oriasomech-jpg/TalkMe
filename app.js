...
(() => {
  ...
  const nextStepBtn2 = $('#nextStepBtn2');
  const prevStepBtn2 = $('#prevStepBtn2');
  const wfStepEl = $('#wfStep');
  const wfHintEl = $('#wfHint');
  ...
  (function bootAuth_(){
    ...
    if(u){
      ...
      // Auto-login demo user to avoid "empty / unresponsive" UI on first load.
      const demo = USERS.find(x => x.username === 'agent1') || USERS[0];
      const safe = { username:demo.username, role:demo.role, displayName:demo.displayName };
      state.currentUser = safe;
      state.agentName = demo.displayName || state.agentName;
      setSessionUser_(safe);
      hideLogin_();
    }
  })();
...
function renderKpisSettings(){
  ...
  // Footer duplicate controls (sticky bar)
  if(nextStepBtn2) nextStepBtn2.addEventListener('click', () => gotoStep(wiz.stepIndex + 1));
  if(prevStepBtn2) prevStepBtn2.addEventListener('click', () => gotoStep(wiz.stepIndex - 1));

  // Clickable stepper
  stepperEl.addEventListener('click', (e) => {
    const s = e.target.closest('.step');
    if(!s || !stepperEl.contains(s)) return;
    const idx = Number(s.dataset.idx);
    if(Number.isNaN(idx)) return;
    gotoStep(idx);
  });

  // Keyboard shortcuts inside wizard
  window.addEventListener('keydown', (e) => {
    if(!wiz) return;
    if(wizardOverlay.classList.contains('hidden')) return;
    ...
    if((e.ctrlKey || e.metaKey) && e.key === 'Enter'){
      e.preventDefault();
      gotoStep(wiz.stepIndex + 1);
      return;
    }
    // Alt+ArrowLeft -> previous (RTL friendly: still 'back')
...
function gotoStep(i){
    ...
    updateSums();
    updateWizardChrome();
    // Always bring the user to the top of the step (heavy screens)
    const mb = document.querySelector('.modalBody');
  ...
  function renderStepper(){
    ...
    stepperEl.innerHTML = WIZ_STEPS.map((s, idx) => {
      ...
      return `<div class="step ${idx===wiz.stepIndex?'active':''} ${idx<wiz.stepIndex?'done':''}" data-idx="${idx}">${escapeHtml(lbl)}</div>`;
    }).join('');
  }
  ...
  function updateWizardChrome(){
    if(!wiz) return;
    const step = WIZ_STEPS[wiz.stepIndex];
    const label = step ? step.label : '—';
    if(wfStepEl) wfStepEl.textContent = `שלב ${wiz.stepIndex+1}/${WIZ_STEPS.length} • ${label}`;
    if(stepProgressFill){
      const pct = (WIZ_STEPS.length<=1) ? 0 : (wiz.stepIndex/(WIZ_STEPS.length-1))*100;
      stepProgressFill.style.width = `${Math.max(0, Math.min(100, pct))}%`;
    }

    const atStart = wiz.stepIndex <= 0;
    const atEnd = wiz.stepIndex >= (WIZ_STEPS.length-1);

    const nextLabel = atEnd ? 'סיום' : 'המשך';
    if(nextStepBtn) nextStepBtn.textContent = nextLabel;
    if(nextStepBtn2) nextStepBtn2.textContent = nextLabel;

    if(prevStepBtn) prevStepBtn.disabled = atStart;
    if(prevStepBtn2) prevStepBtn2.disabled = atStart;
  }
