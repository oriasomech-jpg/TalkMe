...
function debounce(fn, wait){
  ...
  (function setupPwaInstall_(){
    ...
    const show = (on) => { if(wrap) wrap.style.display = on ? 'flex' : 'none'; };
    const showHint = (txt) => { if(!hint) return; hint.textContent = txt; hint.style.display = 'block'; };
    const hideHint = () => { if(!hint) return; hint.style.display = 'none'; hint.textContent = ''; };

    // If already installed, no need to show.
    if(isStandalone()){
      show(false);
      return;
    }

    // Register Service Worker (required for full PWA behavior)
    ...
    // iOS: show instructions (no install prompt API)
    if(isIOS()){
      show(true);
      showHint('באייפון: פתח ב‑Safari → כפתור שיתוף → Add to Home Screen');
      return;
    }
    ...
    window.addEventListener('beforeinstallprompt', (e) => {
      // Prevent automatic mini-infobar; we control install via our button
      e.preventDefault();
      deferredPrompt = e;
      hideHint();
      show(true);
    ...
    window.addEventListener('appinstalled', () => {
      show(false);
      showHint('✅ האפליקציה הותקנה בהצלחה');
    });
    ...
    if(btn){
      btn.addEventListener('click', async () => {
        if(!deferredPrompt){
          show(true);
          showHint('אם אינך רואה התקנה: פתח את המערכת ב‑Chrome/Edge על כתובת HTTPS ואז נסה שוב.');
          return;
        }
        hideHint();
        ...
        try{
          ...
          if(choice && choice.outcome !== 'accepted'){
            showHint('ההתקנה בוטלה. אפשר לנסות שוב.');
          }
        }catch(_){
          deferredPrompt = null;
        }
    ...
    // Hide when not installable
    setTimeout(() => {
      if(!deferredPrompt){
        // Keep hidden until browser fires beforeinstallprompt
        show(false);
      }
    }, 700);
  })();
