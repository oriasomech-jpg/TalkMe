...
// Keep wizard title in sync
  ...
  async function readHarFileToRows_(file){
    ...
    // CSV: parse locally (no XLSX lib needed)
    if(isCsv){
      const text = await file.text();
      const rows = parseCsvToRows_(text);
      return { ok:true, rows, meta:{ type:'csv' } };
    }

    // XLSX: requires XLSX lib (loaded from index.html)
    if(typeof XLSX === 'undefined') return { ok:false, error:'NO_XLSX_LIB' };

    const buf = await file.arrayBuffer();
    const wb = XLSX.read(buf, { type:'array' });
    const sheetName = wb.SheetNames && wb.SheetNames[0] ? wb.SheetNames[0] : '';
    const ws = wb.Sheets[sheetName];
    const rows = XLSX.utils.sheet_to_json(ws, { header:1, raw:true, defval:'' });
    return { ok:true, rows, meta:{ type:'xlsx', sheetName, sheetCount:(wb.SheetNames||[]).length } };
  }

  function detectCsvDelim_(line){
  ...
  function parseCsvLine_(line, delim){
    const out = [];
    let cur = '';
    let inQuotes = false;
  ...
  function parseCsvToRows_(text){
    ...
    const delim = detectCsvDelim_(first);

    const rows = [];
    for(const ln of lines){
      if(!ln) { rows.push(['']); continue; }
      rows.push(parseCsvLine_(ln, delim).map(x => String(x||'').trim()));
    }
    return rows;
  }
    ...
    if(harBtn){
      harBtn.addEventListener('click', async () => {
        try{
          ...
          if(harPrev){
            const meta = rr.meta || {};
            const sheetInfo = meta.type === 'xlsx' ? (` • גיליון: ${escapeHtml(meta.sheetName||'')} (${meta.sheetCount||1})`) : ' • CSV';
            harPrev.innerHTML = `נקרא קובץ: <b>${escapeHtml(file.name||'')}</b>${sheetInfo} • שורות: <b>${(rr.rows||[]).length}</b>`;
