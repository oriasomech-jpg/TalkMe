...
(() => {
  ...
  function loadUsers_(){
    // === AUTO_ADMIN_RECOVERY ===
    // If users exist but no active admin -> recreate default admin
    try{
      const raw = localStorage.getItem(USERS_LS_KEY);
      const arr = raw ? JSON.parse(raw) : null;
      if(Array.isArray(arr) && arr.length){
        const hasAdmin = arr.some(u => u.role === 'admin' && u.active !== false);
        if(!hasAdmin){
          arr.push({ username:'admin', password:'3316', role:'admin', displayName:'מנהל מערכת', active:true, createdAt:new Date().toISOString() });
          saveUsers_(arr);
        }
        // normalize
        return arr.map(u => ({
  ...
  function requireAuth_(){
    if(state.currentUser && state.currentUser.username){ return true; }
    // force login overlay and block app access
    try{ showLogin_(); }catch(_){}
    return false;
  }
  ...
  function doLogout_(){
  try{ clearPersistedUser_(); }catch(_){/* ignore */}
  ...
  if(loginForm){
    loginForm.addEventListener('submit', (e) => {
      ...
      // Hard reset any previous session before authenticating
      try{ clearPersistedUser_(); }catch(_){/*ignore*/}
      state.currentUser = null;
      state.agentName = 'אורח';
if(loginError) loginError.classList.add('hidden');
  ...
  (function bootAuth_(){
    ...
    let users = [];
    try{ users = loadUsers_(); }catch(_){ users = []; }

    // Restore SESSION user only (never auto-login from "Remember me").
    const remembered = getStoredUser_();
    const sessionU = getSessionUser_();

    const rememberEl = document.getElementById('rememberMe');
    if(rememberEl) rememberEl.checked = !!remembered;

    const findActiveUser = (u) => {
      if(!u || !u.username) return null;
      const real = (users||[]).find(x => x.username === u.username);
      if(!real) return null;
      if(real.active === false) return null;
      return { username: real.username, role: real.role, displayName: real.displayName };
    };

    const sessionSafe = findActiveUser(sessionU);
    const rememberedSafe = findActiveUser(remembered);

    // If we have a valid session user -> continue without login overlay
    if(sessionSafe){
      state.currentUser = sessionSafe;
      state.agentName = sessionSafe.displayName || 'משתמש';
      hideLogin_();
      if(!location.hash) setRoute('customers');
      return;
    }

    // Otherwise: require login. If "remember me" exists, just prefill username (no auto-login).
    if(remembered && !rememberedSafe){
      // remembered user no longer exists / disabled -> clear it
      try{ clearStoredUser_(); }catch(_){}
    }

    if(rememberedSafe && loginUser){
      try{ loginUser.value = rememberedSafe.username; }catch(_){}
      // focus password for fast login
      if(loginPass) setTimeout(() => { try{ loginPass.focus(); }catch(_){} }, 0);
    }
