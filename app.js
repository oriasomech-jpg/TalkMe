...
(() => {
  ...
  // ---------- Har HaBituach parser ----------
  function normalizeCompanyFromHar_(s){
    const t = String(s||'').trim();
    if(!t) return '';
    if(t.includes('הראל')) return 'הראל';
    if(t.includes('הפניקס')) return 'הפניקס';
    if(t.includes('מנורה')) return 'מנורה';
    if(t.includes('מגדל')) return 'מגדל';
    if(t.includes('כלל')) return 'כלל';
    if(t.includes('הכשרה')) return 'הכשרה';
    // fallback: keep first word
    return t.split(' ')[0];
  }

  function mapProductFromHar_(main, secondary){
    const a = String(main||'');
    const b = String(secondary||'');
    const s = (a + ' ' + b).trim();

    // direct matches
    if(s.includes('תאונות אישיות')) return 'תאונות אישיות';
    if(s.includes('דירה')) return 'דירה';
    if(s.includes('רכב')) return 'רכב';
    if(s.includes('משכנתא')) return 'משכנתא';
    if(s.includes('מחלות קשות')) return 'מחלות קשות';
    if(s.includes('סרטן')) return 'מחלות סרטן';
    if(s.includes('סיעודי') || s.includes('בריאות')) return 'בריאות';
    if(s.includes('אבדן כושר') || s.includes('ביטוח חיים') || s.includes('חיים') || s.includes('נכויות')) return 'חיים';
    if(s.includes('ריסק')) return 'ריסק';

    // fallback
    return 'בריאות';
  }

  function findHeaderRowIndex_(aoa, requiredKeys){
    for(let i=0;i<Math.min(aoa.length, 40);i++){
      const row = aoa[i] || [];
      const joined = row.map(x=>String(x||'')).join(' | ');
      const ok = requiredKeys.every(k => joined.includes(k));
      if(ok) return i;
    }
    return -1;
  }

  function parseHarAoaToPolicies_(aoa, opt){
    opt = opt || {};
    if(!Array.isArray(aoa) || !aoa.length) return { ok:false, msg:'הקובץ ריק או לא נקרא נכון' };

    const mode = opt.mode || 'auto';

    let headerIdx = 0;
    if(mode === 'auto'){
      headerIdx = findHeaderRowIndex_(aoa, ['מספר פוליסה','חברה']);
      if(headerIdx < 0) headerIdx = findHeaderRowIndex_(aoa, ['מספר','פוליסה']);
    ...
    // Try to resolve columns by contains (robust to minor changes)
    const colByContains = (needle) => {
      const k = Object.keys(colIndex).find(h => h.includes(needle));
      return (k!=null) ? colIndex[k] : -1;
    };

    const cMain = colByContains('ענף ראשי');
    const cSec = colByContains('ענף (משני)');
    const cCompany = colByContains('חברה');
    const cPremium = colByContains('פרמיה');
    const cPolicy = colByContains('מספר פוליסה');

    if(cCompany < 0 || cPolicy < 0){
      return { ok:false, msg:'לא נמצאו עמודות "חברה" ו/או "מספר פוליסה". נסה מצב "פשוט" או יצוא מחדש מהר הביטוח.' };
    }

    const policies = [];
    for(let i=headerIdx+1;i<aoa.length;i++){
      const r = aoa[i] || [];
      // Skip section title rows like "תחום - ..."
      const mainVal = (cMain>=0) ? String(r[cMain]||'').trim() : '';
      if(mainVal && mainVal.startsWith('תחום')) continue;

      const companyRaw = String(r[cCompany]||'').trim();
      const policyNo = String(r[cPolicy]||'').trim();
      if(!companyRaw && !policyNo) continue;

      const company = normalizeCompanyFromHar_(companyRaw);
      const secondaryVal = (cSec>=0) ? String(r[cSec]||'').trim() : '';
      const product = mapProductFromHar_(mainVal, secondaryVal);

      let premium = 0;
      const premRaw = (cPremium>=0) ? r[cPremium] : '';
      if(typeof premRaw === 'number') premium = premRaw;
      else premium = parseFloat(String(premRaw||'').replace(/[^0-9.]/g,'')) || 0;

      if(!policyNo || !company) continue;

      policies.push({ company, product, policyNo, premium });
    }
  ...
  function renderOldPoliciesStep(){
    ...
    wizardMain.innerHTML = `
      ...
      <div class="harBox">
        <div class="harHead">
          <div>
            <div class="harTitle">שליפת פוליסות קיימות מהר הביטוח</div>
            <div class="harSub">צרף קובץ Excel/CSV של “הר הביטוח” והמערכת תוסיף שורות פוליסה אוטומטית למבוטח הנוכחי.</div>
          </div>
          <button class="btn btnSoft" id="harToggleBtn">פתח</button>
        </div>
        <div class="harBody" id="harBody" style="display:none">
          <div class="grid3">
            <div>
              <div class="label">קובץ הר הביטוח (Excel/CSV)</div>
              <input class="input" id="harFile" type="file" accept=".xlsx,.xls,.csv" />
            </div>
            <div>
              <div class="label">אופן זיהוי עמודות</div>
              <select class="input" id="harMode">
                <option value="auto" selected>אוטומטי (מומלץ)</option>
                <option value="simple">פשוט (שורה ראשונה = כותרות)</option>
              </select>
            </div>
            <div style="display:flex;align-items:flex-end;gap:10px">
              <button class="btn btnPrimary" id="harParseBtn">שלח לעיבוד נתונים</button>
              <button class="btn btnSoft" id="harClearBtn" title="מוחק פוליסות שיובאו בקובץ האחרון בלבד">נקה יבוא אחרון</button>
            </div>
          </div>
          <div class="sideHint" style="margin-top:8px">
            ⚠️ חשוב: העלה קובץ שמור מהמחשב (לא קישור). אם לא נטען – בדוק שיש אינטרנט (לספריית XLSX) או נסה CSV.
          </div>
          <div class="harLog" id="harLog" style="margin-top:10px"></div>
        </div>
      </div>
    ...
    // --- Har HaBituach import ---
    const harToggleBtn = $('#harToggleBtn');
    const harBody = $('#harBody');
    const harLog = $('#harLog');
    const harFile = $('#harFile');
    const harMode = $('#harMode');
    const harParseBtn = $('#harParseBtn');
    const harClearBtn = $('#harClearBtn');

    const setHarLog = (html) => { if(harLog) harLog.innerHTML = html || ''; };

    if(harToggleBtn && harBody){
      harToggleBtn.addEventListener('click', () => {
        const open = harBody.style.display !== 'none';
        harBody.style.display = open ? 'none' : 'block';
        harToggleBtn.textContent = open ? 'פתח' : 'סגור';
      });
    }

    if(harParseBtn){
      harParseBtn.addEventListener('click', async () => {
        try{
          setHarLog('');
          if(!harFile || !harFile.files || !harFile.files[0]){
            toast('שגיאה', 'בחר קובץ הר הביטוח');
            return;
          }
          if(typeof XLSX === 'undefined'){
            setHarLog('<div class="harErr">לא נטענה ספריית XLSX (אין אינטרנט או נחסם). נסה להעלות CSV או פתח את האתר עם חיבור תקין.</div>');
            toast('שגיאה', 'ספריית XLSX לא נטענה');
            return;
          }
          ...
          if(!list.length){
            setHarLog('<div class="harWarn">לא נמצאו שורות פוליסה בקובץ (אחרי סינון כותרות/כותרות תחום).</div>');
          ...
          // remember last batch so we can "clear last import"
          p.__lastHarBatch = imported.length ? imported[0].__harBatch : null;

          const added = p.oldPolicies.length - before;
          setHarLog('<div class="harOk">יובאו '+ added +' פוליסות. עכשיו נשאר רק לבחור לכל אחת ביטול מלא/חלקי/מינוי סוכן וכו׳.</div>');
    ...
    if(harClearBtn){
      harClearBtn.addEventListener('click', () => {
        const batch = p.__lastHarBatch;
        if(!batch){
          toast('שימו לב', 'אין יבוא אחרון לניקוי');
          return;
        }
        const before = p.oldPolicies.length;
        p.oldPolicies = p.oldPolicies.filter(x => !(x && x.__harImported && x.__harBatch === batch && (x.insuredId||'main')===insuredId));
        const removed = before - p.oldPolicies.length;
        p.__lastHarBatch = null;
        toast('נוקה', 'נמחקו ' + removed + ' שורות שיובאו');
        renderOldPoliciesStep();
        updateSums();
        markDraft();
      });
