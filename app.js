...
(() => {
  ...
  // If true, always require a fresh login on page load (no auto session restore).
  // This prevents "auto-login" confusion when deploying to GitHub Pages where localStorage persists.
  const FORCE_LOGIN_EACH_LOAD = true;
  ...
  // ---------- Inline field validation (subtle, no toast) ----------
  function _getEl(elOrSel){
    if(!elOrSel) return null;
    if(typeof elOrSel === 'string') return $(elOrSel) || document.getElementById(elOrSel.replace(/^#/, ''));
    return elOrSel;
  }
  function _bindClearOnChange(el){
    if(!el || el.dataset._errBind === '1') return;
    el.dataset._errBind = '1';
    const clear = () => clearFieldError(el);
    el.addEventListener('input', clear);
    el.addEventListener('change', clear);
    el.addEventListener('blur', clear);
  }
  function setFieldError(elOrSel, msg){
    const el = _getEl(elOrSel);
    if(!el) return;
    const field = el.closest('.field');
    if(!field) return;
    field.classList.add('field--error');
    let m = field.querySelector('.fieldErrorMsg');
    if(!m){
      m = document.createElement('div');
      m.className = 'fieldErrorMsg';
      field.appendChild(m);
    }
    m.textContent = msg || 'שדה חובה';
    _bindClearOnChange(el);
  }
  function clearFieldError(elOrSel){
    ...
    const m = field.querySelector('.fieldErrorMsg');
    if(m) m.remove();
  }
  ...
  function loadSessionUser_(){
    try{
      ...
      if(obj && obj.username){
        ...
        const u = users.find(x => x.username === obj.username && x.active !== false);
        if(u){
  ...
  function saveSessionUser_(user){
    try{
  ...
  function authLogin_(username, password){
    ...
    if(String(u.password||'') !== String(password||'')) return { ok:false, msg:'שם משתמש או סיסמה שגויים' };
    const user = { username:u.username, role:u.role, displayName:u.displayName };
    saveSessionUser_(user);
    return { ok:true, user };
  }
...
function getUsers_(){
  ...
  function setLoginError_(msg){
    if(!loginErrEl) return;
    loginErrEl.textContent = msg || '';
    loginErrEl.style.display = msg ? '' : 'none';
  }
  function initAuthUI_(){
    if(loginFormEl){
      loginFormEl.addEventListener('submit', (e) => {
        e.preventDefault();
        const u = String(loginUserEl ? loginUserEl.value : '').trim();
        const p = String(loginPassEl ? loginPassEl.value : '');
        setLoginError_('');
        if(!u || !p){
          setLoginError_('יש להזין שם משתמש וסיסמה');
          return;
        }
        const r = authLogin_(u, p);
        if(!r.ok){
          setLoginError_(r.msg || 'שם משתמש או סיסמה שגויים');
          return;
        }
        state.currentUser = r.user;
        state.agentName = r.user.displayName || r.user.username;
        hideLogin_();
        applyPermissions_();
        render();
      });
...
// ---------- Routing ----------
  ...
  // ---------- Default route ----------
  // Keep a stable landing route. Login gating is handled by render().
...
// Keep wizard title in sync
  ...
  function renderOldPoliciesStep(){
    ...
    $('#addOldBtn').addEventListener('click', () => {
      ...
      if(!company || !product){
        if(!company) setFieldError('#old_company','יש לבחור חברת ביטוח');
        if(!product) setFieldError('#old_product','יש לבחור סוג ביטוח');
        return;
      }
...
function renderNewPoliciesStep(){
    ...
    $('#addNewBtn').addEventListener('click', () => {
      ...
      if(!company || !product){
        if(!company) setFieldError('#new_company','יש לבחור חברת ביטוח');
        if(!product) setFieldError('#new_product','יש לבחור מוצר');
        return;
      }
...
function debounce(fn, wait){
  ...
  // When FORCE_LOGIN_EACH_LOAD is true, we always require the user to login again.
  let sess = null;
  if(!FORCE_LOGIN_EACH_LOAD){
    sess = loadSessionUser_();
  }else{
    // Clear any persisted session so GitHub Pages deployments won't "skip" login unexpectedly.
    clearSessionUser_();
  }
  if(sess){
    state.currentUser = sess;
    state.agentName = sess.displayName || sess.username;
  }else{
    state.currentUser = null;
    state.agentName = 'אורח';
  }
