...
function renderKpisSettings(){
  ...
  function renderPayerStep(){
    const p = wiz.proposal;
    if(!p.payer) p.payer = { method:'card', card:{tz:'',name:'',number:'',exp:'',cvv:''}, hok:{bank:'',branch:'',account:''} };
    ...
    wizardMain.innerHTML = `
      ...
      <div class="payMethods" role="radiogroup" aria-label="שיטת תשלום">
        ...
        <button class="payMethod ${method==='hok'?'active':''}" data-pay-method="hok" type="button">
          ...
          <div class="pmSub">בנק + סניף + מספר חשבון</div>
        </button>
      </div>
      ...
      <div class="payPanel">
        ${method==='card' ? `
          <div class="grid3">
            ...
            ${field('מספר כרטיס', 'payer_card_number', p.payer.card.number || '', 'numeric')}
          </div>
          <div class="grid3" style="margin-top:10px">
            ${field('תוקף (MM/YY)', 'payer_card_exp', p.payer.card.exp || '', 'numeric')}
            ${field('CVV', 'payer_card_cvv', p.payer.card.cvv || '', 'numeric')}
            <div class="field"><div class="label">הערה</div><div class="value">CVV אופציונלי (אפשר להשאיר ריק)</div></div>
          </div>
        ` : `
          <div class="grid3">
    ...
    bindInput('payer_card_number', v => p.payer.card.number = v);
    bindInput('payer_card_exp', v => p.payer.card.exp = v);
    bindInput('payer_card_cvv', v => p.payer.card.cvv = v);
