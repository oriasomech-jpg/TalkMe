...
(() => {
  ...
  const CONFIG = Object.freeze(Object.assign({
    mode: 'prod',              // 'prod' | 'demo'
    serverUrl: '',             // Apps Script Web App /exec URL
    autoConnect: true,
    seedDemoData: false,
    allowNoLogin: false        // true = bypass login (NOT recommended for production)
  }, (typeof window !== 'undefined' && window.GEMEL_CONFIG) ? window.GEMEL_CONFIG : {}));

  const LS_SERVER_URL_KEY = 'gemel_server_url_v1';
  function getServerUrl_(){
    const fromCfg = String(CONFIG.serverUrl || '').trim();
    let fromLs = '';
    try{ fromLs = String(localStorage.getItem(LS_SERVER_URL_KEY) || '').trim(); }catch(_){}
    return fromCfg || fromLs || '';
  }
  function setServerUrl_(url){
    try{ localStorage.setItem(LS_SERVER_URL_KEY, String(url||'').trim()); }catch(_){}
  }

  // If true, the app will try to auto-connect on load (ping).
  const AUTO_CONNECT = !!CONFIG.autoConnect;

  // ---------- Server helpers ----------
  async function apiCall(action, payload){
    const baseUrl = getServerUrl_();
    if(!baseUrl) return { ok:false, demo:(CONFIG.mode!=='prod'), error:'NO_URL' };
    ...
    try{
      ...
      if(!res.ok){
        ...
        return { ok:false, error:'HTTP_' + res.status, details: text.slice(0, 300) };
      }
      ...
      return { ok:false, error:'FETCH', details:String(e) };
    }finally{
      clearTimeout(t);
    }
  }
  ...
  async function apiPing(){
    const baseUrl = getServerUrl_();
    if(!baseUrl) return { ok:false, demo:(CONFIG.mode!=='prod') };
  ...
  // Optional demo seed (only when CONFIG.seedDemoData === true)
  const DEMO_SEED = {
    agentName: 'אוריה (דמו)',
  ...
  if(CONFIG.seedDemoData){
    state.agentName = DEMO_SEED.agentName;
    state.teamAgents = DEMO_SEED.teamAgents.slice();
    state.customers = DEMO_SEED.customers.slice();
  }
  ...
  // ---------- Boot (Production-ready) ----------
  const SESSION_KEY = 'gemel_session_v1';

  function saveSession_(userSafe){
    try{ sessionStorage.setItem(SESSION_KEY, JSON.stringify(userSafe||null)); }catch(_){}
  }
  function loadSession_(){
    try{
      const raw = sessionStorage.getItem(SESSION_KEY);
      const obj = raw ? JSON.parse(raw) : null;
      if(obj && obj.username && obj.role && obj.displayName) return obj;
    }catch(_){}
    return null;
  }
  ...
  function authOverlay_(title, subtitle, fieldsHtml, actionsHtml){
    const overlay = document.createElement('div');
    overlay.className = 'authOverlay';
    overlay.innerHTML = `
      <div class="authCard" role="dialog" aria-modal="true">
        <h3 class="authTitle">${escapeHtml(title||'')}</h3>
        <p class="authSub">${escapeHtml(subtitle||'')}</p>
        <div class="authGrid">${fieldsHtml||''}</div>
        <div class="authError" id="authErr"></div>
        <div class="authActions">${actionsHtml||''}</div>
        <div class="authNote" style="display:${CONFIG.mode==='prod'?'block':'none'}">המלצה: לאחר התחברות ראשונה, כנס ל״הגדרות״ והחלף סיסמת מנהל.</div>
      </div>
    `;
    return overlay;
  }

  function showError_(overlay, msg){
  ...
  function tryLogin_(username, password){
    ...
    const u = users.find(x => x.username === username && x.password === password && x.active !== false);
  ...
  function showLogin_(){
    return new Promise((resolve) => {
      const overlay = authOverlay_(
        'התחברות למערכת',
        'GEMEL INVEST • CRM',
        `
          <div class="authRow">
            <div class="authLabel">שם משתמש</div>
            <input class="input" id="lgUser" autocomplete="username" placeholder="לדוגמה: admin">
          </div>
          <div class="authRow">
            <div class="authLabel">סיסמה</div>
            <input class="input" id="lgPass" type="password" autocomplete="current-password" placeholder="••••">
          </div>
        `,
        `
          <button class="btn btnSoft" id="lgHelp">עזרה</button>
          <button class="btn btnPrimary" id="lgGo">התחבר</button>
        `
      );

      document.body.appendChild(overlay);

      const userEl = overlay.querySelector('#lgUser');
      const passEl = overlay.querySelector('#lgPass');
      const go = overlay.querySelector('#lgGo');
      const help = overlay.querySelector('#lgHelp');

      const doLogin = () => {
        const u = String(userEl?.value||'').trim();
        const p = String(passEl?.value||'').trim();
        if(!u || !p){ showError_(overlay, 'חובה להזין שם משתמש וסיסמה'); return; }
        const r = tryLogin_(u, p);
        if(!r.ok){ showError_(overlay, r.msg); return; }
        state.currentUser = r.user;
        state.agentName = r.user.displayName;
        saveSession_(r.user);
        applyPermissions_();
        overlay.remove();
        resolve(true);
      ...
      help?.addEventListener('click', () => {
        showError_(overlay, 'אם שכחת סיסמה — פנה למנהל המערכת.');
      });
  ...
  function showFirstRunAdminSetup_(){
    return new Promise((resolve) => {
      const overlay = authOverlay_(
        'הגדרה ראשונית',
        'צור משתמש מנהל כדי להתחיל לעבוד (פעם אחת בלבד)',
        `
          <div class="authRow">
            <div class="authLabel">שם משתמש (Admin)</div>
            <input class="input" id="suUser" autocomplete="username" placeholder="admin">
          </div>
          <div class="authRow">
            <div class="authLabel">שם תצוגה</div>
            <input class="input" id="suName" placeholder="מנהל מערכת">
          </div>
          <div class="authRow">
            <div class="authLabel">סיסמה</div>
            <input class="input" id="suPass" type="password" autocomplete="new-password" placeholder="בחר סיסמה חזקה">
          </div>
        `,
        `
          <button class="btn btnPrimary" id="suGo">צור מנהל</button>
        `
      );

      document.body.appendChild(overlay);

      const uEl = overlay.querySelector('#suUser');
      const nEl = overlay.querySelector('#suName');
      const pEl = overlay.querySelector('#suPass');
      const go = overlay.querySelector('#suGo');

      const doSetup = () => {
        const username = String(uEl?.value||'').trim();
        const displayName = String(nEl?.value||'').trim();
        const password = String(pEl?.value||'').trim();
        if(!username || !displayName || !password){
          showError_(overlay, 'חובה למלא את כל השדות'); return;
        }
        const users = loadUsers_();
        if(users.find(x => x.username === username)){
          showError_(overlay, 'שם משתמש כבר קיים'); return;
        }
        const nowIso = new Date().toISOString();
        users.unshift({ username, password, role:'admin', displayName, active:true, createdAt: nowIso, updatedAt: nowIso });
        saveUsers_(users);

        const safe = { username, role:'admin', displayName };
        state.currentUser = safe;
        state.agentName = displayName;
        saveSession_(safe);
        applyPermissions_();
      ...
      pEl?.addEventListener('keydown', (e) => { if(e.key==='Enter') doSetup(); });
  ...
  function wireLogout_(){
    ...
    if(!badge) return;
    if(badge.querySelector('[data-logout]')) return;

    const btn = document.createElement('button');
    btn.className = 'btn btnTiny';
    btn.textContent = 'התנתק';
    btn.setAttribute('data-logout','1');
  ...
  async function boot_(){
    state.server.url = getServerUrl_();

    const users = getUsers_();
    if(!users.length && !CONFIG.seedDemoData){
      if(CONFIG.allowNoLogin){
        const nowIso = new Date().toISOString();
        saveUsers_([{ username:'admin', password:'', role:'admin', displayName:'מנהל מערכת', active:true, createdAt: nowIso, updatedAt: nowIso }]);
      }else{
        await showFirstRunAdminSetup_();
      }
    }

    if(CONFIG.allowNoLogin){
      const list = getUsers_();
      const admin = list.find(u => u.role==='admin' && u.active !== false) || list[0] || { username:'guest', role:'agent', displayName:'אורח' };
      state.currentUser = { username: admin.username, role: admin.role, displayName: admin.displayName };
    ...
    if(sess){
      ...
      state.agentName = sess.displayName;
      applyPermissions_();
      wireLogout_();
      if(!location.hash) setRoute('customers');
      return;
...
function renderKpisSettings(){
  ...
  function renderSettings(){
    ...
    $('#connectBtn').addEventListener('click', async () => {
      ...
      setServerUrl_(url);
      // NOTE: כתובת השרת נשמרת מקומית (localStorage) ו/או דרך GEMEL_CONFIG.
    ...
    $('#pullBtn').addEventListener('click', async () => {
      if(!getServerUrl_()){ toast('הגדרה חסרה', 'אין כתובת שרת מוגדרת לסנכרון'); return; }
      await syncDown();
...
function gotoStep(i){
  ...
  function renderWizard(){
    ...
    try{
      ...
      // Ensure insurer logo pickers (if present) are wired after each step render
      if(typeof wireInsurerPickers === 'function') wireInsurerPickers();
    } catch(err){
...
`;
  ...
  function selectField(label, id, options){
    // Special case: insurer selection -> show only logos (no text), while keeping the value for reports
    try{
      if(label === 'חברה' && Array.isArray(options) && typeof INSURER_LOGOS === 'object' && options.every(o => INSURER_LOGOS[String(o)])){
        return insurerLogoPickerField(label, id, options);
      }
    } catch(e){ /* fallback to normal select */ }
...
function setSelectValue(id, value){
    ...
    // If it's a standard <select>, keep existing behavior
    ...
    // Otherwise (e.g., hidden input used by insurer logo picker)
    el.value = value || '';
    try{ updateInsurerPickerUI(id, el.value); } catch(_){}
  }
...
function debounce(fn, wait){
  ...
  (async function init_(){
    ...
    state.server.url = getServerUrl_() || '';
    if(AUTO_CONNECT && state.server.url){
      const ping = await apiPing();
      state.connected = !!ping.ok;
      ...
      if(state.connected){
        ...
        const msg = (ping && ping.error) ? ('לא הצלחנו להתחבר לשרת (' + ping.error + ')') : 'לא הצלחנו להתחבר לשרת';
        toast('לא מחובר', msg);
      }
      render();
    }else{
      if(CONFIG.mode === 'prod' && !state.server.url){
        toast('הגדרה חסרה', 'חבר שרת ב"הגדרות" → כתובת Web App');
      }
    }
  })();
...
// ===== Insurance Logos Mapping (SCOPED, non-breaking) =====
const INSURER_LOGOS = {
  'הכשרה':'assets/achshara.svg.png',
  'הפניקס':'assets/afenix.svg.png',
  'כלל':'assets/clal.svg.png',
  'הראל':'assets/harel.svg.png',
  'מגדל':'assets/megdl.svg.png',
  'מנורה':'assets/menora.svg.png'
};

function renderInsurerLogo(name, size=''){
  const src = INSURER_LOGOS[name];
  if(!src) return '';
  return `<img class="insurer-logo ${size}" src="${src}" alt="${escapeAttr(name)}">`;
}

/**
 * Insurer picker field: shows ONLY logos (no text) in the UI.
 * The chosen value is stored in a hidden input with the same id, so reports keep the company name.
 */
function insurerLogoPickerField(label, id, options){
  const chips = options.map(name => {
    const src = INSURER_LOGOS[String(name)];
    return `
      <button type="button" class="insurer-chip" data-value="${escapeAttr(name)}" aria-label="${escapeAttr(name)}" title="${escapeAttr(name)}">
        <img class="insurer-logo" src="${src}" alt="${escapeAttr(name)}" />
      </button>
    `;
  }).join('');

  return `
    <div class="field insurerField">
      <label>${escapeHtml(label)}</label>
      <div class="insurer-picker" data-target="${escapeAttr(id)}">
        ${chips}
      </div>
      <input type="hidden" id="${escapeAttr(id)}" value="" />
    </div>
  `;
}

function updateInsurerPickerUI(id, value){
  const esc = (window.CSS && CSS.escape) ? CSS.escape(id) : id;
  const host = document.querySelector(`.insurer-picker[data-target="${esc}"]`);
  if(!host) return;
  host.querySelectorAll('.insurer-chip').forEach(btn => {
    const v = btn.getAttribute('data-value') || '';
    btn.classList.toggle('isSelected', v === value);
  });
}

function wireInsurerPickers(root=document){
  root.querySelectorAll('.insurer-picker').forEach(host => {
    ...
    host.addEventListener('click', (e) => {
      const btn = e.target.closest('.insurer-chip');
      if(!btn) return;
      const value = btn.getAttribute('data-value') || '';
      if(input){
        input.value = value;
        // Trigger change for any listeners (safe even if none)
        try{ input.dispatchEvent(new Event('change', { bubbles:true })); }catch(_){}
      }
      updateInsurerPickerUI(targetId, value);
      try{ if(typeof markDraft === 'function') markDraft(); }catch(_){}
    });

    // If the input already has a value (e.g., editing), reflect it
    if(input && input.value) updateInsurerPickerUI(targetId, input.value);
  });
}
