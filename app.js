...
function gotoStep(i){
  ...
  function renderCustomerStep(){
    ...
    bindSelect('customer_smoker', v => {
      d.smoker = v;
      toggleSmokeRow(v === 'כן');
      if(v !== 'כן'){ d.smokePerDay = ''; d.cannabisMedical = false; d.cannabisSocial = false; const el = document.getElementById('customer_smokePerDay'); if(el) el.value = ''; }
    });
...
function renderNewPoliciesStep(){
    ...
    wizardMain.innerHTML = `
      ...
      <div class="tableWrap">
        <table>
          <thead><tr><th>חברה</th><th>מוצר</th><th>פרמיה</th><th>הנחה</th><th></th></tr></thead>
          <tbody>
            ${rows.length ? rows.map(({x,idx}) => `
              <tr>
                <td>${escapeHtml(x.company)}</td>
                ...
                <td style="min-width:190px">
                  <button class="btn btnSoft" data-edit-discount="${idx}">${x.discountNote ? 'הנחה עודכנה' : 'עדכן הנחה'}</button>
                  ${x.discountNote ? `<div class="discPreview">${escapeHtml(clipText(x.discountNote, 60))}</div>` : ``}
                </td>
                <td><button class="btn btnSoft" data-del-new="${idx}">מחיקה</button></td>
              </tr>
            `).join('') : `<tr><td colspan="5" style="color:rgba(18,19,25,.55)">אין רכישות חדשות למבוטח זה עדיין.</td></tr>`}
          </tbody>
        </table>
      </div>
    ...
    $$('[data-edit-discount]').forEach(b => b.addEventListener('click', () => {
      const idx = parseInt(b.getAttribute('data-edit-discount'),10);
      const cur = (p.newPolicies[idx] && p.newPolicies[idx].discountNote) ? String(p.newPolicies[idx].discountNote) : '';
      openTextModal_({
        title: 'הנחה לפוליסה',
        subtitle: 'רשום במלל מה ההנחה שניתנה (לדוגמה: 10% על ריסק ל-12 חודשים / פטור מדמי ניהול / הטבה חד-פעמית).',
        placeholder: 'הקלד כאן את ההנחה…',
        value: cur,
        onSave: (val) => {
          if(!p.newPolicies[idx]) return;
          p.newPolicies[idx].discountNote = (val||'').trim();
          renderNewPoliciesStep();
          updateSums();
          markDraft();
        }
        ...
        ${list.map(ins => {
          ...
          return `
            <div class="sideCard" style="box-shadow:none">
              ...
              ${d.smoker === 'כן' && (d.cannabisMedical || d.cannabisSocial) ? renderKV('קנאביס', [d.cannabisMedical ? 'רפואי' : '', d.cannabisSocial ? 'חברתי' : ''].filter(Boolean).join(' + ')) : ''}
              ${d.smoker === 'כן' && (d.cannabisMedical || d.cannabisSocial) ? renderKV('קנאביס', [d.cannabisMedical ? 'רפואי' : '', d.cannabisSocial ? 'חברתי' : ''].filter(Boolean).join(' + ')) : ''}
      ...
      return `
        <div class="ibox">
          ...
          <div class="igrid">
            ...
            ${d.smoker === 'כן' && (d.cannabisMedical || d.cannabisSocial) ? ('<div><b>קנאביס:</b> ' + esc([d.cannabisMedical ? 'רפואי' : '', d.cannabisSocial ? 'חברתי' : ''].filter(Boolean).join(' + ')) + '</div>') : ''}
      ...
      try{
        ...
        w.document.write(`<html lang="he" dir="rtl"><head><meta charset="UTF-8"><title>שגיאה</title></head>
          <body style="font-family:Arial;padding:18px">
            ...
            <div>בדרך כלל זה קורה בגלל נתון לא תקין במסך הסיכום.</div>
            <div style="margin-top:10px" class="muted">פתח קונסול (F12) כדי לראות פרטים.</div>
          </body></html>`);
        w.document.close();
      }catch(_){}
...
function setSelectValue(id, value){
  ...
  // Small textarea modal (scoped) for notes like "discount"
  function openTextModal_({title='עריכה', subtitle='', placeholder='', value='', onSave}={}){
    // Remove existing if any
    const old = document.getElementById('textModalOverlay');
    if(old) old.remove();

    const overlay = document.createElement('div');
    overlay.id = 'textModalOverlay';
    overlay.className = 'discOverlay';
    overlay.innerHTML = `
      <div class="discBackdrop" aria-hidden="true"></div>
      <div class="discCard" role="dialog" aria-modal="true" aria-label="${escapeAttr(title)}">
        <div class="discTop">
          <div>
            <div class="discTitle">${escapeHtml(title)}</div>
            ${subtitle ? `<div class="discSub">${escapeHtml(subtitle)}</div>` : ``}
          </div>
          <button class="btn btnSoft discClose" type="button" aria-label="סגור">✕</button>
        </div>
        <div class="discBody">
          <textarea class="discTextarea" id="discTextarea" placeholder="${escapeAttr(placeholder)}"></textarea>
          <div class="discActions">
            <button class="btn btnPrimary" id="discSaveBtn" type="button">שמור</button>
            <button class="btn btnSoft" id="discCancelBtn" type="button">ביטול</button>
          </div>
        </div>
      </div>
    `;
    ...
    const ta = overlay.querySelector('#discTextarea');
    ta.value = String(value ?? '');
    setTimeout(() => { try{ ta.focus(); ta.selectionStart = ta.value.length; ta.selectionEnd = ta.value.length; }catch(e){} }, 0);

    const close = () => overlay.remove();
    overlay.querySelector('.discBackdrop').addEventListener('click', close);
    overlay.querySelector('.discClose').addEventListener('click', close);
    overlay.querySelector('#discCancelBtn').addEventListener('click', close);
    overlay.querySelector('#discSaveBtn').addEventListener('click', () => {
      const val = ta.value;
    ...
    overlay.addEventListener('keydown', (e) => {
      ...
      if((e.ctrlKey || e.metaKey) && e.key === 'Enter'){
        e.preventDefault();
        overlay.querySelector('#discSaveBtn').click();
      }
    });
  }
