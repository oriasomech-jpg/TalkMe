...
// Keep wizard title in sync
  ...
  <button class="btn btnSoft" id="hbImportBtn" title="ייבוא אוטומטי מקובץ הר הביטוח">משוך פוליסות קיימות</button>
</div>
      ...
      hbBtn.addEventListener('click', () => {
        openHbImportModal({
          ...
          onImport: (items) => {
            ...
            // Dedup by company|policyNo|product|premium for this insured
            const seen = new Set(p.oldPolicies.map(op => {
              const iid = op.insuredId || 'main';
              const key = [iid, (op.company||'').trim(), String(op.policyNo||'').trim(), (op.product||'').trim(), String(op.premium||0)].join('|');
              return key;
            }));
    ...
    wrap.innerHTML = `
      <div class="hbImportCard" role="dialog" aria-modal="true" aria-labelledby="hbImportTitle">
        ...
        <div class="hbImportActions">
          ...
          <button class="btn btnSoft" id="hbImportCancel">סגור</button>
        </div>
      </div>
    `;
