...
(() => {
  ...
  // Used for logo-only pickers in the wizard (display only). Stored values remain the company names.
  const COMPANY_LOGOS = {
    'הכשרה': 'achshara.svg.png',
    'הפניקס': 'afenix.svg.png',
    'כלל': 'clal.svg.png',
    'הראל': 'harel.svg.png',
    'מגדל': 'megdl.svg.png',
    'מנורה': 'menora.svg.png',
  };
...
// Keep wizard title in sync
  ...
  function renderOldPoliciesStep(){
    ...
    wizardMain.innerHTML = `
      ...
      <div class="grid2">
        ${companyLogoPickerField('חברה', 'old_company', ['הראל','כלל','מנורה','הפניקס','מגדל','הכשרה'])}
    ...
    wireCompanyLogoPicker('old_company');
...
function renderNewPoliciesStep(){
    ...
    wizardMain.innerHTML = `
      ...
      <div class="grid3">
        ${companyLogoPickerField('חברה', 'new_company', ['הראל','כלל','מנורה','הפניקס','מגדל','הכשרה'])}
    ...
    `;
    wireCompanyLogoPicker('new_company');
...
`;
  ...
  function companyLogoPickerField(label, id, companies){
    const opts = (companies||[]).map(name => `<option value="${escapeAttr(name)}">${escapeHtml(name)}</option>`).join('');
    const btns = (companies||[]).map((name, i) => {
      const src = companyLogoSrc(name);
      const safe = escapeAttr(name);
      const cls = i===0 ? 'isActive' : '';
      return `
        <button type="button" class="companyLogoBtn ${cls}" data-company="${safe}" aria-label="${escapeAttr(label + ' ' + name)}" title="${escapeAttr(name)}">
          ${src ? `<img class="companyLogoImg" src="${escapeAttr(src)}" alt="${escapeAttr(name)}">` : `<span class="companyLogoText">${escapeHtml(name)}</span>`}
        </button>
      `;
    }).join('');
    return `
      <div class="field companyLogoField">
        <label for="${id}">${escapeHtml(label)}</label>
        <select id="${id}" class="companyHiddenSelect">${opts}</select>
        <div class="companyLogoList" data-company-picker="${escapeAttr(id)}">
          ${btns}
        </div>
      </div>
    `;
  ...
  function wireCompanyLogoPicker(selectId){
    const sel = document.getElementById(selectId);
    const host = document.querySelector(\`[data-company-picker="\${selectId}"]\`);
    if(!sel || !host) return;

    // ensure select has a value (default first option)
    if(!sel.value && sel.options && sel.options.length) sel.value = sel.options[0].value;

    const setActive = (val) => {
      sel.value = val;
      // mark active
      host.querySelectorAll('.companyLogoBtn').forEach(b => {
        b.classList.toggle('isActive', b.dataset.company === val);
      });
      sel.dispatchEvent(new Event('change', { bubbles:true }));
    };

    host.querySelectorAll('.companyLogoBtn').forEach(btn => {
      btn.addEventListener('click', () => setActive(btn.dataset.company || ''));
    });

    // sync initial from select value if any
    setActive(sel.value);
  }
