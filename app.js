...
// ---------- CONFIG (Google Sheets via Apps Script Web App) ----------
  ...
  const PARTIAL_CANCEL_ENDORSEMENTS = ["ניתוחים ומחליפי ניתוח בחו\"ל", "השתלות וטיפולים מיוחדים בחו\"ל", "תרופות", "משלים שב\"ן לניתוחים ומחליפי ניתוח בישראל עם השתתפות עצמית של 5,000 ₪", "משלים שב\"ן לניתוחים ומחליפי ניתוח בישראל", "ניתוחים ומחליפי ניתוח בישראל", "ייעוץ ובדיקות", "אבחון רפואי מהיר", "ייעוץ, בדיקות ואבחון רפואי מהיר", "טיפולים בטכנולוגיות מתקדמות ואביזרים רפואיים", "ליווי אישי פלוס", "שירותים לילד", "ביקור רופא און-ליין אקסטרה", "רפואה משלימה"];

  function openPartialCancelModal_(opts){
    // opts: { selected: string[], onSave(selected), onCancel() }
    try{
      const selectedSet = new Set(Array.isArray(opts?.selected) ? opts.selected : []);
      const overlay = document.createElement('div');
      overlay.className = 'pcOverlay';
      overlay.innerHTML = `
        <div class="pcModal" role="dialog" aria-modal="true" aria-label="ביטול חלקי">
          <div class="pcHead">
            <div>
              <div class="pcTitle">ביטול חלקי</div>
              <div class="pcSub">בחר נספחים לביטול וסמן ✔️</div>
            </div>
            <button class="btn btnSoft pcX" type="button" aria-label="סגור">סגור</button>
          </div>

          <div class="pcBody">
            <div class="pcList">
              ${PARTIAL_CANCEL_ENDORSEMENTS.map((name,i) => {
                const checked = selectedSet.has(name) ? 'checked' : '';
                const id = 'pc_' + i;
                return `
                  <label class="pcItem" for="${id}">
          ...
          <div class="pcFoot">
            <button class="btn btnSoft" type="button" data-pc-cancel>ביטול</button>
            <button class="btn btnPrimary" type="button" data-pc-save>שמור</button>
          </div>
        </div>
      `;
      document.body.appendChild(overlay);
      ...
      overlay.querySelector('.pcX')?.addEventListener('click', cancel);
      overlay.querySelector('[data-pc-cancel]')?.addEventListener('click', cancel);

      overlay.querySelector('[data-pc-save]')?.addEventListener('click', () => {
        const checks = Array.from(overlay.querySelectorAll('input[type="checkbox"]'));
        const chosen = checks.filter(c => c.checked).map(c => String(c.value||''));
        if(chosen.length < 1){
          toast('שגיאה', 'חובה לבחור לפחות נספח אחד לביטול');
          return;
        }
        close();
        try{ opts?.onSave && opts.onSave(chosen); }catch(_){}
      });
    }catch(e){
      // Fallback: never block user
      const note = prompt('ציין את הנספחים לביטול (מופרד בפסיק)', (opts?.selected||[]).join(', '));
      if(note === null){ opts?.onCancel && opts.onCancel(); return; }
      const chosen = String(note||'').split(',').map(s=>s.trim()).filter(Boolean);
      if(!chosen.length){ toast('שגיאה','חובה לבחור לפחות נספח אחד'); opts?.onCancel && opts.onCancel(); return; }
      opts?.onSave && opts.onSave(chosen);
    }
  }
...
// Keep wizard title in sync
  ...
  function renderOldPoliciesStep(){
    ...
    // partial cancel attachments on form (cancel_partial)
    if($('#old_decision')){
      $('#old_decision').addEventListener('change', () => {
        const sel = $('#old_decision');
        const addBtn = $('#addOldBtn');
        if(!sel || !addBtn) return;

        if(sel.value === 'cancel_partial'){
          const prev = addBtn.dataset.partialCancelEndorsements ? JSON.parse(addBtn.dataset.partialCancelEndorsements || '[]') : [];
          openPartialCancelModal_({
            selected: Array.isArray(prev) ? prev : [],
            onCancel: () => { sel.value = 'cancel_full'; },
            onSave: (chosen) => {
              // If all endorsements were chosen, treat as full cancel
              const all = PARTIAL_CANCEL_ENDORSEMENTS.length;
              const list = Array.isArray(chosen) ? chosen : [];
              if(list.length >= all){
                sel.value = 'cancel_full';
                delete addBtn.dataset.partialCancelEndorsements;
                toast('עודכן', 'סומן כביטול מלא (כל הנספחים בוטלו)');
                return;
              }
              addBtn.dataset.partialCancelEndorsements = JSON.stringify(list);
              toast('נשמר', 'נספחים לביטול חלקי נשמרו');
            }
          });
        } else {
          if(addBtn) delete addBtn.dataset.partialCancelEndorsements;
        }
      });
    }
    ...
    $('#addOldBtn').addEventListener('click', () => {
      ...
      const partialCancelEndorsements = (decision==='cancel_partial')
        ? (function(){
            try{ return JSON.parse(String($('#addOldBtn')?.dataset?.partialCancelEndorsements || '[]')) || []; }catch(_){ return []; }
          })()
        : [];
      p.oldPolicies.push({
        insuredId,
        ...
        decision,
        partialCancelEndorsements
      });
      if($('#addOldBtn') && $('#addOldBtn').dataset) delete $('#addOldBtn').dataset.partialCancelEndorsements;
    ...
    $$('[data-old-decision]').forEach(sel => sel.addEventListener('change', () => {
      ...
      const applyDecision = (decision, chosenEndorsements) => {
        const d = decision || 'cancel_full';
        item.decision = d;

        if(d === 'cancel_partial'){
          item.partialCancelEndorsements = Array.isArray(chosenEndorsements) ? chosenEndorsements : [];
        }else{
          item.partialCancelEndorsements = [];
        }
        ...
        // ensure UI reflects final decision
        sel.value = d;
      ...
      if(v === 'cancel_partial'){
        openPartialCancelModal_({
          selected: Array.isArray(item.partialCancelEndorsements) ? item.partialCancelEndorsements : [],
          onCancel: () => {
            sel.value = prev;
          },
          onSave: (chosen) => {
            ...
            const all = PARTIAL_CANCEL_ENDORSEMENTS.length;
            if(list.length >= all){
              // all endorsements cancelled => full cancel
              applyDecision('cancel_full', list);
            }else{
              applyDecision('cancel_partial', list);
            }
          }
        });
        return;
      ...
      // switching away from partial => clear selections
      applyDecision(v, []);
    }));
