...
function gotoStep(i){
  ...
  function buildInsuredTabs(){
    ...
    $$('[data-insured]', insuredTabsEl).forEach(b => b.addEventListener('click', (e) => {
      ...
      buildInsuredTabs();
      const k = WIZ_STEPS[wiz.stepIndex].key;
      if(k === 'medical'){
        wiz.medicalStepIndex = 0;
      }
      // Re-render current step for any insured switch (prevents data "bleed" perception)
      renderWizard();
    }));
  ...
  function renderCustomerStep(){
    ...
    wizardMain.innerHTML = `
      ...
      <div class="grid3" style="margin-top:10px;display:none" id="smokeRow">
        <div class="field">
  ...
  function renderOldPoliciesStep(){
    const p = wiz.proposal;
    const activeId = p.activeInsuredId;
    const rows = [];
    p.oldPolicies.forEach((x,idx)=>{ if((x.insuredId||'main')===activeId){ rows.push({x,idx}); }});
    const rowsHtml = rows.length ? rows.map(({x,idx}) => `
              <tr>
            ...
            `).join('') : `<tr><td colspan="4" style="color:rgba(18,19,25,.55)">אין פוליסות קיימות למבוטח זה עדיין.</td></tr>`;

    wizardMain.innerHTML = `
      <div class="sectionTitle">פוליסות קיימות</div>
      <div class="grid2">
        ${selectField('מבוטח', 'old_insured', p.insuredList.map(i => i.id+'|'+i.label))}
    ...
    `;
        ...
        // Default insured selection to current tab
    const activeLabel = (p.insuredList.find(i=>i.id===activeId)||{}).label || '';
    if($('#old_insured')) setSelect('old_insured', activeId + '|' + activeLabel);

    $('#addOldBtn').addEventListener('click', () => {
      const insuredRaw = $('#old_insured') ? $('#old_insured').value : (p.activeInsuredId+'|');
      const insuredId = String(insuredRaw).split('|')[0] || p.activeInsuredId;
      ...
      p.oldPolicies.push({ insuredId, company, product, premium });
  ...
  function renderNewPoliciesStep(){
    const p = wiz.proposal;
    const activeId = p.activeInsuredId || 'main';
    const activeLabel = (p.insuredList.find(i=>i.id===activeId)||{}).label || '';

    // show only purchases of the currently selected insured tab
    const rows = [];
    p.newPolicies.forEach((x, idx) => {
      if((x.insuredId || 'main') === activeId) rows.push({ x, idx });
    });

    wizardMain.innerHTML = `
      <div class="sectionTitle">רכישות חדשות – ${escapeHtml(labelInsured(activeId))}</div>
      <div class="grid2">
    ...
    // default insured selection to current insured tab
    if($('#new_insured')) setSelect('new_insured', activeId + '|' + activeLabel);

    $('#addNewBtn').addEventListener('click', () => {
      const insuredRaw = $('#new_insured') ? $('#new_insured').value : (activeId + '|' + activeLabel);
      const insuredId = (insuredRaw||'').split('|')[0] || activeId || 'main';
...
function renderMedicalStep(){
  ...
  // Backward-compat alias (some steps used setSelect)
