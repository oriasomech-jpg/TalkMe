...
function debounce(fn, wait){
  ...
  function ensureHbImportModal(){
    ...
    const overlay = document.createElement('div');
    // Reuse existing modal overlay styling
    overlay.className = 'modalOverlay hidden';
    overlay.id = 'hbImportOverlay';
    overlay.setAttribute('aria-hidden','true');

    overlay.innerHTML = `
      <div class="usersDialog" style="max-width:720px">
        <div class="usersDialogHead">
          <div>
            <div class="usersDialogTitle">העלה קובץ הר הביטוח</div>
            <div class="usersDialogSub">XLSX / CSV • המערכת תזהה חברה, מס׳ פוליסה, מוצר ופרמיה</div>
          </div>
          <button class="btn btnSoft" data-hb-x>סגור</button>
        </div>

        <div class="usersDialogBody">
          <div style="display:flex; flex-direction:column; gap:10px">
            <div class="box" style="margin:0">
              <div class="boxTitle">בחירת קובץ</div>
              <input id="hbFileInput" class="input" type="file" accept=".xlsx,.xls,.csv"/>
              <div class="muted" style="margin-top:6px">טיפ: אם אין טעינת XLSX בדפדפן – ייצא מהר הביטוח כ‑CSV וייבא.</div>
            </div>

            <div id="hbImportMeta" class="muted"></div>
            <div id="hbImportErr" class="muted" style="display:none; color:#b42318; font-weight:800"></div>

            <div style="display:flex; gap:10px; justify-content:flex-end; margin-top:4px">
              <button class="btn btnSoft" data-hb-x>ביטול</button>
              <button class="btn btnPrimary" id="hbImportDo">ייבוא</button>
            </div>
          </div>
        </div>
      </div>
    ...
    overlay.addEventListener('click', (e) => {
      const t = e.target;
      if(t === overlay) closeHbImportModal();
      if(t && t.matches && t.matches('[data-hb-x]')) closeHbImportModal();
    });
  }
