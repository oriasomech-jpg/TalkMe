...
(() => {
  ...
  (function bootAuth_(){
    ...
    if(u && u.username){
      const full = getUserByUsername_(u.username);

      // If session points to a missing/disabled user, force login (prevents "frozen" state)
      ...
      state.currentUser = {
        ...
        role: full.role || 'agent',
        displayName: full.displayName || full.username,
        agencyRole: full.agencyRole || ''
      };
      state.agentName = state.currentUser.displayName || state.agentName;
