...
(() => {
  ...
  // ---------- Insurance company logos (wizard pickers) ----------
  const COMPANY_LOGOS = {
    'הכשרה':'assets/logos/achshara.svg.png',
    'הפניקס':'assets/logos/afenix.svg.png',
    'כלל':'assets/logos/clal.svg.png',
    'הראל':'assets/logos/harel.svg.png',
    'מגדל':'assets/logos/megdl.svg.png',
    'מנורה':'assets/logos/menora.svg.png'
  };
...
function renderKpisSettings(){
  ...
  function validateCurrentStep(){
    ...
    for(const el of controls){
      ...
      if(v === ''){
        // Special handling for logo-based company picker (select is offscreen)
        if(el.classList && el.classList.contains('companySelect')){
          const wrap = el.closest('.companyPickerWrap');
          if(wrap) wrap.classList.add('reqError');
          toast('חסר מידע', 'יש להשלים את כל השדות לפני מעבר לשלב הבא');
...
`;
  ...
  // Company picker: logo-only selection (keeps hidden select for existing logic)
  function companyPickerField(label, id, options){
    ...
    const tiles = options.map(o => {
      const [val] = String(o).includes('|') ? String(o).split('|') : [o,o];
      const logo = COMPANY_LOGOS[val] || '';
      const alt = val;
      return `
        <button type="button" class="companyTile" data-company="${escapeAttr(val)}" title="${escapeAttr(val)}" aria-label="${escapeAttr(val)}">
          ${logo ? `<img src="${escapeAttr(logo)}" alt="${escapeAttr(alt)}" loading="lazy" />` : `<span class="companyFallback">${escapeHtml(val.slice(0,2))}</span>`}
        </button>
      `;
    }).join('');

    return `
      <div class="field companyPickerWrap">
        <label>${escapeHtml(label)}</label>

        <!-- keep the select for value + validation; visually hidden offscreen -->
        <select id="${id}" class="companySelect" style="position:absolute;left:-9999px;top:auto;width:1px;height:1px;opacity:.01">${opts}</select>

        <div class="companyPicker" role="listbox" aria-label="${escapeAttr(label)}">
          ${tiles}
        </div>
      </div>
    `;
  ...
  function initCompanyPicker(selectId){
    const sel = document.getElementById(selectId);
    if(!sel) return;
    const wrap = sel.closest('.companyPickerWrap');
    if(!wrap) return;
    const tiles = Array.from(wrap.querySelectorAll('.companyTile'));
    const setVal = (v) => {
      if(v && Array.from(sel.options).some(o => o.value === v)){
        sel.value = v;
      }
      tiles.forEach(t => t.classList.toggle('isSelected', t.dataset.company === sel.value));
      wrap.classList.remove('reqError');
      markDraft();
    };
    ...
    tiles.forEach(t => {
      t.addEventListener('click', () => setVal(t.dataset.company));
      t.addEventListener('keydown', (e) => {
        if(e.key==='Enter' || e.key===' '){
          e.preventDefault();
          setVal(t.dataset.company);
        }
      });
    });
  }
