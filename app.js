...
function gotoStep(i){
  ...
  function renderCustomerStep(){
    ...
    const updateBMI = () => {
      ...
      // Sanity check (generic, for all ages). Flags obviously out-of-range values.
      if(flagEl){
        let ok = true;
        const reasons = [];
        if(h < 40 || h > 240){ ok = false; reasons.push('גובה'); }
        if(w < 2 || w > 250){ ok = false; reasons.push('משקל'); }
        if(rounded < 10 || rounded > 60){ ok = false; reasons.push('BMI'); }

        flagEl.textContent = ok ? 'תקין' : 'חריג';
        flagEl.classList.toggle('ok', ok);
        flagEl.classList.toggle('bad', !ok);
        flagEl.title = ok ? 'ערכים בטווח הגיוני' : ('ערכים חריגים: ' + reasons.join(', '));
      }
    };    };
