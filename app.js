...
(() => {
  ...
  function normalizeUsername_(s){
    return String(s||'').trim().replace(/\s+/g,' ').replace(/\s/g,'_');
  }
  function isValidCode_(code){
    const c = String(code||'').trim();
    return /^\d{4,8}$/.test(c);
  }
  function ensureUniqueUsername_(base){
    let u = normalizeUsername_(base) || ('agent_' + Math.floor(Math.random()*9000+1000));
    const exists = (name) => USERS.some(x => String(x.username).toLowerCase() === String(name).toLowerCase());
    if(!exists(u)) return u;
    // add suffix
    let i = 2;
    while(exists(u + '_' + i) && i < 999) i++;
    return u + '_' + i;
  }

  let USERS = loadUsers_();

  function syncTeamAgentsFromUsers_(){
    // Pull agent display names from USERS, keep current agentName if already set
    const agents = USERS.filter(u => u.role === 'agent').map(u => u.displayName).filter(Boolean);
    state.teamAgents = agents.length ? agents : state.teamAgents;
  }
  syncTeamAgentsFromUsers_();
    ...
    viewEl.innerHTML = `
      <div class="grid2">
        ...
        <div class="sideCard">
          ...
          <div class="sideHint">יצירת נציגים חדשים (שם + קוד). כניסה: <b>שם משתמש = השם</b>, <b>סיסמה = הקוד</b>.</div>
          <div style="height:10px"></div>
          <div class="grid2" style="grid-template-columns:1fr 1fr;gap:10px">
            <div class="field">
              <label>שם נציג</label>
              <input id="newUserName" placeholder="לדוגמה: סתיו"/>
            </div>
            <div class="field">
              <label>קוד (4-8 ספרות)</label>
              <input id="newUserCode" placeholder="לדוגמה: 2222" inputmode="numeric"/>
            </div>
          </div>
          <div style="height:10px"></div>
          <button class="btn btnPrimary" id="addUserBtn">+ צור יוזר</button>
          <div style="height:12px"></div>

          <div class="tableWrap">
            <table>
              <thead><tr><th>שם</th><th>יוזר</th><th>תפקיד</th><th>קוד</th><th></th></tr></thead>
              <tbody>
                ${USERS.map((u,idx)=>`
                  <tr>
                    <td><b>${escapeHtml(u.displayName||'')}</b></td>
                    <td class="mono">${escapeHtml(u.username||'')}</td>
                    <td>${u.role==='admin'?'מנהל':'נציג'}</td>
                    <td class="mono">${escapeHtml(u.password||'')}</td>
                    <td style="text-align:left;white-space:nowrap">
                      ${u.role==='admin' ? '' : `
                        <button class="btn btnSoft" data-edit-user="${idx}">ערוך קוד</button>
                        <button class="btn btnSoft" data-del-user="${idx}">מחק</button>
                      `}
                    </td>
                  </tr>
                `).join('')}
          ...
          <div class="miniNote">טיפ: מומלץ להשתמש בקוד 4 ספרות לכל נציג.</div>
        </div>
        ` : ''}
    ...
    if(isAdmin_()){
      ...
      if(addBtn){
        addBtn.addEventListener('click', () => {
          const nameRaw = String($('#newUserName')?.value || '').trim();
          const codeRaw = String($('#newUserCode')?.value || '').trim();
          const name = nameRaw.replace(/\s+/g,' ').trim();
          if(!name){
            toast('חסר שם', 'נא להזין שם לנציג');
            return;
          }
          if(!isValidCode_(codeRaw)){
            toast('קוד לא תקין', 'נא להזין קוד מספרי 4–8 ספרות');
            return;
          }
          const username = ensureUniqueUsername_(name);
          // username is what you type at login (can be Hebrew)
          USERS.push({ username, password: codeRaw, role:'agent', displayName: name });
          saveUsers_();
          syncTeamAgentsFromUsers_();
          toast('נוצר יוזר', name + ' נוסף למערכת');
          render();
        });
      }

      // Edit code / delete
      viewEl.querySelectorAll('[data-edit-user]').forEach(btn => {
        btn.addEventListener('click', () => {
          const i = Number(btn.getAttribute('data-edit-user'));
          const u = USERS[i];
          if(!u || u.role==='admin') return;
          const next = prompt('קוד חדש (4–8 ספרות):', String(u.password||''));
          if(next === null) return;
          if(!isValidCode_(next)){
            toast('קוד לא תקין', 'נא להזין קוד מספרי 4–8 ספרות');
            return;
          }
          u.password = String(next).trim();
          saveUsers_();
      ...
      viewEl.querySelectorAll('[data-del-user]').forEach(btn => {
        btn.addEventListener('click', () => {
          ...
          const u = USERS[i];
          if(!u || u.role==='admin') return;
          if(!confirm('למחוק את היוזר של ' + (u.displayName||u.username) + '?')) return;
          USERS.splice(i,1);
          saveUsers_();
          syncTeamAgentsFromUsers_();
          toast('נמחק', 'היוזר הוסר');
          render();
        });
      });
    }
