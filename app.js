...
// Keep wizard title in sync
  ...
  // ---------- Har Habituach Import (Existing Policies) ----------
  function normalizeCompanyShort_(name){
    ...
    // fallback: trim corporate suffixes
    return low
      .replace(/חברה לביטוח בע\"מ/g,'')
      .replace(/בע\"מ/g,'')
      .replace(/\s{2,}/g,' ')
      .trim();
  }

  function detectHarHeaderRow_(rows){
    // rows: array of arrays
    for(let i=0;i<rows.length;i++){
      const r = rows[i] || [];
      const joined = r.map(x => String(x||'').trim()).join('|');
      if(joined.includes('חברה') && joined.includes('מספר פוליסה') && (joined.includes('פרמיה') || joined.includes('פרמיה בש\"ח'))){
        return i;
      }
    }
    return -1;
  ...
  function harRowsToOldPolicies_(rows, insuredId){
    const headerIdx = detectHarHeaderRow_(rows);
    if(headerIdx < 0) return { ok:false, error:'NO_HEADER' };
    const header = (rows[headerIdx]||[]).map(x => String(x||'').trim());
    const col = (name) => header.findIndex(h => h === name);
    const cMain = col('ענף ראשי');
    const cSub = col('ענף (משני)');
    const cCompany = col('חברה');
    const cPeriod = col('תקופת ביטוח');
    const cPremium = col('פרמיה בש"ח');
    const cPremType = col('סוג פרמיה');
    const cPolicy = col('מספר פוליסה');
    const cProgram = col('סיווג תכנית');

    if(cCompany < 0 || cPremium < 0 || cPolicy < 0) return { ok:false, error:'MISSING_COLS' };

    const out = [];
    for(let i=headerIdx+1;i<rows.length;i++){
      const r = rows[i] || [];
      const main = cMain>=0 ? String(r[cMain]||'').trim() : '';
      const sub = cSub>=0 ? String(r[cSub]||'').trim() : '';
      const companyRaw = String(r[cCompany]||'').trim();
      const policyNoRaw = (r[cPolicy]===0 ? '0' : (r[cPolicy]||'')); // allow 0
      const policyNo = String(policyNoRaw||'').trim();
      const premiumRaw = r[cPremium];
      const premType = cPremType>=0 ? String(r[cPremType]||'').trim() : '';
      const period = cPeriod>=0 ? String(r[cPeriod]||'').trim() : '';
      const program = cProgram>=0 ? String(r[cProgram]||'').trim() : '';

      // Skip section/header rows like "תחום - ..."
      const isSection = String(main||'').startsWith('תחום -') || String(companyRaw||'').startsWith('תחום -');
      if(isSection) continue;

      if(!companyRaw || !policyNo) continue;

      let premium = 0;
      if(typeof premiumRaw === 'number') premium = premiumRaw;
      else premium = parseFloat(String(premiumRaw||'').replace(/[^\d.]/g,'') || '0') || 0;

      // Normalize monthly
      const premTypeLow = premType.toLowerCase();
      if(premTypeLow.includes('שנתי') || premType.includes('שנתית')) premium = premium / 12;

      const product = (sub || main || '').trim();

      out.push({
        insuredId: insuredId || 'main',
        company: normalizeCompanyShort_(companyRaw),
        companyRaw,
        product,
        policyNo,
        premium: Math.round((premium||0)*100)/100,
        decision: '',
        source: 'har',
        meta: { main, sub, period, premType, program }
      });
    }
    return { ok:true, items: out };
  }

  async function readHarFileToRows_(file){
    if(!file) return { ok:false, error:'NO_FILE' };
    if(typeof XLSX === 'undefined') return { ok:false, error:'NO_XLSX_LIB' };
    ...
    wizardMain.innerHTML = `
      ...
      <div class="importBox">
        <div class="importHead">
          <div>שליפת פוליסות קיימות מהר הביטוח</div>
          <span class="pill" style="border-color:rgba(214,178,94,.28);background:rgba(214,178,94,.12)">Excel/CSV</span>
        </div>
        <div class="importSub">העלה קובץ "הר הביטוח" (XLSX/CSV) כדי ליצור אוטומטית שורות פוליסות קיימות עבור <b>${escapeHtml(insuredLabel)}</b>.</div>
        <div class="importRow">
          <input class="input" id="harFile" type="file" accept=".xlsx,.xls,.csv"/>
          <button class="btn btnSoft" id="harProcessBtn">שלח לעיבוד נתונים</button>
        </div>
        <div class="importPreview" id="harPreview"></div>
      </div>
<div class="grid2">
    ...
    // Har Habituach import (Excel/CSV -> existing policies)
    const harFileEl = $('#harFile');
    const harBtn = $('#harProcessBtn');
    const harPrev = $('#harPreview');
    if(harPrev){ harPrev.classList.remove('show'); harPrev.textContent = ''; }

    if(harBtn){
      harBtn.addEventListener('click', async () => {
        try{
          const file = harFileEl && harFileEl.files ? harFileEl.files[0] : null;
          if(!file){ toast('חסר', 'בחר קובץ הר הביטוח (Excel/CSV)'); return; }

          harBtn.disabled = true;
          harBtn.textContent = 'מעבד...';

          const rr = await readHarFileToRows_(file);
          if(!rr.ok){
            toast('שגיאה', rr.error === 'NO_XLSX_LIB' ? 'חסר רכיב XLSX (בדוק חיבור אינטרנט)' : 'לא ניתן לקרוא את הקובץ');
            return;
          }

          const mapped = harRowsToOldPolicies_(rr.rows, insuredId);
          if(!mapped.ok){
            const msg = (mapped.error === 'NO_HEADER')
              ? 'לא זוהתה כותרת טבלה בקובץ. ודא שזה קובץ "הר הביטוח" תקין.'
              : 'לא נמצאו עמודות חובה (חברה / מספר פוליסה / פרמיה).';
            toast('שגיאה', msg);
            return;
          }

          const existingKeys = new Set((p.oldPolicies||[]).map(x => `${x.insuredId||'main'}|${x.policyNo||''}|${x.company||''}`));
          let added = 0;
          mapped.items.forEach(it => {
            const key = `${it.insuredId||'main'}|${it.policyNo||''}|${it.company||''}`;
            if(existingKeys.has(key)) return;
            existingKeys.add(key);
            p.oldPolicies.push(it);
            added++;
          });

          if(harPrev){
            const total = mapped.items.length;
            harPrev.innerHTML = `נמצאו <b>${total}</b> שורות בקובץ. נוספו למערכת <b>${added}</b> פוליסות (ללא כפילויות).`;
            harPrev.classList.add('show');
          }

          toast('הושלם', `נוספו ${added} פוליסות קיימות מהר הביטוח`);
          renderOldPoliciesStep();
          updateSums();
          markDraft();
        }catch(e){
          console.error('har import', e);
          toast('שגיאה', 'אירעה שגיאה בעיבוד הקובץ');
        }finally{
          if(harBtn){
            harBtn.disabled = false;
            harBtn.textContent = 'שלח לעיבוד נתונים';
          }
        }
      });
    }
...
function debounce(fn, wait){
  ...
  // ---------- HAR IMPORT (Event-delegation, robust file reading) ----------
  // This ensures the "Send to processing" button works even after re-renders.
  ...
  function parseCsvToAoa_(text){
    // Simple CSV parser with quotes support
    const rows = [];
    const lines = String(text||'').replace(/\r\n/g,'\n').replace(/\r/g,'\n').split('\n');
    const delim = detectCsvDelimiter_(lines.find(l => l && l.trim()) || '');
  ...
  async function readHarFileToAoa_(file){
    const ext = (file.name || '').toLowerCase().split('.').pop();
    const buf = await file.arrayBuffer();
    if(ext === 'csv'){
      // Always parse CSV without XLSX dependency
      const text = new TextDecoder('utf-8').decode(buf);
      return { ok:true, aoa: parseCsvToAoa_(text), meta:{ kind:'csv', sheetName:'CSV' } };
    }
    // XLSX path
    if(typeof XLSX === 'undefined'){
      return { ok:false, msg:'ספריית XLSX לא נטענה. נסה CSV או ודא שיש אינטרנט.' };
    }
    const wb = XLSX.read(buf, { type:'array' });

    // Prefer a sheet that includes "תיק" / "ביטוח"
    const names = wb.SheetNames || [];
    let sheetName = names[0] || '';
    const preferred = names.find(n => /תיק|ביטוח/i.test(String(n)));
    if(preferred) sheetName = preferred;

    const ws = wb.Sheets[sheetName];
    const aoa = XLSX.utils.sheet_to_json(ws, { header:1, raw:true, defval:'' });
    return { ok:true, aoa, meta:{ kind:'xlsx', sheetName } };
  }

  async function handleHarImportClick_(){
    const p = (typeof wiz !== 'undefined' && wiz && wiz.proposal) ? wiz.proposal : null;
    if(!p){
      toast('שגיאה', 'לא נמצאה הצעה פעילה');
      return;
    }
    const insuredId = p.activeInsuredId || 'main';
    p.oldPolicies = Array.isArray(p.oldPolicies) ? p.oldPolicies : [];

    const harLog = document.getElementById('harLog');
    const setHarLog = (html) => { if(harLog) harLog.innerHTML = html || ''; };

    const harFile = document.getElementById('harFile');
    const harMode = document.getElementById('harMode');
    const file = (harFile && harFile.files && harFile.files[0]) ? harFile.files[0] : null;

    // Visual confirmation that the click handler is alive
    setHarLog('<div class="harHint">⏳ מעבד קובץ...</div>');

    if(!file){
      toast('שגיאה', 'בחר קובץ הר הביטוח');
      setHarLog('<div class="harErr">חסר קובץ. בחר קובץ Excel/CSV ואז לחץ שוב.</div>');
      return;
    }

    const mode = harMode ? harMode.value : 'auto';
    try{
      const read = await readHarFileToAoa_(file);
      if(!read.ok){
        toast('שגיאה', read.msg || 'קריאת הקובץ נכשלה');
        setHarLog('<div class="harErr">'+ escapeHtml(read.msg || 'קריאת הקובץ נכשלה') +'</div>');
        return;
      }

      const aoa = read.aoa || [];
      const parsed = parseHarAoaToPolicies_(aoa, { mode });

      if(!parsed.ok){
        toast('שגיאה', parsed.msg || 'היבוא נכשל');
        setHarLog('<div class="harErr">'+ escapeHtml(parsed.msg || 'לא הצלחתי לקרוא את הטבלה בקובץ') +'</div>'
          + '<div class="harHint">נקרא: '+ escapeHtml(file.name) +' • גיליון: '+ escapeHtml(read.meta.sheetName||'') +' • שורות: '+ aoa.length +'</div>');
        return;
      }

      const list = parsed.policies || [];
      if(!list.length){
        toast('שימו לב', 'לא נמצאו פוליסות לייבוא');
        setHarLog('<div class="harWarn">הקובץ נקרא בהצלחה, אבל לא נמצאו שורות פוליסה לייבוא.</div>'
          + '<div class="harHint">נקרא: '+ escapeHtml(file.name) +' • גיליון: '+ escapeHtml(read.meta.sheetName||'') +' • שורות: '+ aoa.length +'</div>');
        return;
      }

      // De-dup + push
      const before = p.oldPolicies.length;
      const existingKeys = new Set(p.oldPolicies.map(x => `${(x.insuredId||'main')}|${x.company||''}|${x.policyNo||''}`));
      const imported = [];
      list.forEach(rec => {
        const key = `${insuredId}|${rec.company||''}|${rec.policyNo||''}`;
        if(existingKeys.has(key)) return;
        const row = {
          insuredId,
          company: rec.company || '',
          ...
          premium: Number(rec.premium)||0,
          decision: '', // נציג יבחר
          __harImported: true,
          __harBatch: String(Date.now())
        };
        p.oldPolicies.push(row);
        imported.push(row);
        existingKeys.add(key);
      });

      p.__lastHarBatch = imported.length ? imported[0].__harBatch : null;
      const added = p.oldPolicies.length - before;

      toast('הצלחה', 'יובאו ' + added + ' פוליסות');
      setHarLog('<div class="harOk">✅ נקרא הקובץ והוספו '+ added +' שורות.</div>'
        + '<div class="harHint">נקרא: '+ escapeHtml(file.name) +' • גיליון: '+ escapeHtml(read.meta.sheetName||'') +' • שורות: '+ aoa.length +' • פוליסות: '+ list.length +'</div>');

      // Re-render current step if available
      if(typeof renderOldPoliciesStep === 'function'){
        renderOldPoliciesStep();
      }else{
        render();
      }
      if(typeof updateSums === 'function') updateSums();
      if(typeof markDraft === 'function') markDraft();

    }catch(err){
      console.error('HAR import failed', err);
      toast('שגיאה', 'שגיאה בעיבוד הקובץ');
      setHarLog('<div class="harErr">שגיאה בעיבוד הקובץ. פתח קונסול (F12) לראות פרטים.</div>');
    }
  }

  document.addEventListener('click', (e) => {
    const t = e.target;
    const btn = t && t.closest ? t.closest('#harParseBtn') : null;
    if(btn){
      e.preventDefault();
      handleHarImportClick_();
      return;
    }
    const clearBtn = t && t.closest ? t.closest('#harClearBtn') : null;
    if(clearBtn){
      // let existing per-render handler run; if none, we can implement later
