...
els.loadSampleBtn.addEventListener('click', async () => {
  ...
  setStatus('דוגמת נתונים נטענה.', 'success');
});
...
els.jsonFile.addEventListener('change', async (event) => {
  ...
  setStatus(`נטען הקובץ ${file.name}`, 'success');
});
...
async function generatePdf() {
  try {
    setStatus('מכין את ה־PDF...', '');
    const data = safeJsonParse(els.jsonInput.value.trim());

    const existingPdfBytes = await fetch('./templates/GetPDF.pdf').then((res) => res.arrayBuffer());
    ...
    fillPage1(pages[0], data, helvetica, helveticaBold);
    fillHealthPages(pages, data, helvetica, helveticaBold);
    addEditableFields(pdfDoc, form, pages, helvetica);

    form.updateFieldAppearances(helvetica);

    const output = await pdfDoc.save();
    downloadBlob(new Blob([output], { type: 'application/pdf' }), buildOutputName(data));
    setStatus('ה־PDF נוצר בהצלחה.', 'success');
  } catch (error) {
    console.error(error);
...
function fillPage1(page, data, font, boldFont) {
  const primary = data.primaryInsured || {};
  const secondary = data.secondaryInsured || {};
  const agent = data.agent || {};

  drawText(page, splitName(primary.fullName).firstName, 250, 710, font, 9);
  drawText(page, splitName(primary.fullName).lastName, 182, 710, font, 9);
  drawText(page, primary.idNumber, 132, 710, font, 9);
  ...
  drawText(page, primary.birthDate, 220, 688, font, 9);
  drawText(page, primary.healthFund, 164, 688, font, 9);
  drawSupplementaryMark(page, primary.hasSupplementaryInsurance, 118, 688, 96, 688);
  drawText(page, primary.occupation, 267, 666, font, 9);
  drawText(page, primary.mobile, 183, 666, font, 9);
  drawText(page, primary.email, 84, 666, font, 8);
  drawDeliveryMark(page, primary.deliveryPreference, { email:[285,646], home:[267,646] });
  drawText(page, primary.address?.street, 236, 626, font, 8);
  drawText(page, primary.address?.houseNumber, 185, 626, font, 8);
  drawText(page, primary.address?.city, 121, 626, font, 8);
  drawText(page, primary.address?.zipCode, 69, 626, font, 8);

  drawText(page, splitName(secondary.fullName).firstName, 250, 604, font, 9);
  drawText(page, splitName(secondary.fullName).lastName, 182, 604, font, 9);
  drawText(page, secondary.idNumber, 132, 604, font, 9);
  ...
  drawText(page, secondary.birthDate, 220, 582, font, 9);
  drawText(page, secondary.healthFund, 164, 582, font, 9);
  drawSupplementaryMark(page, secondary.hasSupplementaryInsurance, 118, 582, 96, 582);
  drawText(page, secondary.occupation, 267, 560, font, 9);
  drawText(page, secondary.mobile, 183, 560, font, 9);
  drawText(page, secondary.email, 84, 560, font, 8);
  drawDeliveryMark(page, secondary.deliveryPreference, { email:[285,540], home:[267,540] });
  drawText(page, secondary.address?.street, 236, 520, font, 8);
  drawText(page, secondary.address?.houseNumber, 185, 520, font, 8);
  drawText(page, secondary.address?.city, 121, 520, font, 8);
  drawText(page, secondary.address?.zipCode, 69, 520, font, 8);

  drawText(page, agent.name, 499, 710, font, 9);
  drawText(page, agent.number, 499, 690, font, 9);
  drawText(page, agent.supervisorName, 499, 670, font, 9);
  drawText(page, agent.teamNumber, 499, 650, font, 9);
  drawText(page, agent.salesManager, 499, 630, font, 9);
  drawText(page, agent.proposalNumber, 499, 610, font, 9);
  drawText(page, data.requestedStartDate, 335, 371, boldFont, 9);
}

function fillHealthPages(pages, data, font) {
  ...
  if (pages[6]) {
    drawText(pages[6], primary.fullName, 317, 701, font, 9);
    drawText(pages[6], primary.heightCm, 147, 701, font, 9);
    drawText(pages[6], primary.weightKg, 96, 701, font, 9);
  ...
  if (pages[8]) {
    drawText(pages[8], secondary.fullName, 317, 701, font, 9);
    drawText(pages[8], secondary.heightCm, 147, 701, font, 9);
    drawText(pages[8], secondary.weightKg, 96, 701, font, 9);
...
function addEditableFields(pdfDoc, form, pages, font) {
  if (pages[2]) {
    const page = pages[2];
    addTextField(form, page, 'beneficiary_1_name', 354, 520, 170, 18, font, 'שם מוטב');
    addTextField(form, page, 'beneficiary_1_id', 254, 520, 85, 18, font, 'ת"ז');
    addTextField(form, page, 'beneficiary_1_relationship', 162, 520, 75, 18, font, 'קרבה');
    addTextField(form, page, 'beneficiary_1_share', 72, 520, 70, 18, font, 'אחוז');
  }

  if (pages[7]) {
    addTextField(form, pages[7], 'medical_details_primary', 60, 120, 475, 96, font, 'פירוט ממצאים חיוביים - מבוטח ראשי');
  }
  if (pages[9]) {
    addTextField(form, pages[9], 'medical_details_secondary', 60, 120, 475, 96, font, 'פירוט ממצאים חיוביים - מבוטח משני');
  }

  if (pages[0]) {
    addTextField(form, pages[0], 'page1_candidate1_signature', 344, 69, 95, 16, font, 'חתימת מועמד ראשון');
    addTextField(form, pages[0], 'page1_candidate2_signature', 204, 69, 95, 16, font, 'חתימת מועמד שני');
  }
  if (pages[2]) {
    addTextField(form, pages[2], 'page3_agent_signature', 75, 98, 105, 18, font, 'חתימת סוכן');
    addTextField(form, pages[2], 'page3_customer_signature', 224, 98, 115, 18, font, 'חתימת מועמד');
  }
}
