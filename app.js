...
// Keep wizard title in sync
  ...
  function renderOldPoliciesStep(){
    ...
    wizardMain.innerHTML = `
      ...
      <div class="grid2">
        ${companyLogoField('חברה', 'old_company', ['הראל','כלל','מנורה','הפניקס','מגדל','הכשרה'])}
    ...
    initCompanyLogoPicker('old_company', 'הראל');
...
function renderNewPoliciesStep(){
    ...
    wizardMain.innerHTML = `
      ...
      <div class="grid3">
        ${companyLogoField('חברה', 'new_company', ['הראל','כלל','מנורה','הפניקס','מגדל','הכשרה'])}
    ...
    initCompanyLogoPicker('new_company', 'הראל');
...
`;
  ...
  function companyLogoPath(name){
    const key = (name||'').trim();
    return COMPANY_LOGOS[key] || '';
  }

  function companyLogoImg(name){
    const src = companyLogoPath(name);
    if(!src) return '';
    const title = escapeAttr(name || '');
    return `<img class="companyLogo" src="${src}" alt="${title}" title="${title}" loading="lazy" />`;
  }

  function companyCell(name){
    // UI requirement: logo only (text remains in data for exports/reports)
    const img = companyLogoImg(name);
    return img ? `<div class="companyCell">${img}</div>` : escapeHtml(name||'');
  }

  function companyLogoField(label, inputId, companies){
    // Native <select> can't reliably show images -> custom logo picker
    const btns = (companies||[]).map(c => {
      const src = companyLogoPath(c);
      const t = escapeAttr(c);
      const activeAttr = ''; // set by initCompanyLogoPicker
      return `
        <button class="logoBtn" type="button" data-company="${t}" ${activeAttr}>
          ${src ? `<img src="${src}" alt="${t}" loading="lazy" />` : `<span class="logoFallback">${escapeHtml(c)}</span>`}
        </button>
      `;
    }).join('');
    ...
    return `
      <div class="field">
        ...
        <div class="logoPick" id="${inputId}_pick">${btns}</div>
        <input type="hidden" id="${inputId}" value="" />
      </div>
    `;
  }

  function initCompanyLogoPicker(inputId, initialValue){
    const wrap = $('#' + inputId + '_pick');
    const hidden = $('#' + inputId);
    if(!wrap || !hidden) return;

    const set = (val) => {
      hidden.value = val || '';
      wrap.querySelectorAll('.logoBtn').forEach(b => {
        const isOn = (b.getAttribute('data-company')||'') === (val||'');
    ...
    wrap.addEventListener('click', (e) => {
      ...
      if(!btn) return;
      const val = btn.getAttribute('data-company') || '';
      set(val);
    });

    set(initialValue || hidden.value || '');
  }
