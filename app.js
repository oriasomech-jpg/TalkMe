...
email: c?.email || '',
  ...
  function getLatestProposalFromCustomer(c){
    if(!c) return null;
    if(Array.isArray(c.proposalsHistory) && c.proposalsHistory.length){
      return c.proposalsHistory[c.proposalsHistory.length - 1];
    }
    if(c.lastProposal && typeof c.lastProposal === 'object') return c.lastProposal;
    // fallback: some builds store policies directly on customer
    if(Array.isArray(c.oldPolicies) || Array.isArray(c.newPolicies)){
      return { oldPolicies: c.oldPolicies || [], newPolicies: c.newPolicies || [], status: c.status || '' };
    }
    return null;
  }
  ...
  function normalizeCustomerForUI(c){
    ...
    const copy = JSON.parse(JSON.stringify(c)); // safe deep clone
    // map common aliases
    copy.fullName = copy.fullName || copy.name || copy.customerName || '';
    copy.tz = copy.tz || copy.idNumber || copy.teudatZehut || '';
  ...
  function renderStage2(){
    const c = currentCustomerRaw || currentProfile || {};
    const proposal = getLatestProposalFromCustomer(c) || null;
    const oldPolicies = proposal ? (proposal.oldPolicies || []) : [];
    const newPolicies = proposal ? (proposal.newPolicies || []) : [];

    return `
      <div class="stageHeader">
        <div class="stageTitle">מסך התאמה</div>
        <div class="stageSub">שלב 2</div>
      </div>

      <div class="scriptBox">
        <div class="scriptText">
          חשוב לי לעדכן אותך כי בשוק ישנן 8 בבריאות 9 בחיים חברות המשווקות את המוצר.<br>
          חברות ביטוח העיקריות שאנו עובדים איתם בתחום ביטוחי הבריאות כלל ואיילון ובתחום ביטוחי החיים כלל ומגדל..<br>
          אז לאחר שקיבלנו את פנייתך
        </div>

        <div class="redQuestion">
          האם אתה מאשר שאתה זה שאישרת לנו להיכנס עבורך לממשק הר הביטוח ולבצע עבורך בדיקה על מנת להתאים עבורך ביטוח העונה על צרכיך?
        </div>

        <div class="ynRow">
          <label class="yn"><input type="radio" name="harConsent" value="yes"> כן</label>
          <label class="yn"><input type="radio" name="harConsent" value="no"> לא</label>
        </div>

        <div class="scriptText" id="harAfterText" style="display:none;margin-top:10px">
          לאחר ביצוע בדיקה באתר הר הביטוח שתקף ל 5 ימי עבודה. להלן הביטוחים הקיימים לך כיום :
        </div>
      </div>

      <div id="policiesWrap" style="display:none">
        <div class="sectionTitle">ביטוחים קיימים</div>
        ${renderPoliciesTable(oldPolicies, true)}

        <div class="hr"></div>

        <div class="scriptBox" style="margin-top:10px">
          <div class="scriptText">
            בהתאם לביטוחים הקיימים לך כיום הפוליסה שאנו מציעים לך לרכוש היא פוליסת...
          </div>
        </div>

        <div class="sectionTitle" style="margin-top:10px">פוליסות חדשות בהצעה</div>
        ${renderPoliciesTable(newPolicies, false)}
      </div>

      <div class="footerActions">
        <button class="btn btnSoft" id="backToStage1">חזרה לשלב 1</button>
      </div>
    `;
  }

  function renderPoliciesTable(items, isExisting){
    const arr = Array.isArray(items) ? items : [];
    if(!arr.length){
      return `<div class="emptyBox">לא נמצאו ${isExisting ? 'פוליסות קיימות' : 'פוליסות חדשות'} להצגה (כנראה לא נשמרה הצעה ללקוח).</div>`;
    }
    const rows = arr.map((x) => {
      const company = escapeHtml(x.company || x.insCompany || x.insuranceCompany || '—');
      const product = escapeHtml(x.product || x.plan || x.coverage || x.type || '—');
      const prem = escapeHtml(x.premium ?? x.price ?? x.monthly ?? '—');
      const sum = escapeHtml(x.sumInsured ?? x.coverAmount ?? x.compensation ?? '—');
      const status = escapeHtml(x.status || x.action || x.policyStatus || '—');
      const discount = escapeHtml(x.discount ?? x.discountPct ?? '—');
      const priceBefore = escapeHtml(x.priceBeforeDiscount ?? x.beforeDiscount ?? '—');
      const priceAfter = escapeHtml(x.priceAfterDiscount ?? x.afterDiscount ?? x.finalPrice ?? '—');
      const lien = escapeHtml(x.lien ?? x.shibud ?? x.pledge ?? '');
      return `
        <tr>
          <td>${company}</td>
          <td>${product}</td>
          <td>${prem}</td>
          <td>${sum}</td>
          ${isExisting ? `<td>${status}</td>` : `<td>${priceBefore}</td><td>${discount}</td><td>${priceAfter}</td><td>${lien||'—'}</td>`}
        </tr>
      `;
    }).join('');
    return `
        ...
        <table class="tbl">
          <thead>
            <tr>
              ...
              <th>מוצר</th>
              <th>פרמיה</th>
              <th>${isExisting ? 'סכום ביטוח/פיצוי' : 'סכום ביטוח/פיצוי'}</th>
              ${isExisting ? `<th>סטטוס</th>` : `<th>מחיר לפני הנחה</th><th>הנחה</th><th>מחיר אחרי הנחה</th><th>שיעבוד</th>`}
            </tr>
          </thead>
          <tbody>${rows}</tbody>
        </table>
  ...
  function bindStage2Events(){
    const back = document.getElementById('backToStage1');
    if(back) back.addEventListener('click', () => { currentStage = 1; renderDetails(currentCustomerRaw || currentProfile); });

    document.querySelectorAll('input[name="harConsent"]').forEach(r => {
      r.addEventListener('change', () => {
        const v = document.querySelector('input[name="harConsent"]:checked')?.value || '';
        const after = document.getElementById('harAfterText');
        const wrap = document.getElementById('policiesWrap');
        if(v === 'yes'){
...
function renderStage1(){
    const raw = currentProfile || normalizeCustomerForUI({});
    const p = raw.__editable || makeEditableProfileFromCustomer(raw);
      ...
      <div class="stage">
        ...
        <div class="formGrid">
          ...
          <div class="field">
            ...
            <input class="input inputSm" id="f_phone" value="${esc(p.phone)}" placeholder="טלפון" />
          </div>
          <div class="field">
  ...
  function renderDetails(c){
    ...
    if(c && !c.__profile){
      currentProfile = normalizeCustomerForUI(c);
      // Keep editable subset for stage1 fields, but do NOT drop other fields (policies/history)
      currentProfile.__editable = makeEditableProfileFromCustomer(currentProfile);
    ...
    detailsBody.innerHTML = (currentStage === 2) ? renderStage2() : renderStage1();

    // bind after DOM updates
    setTimeout(() => { if(currentStage===2) bindStage2Events(); else bindStage1Events(); }, 0);
  }

  function selectCustomer(id){
    activeId = id;
    currentStage = 1;
    const c0 = customers.find(x => x.id === id) || null;
    currentCustomerRaw = normalizeCustomerForUI(c0);
    renderDetails(currentCustomerRaw);
