(() => {
  ...
  function makeEditableProfileFromCustomer(c){
    // IMPORTANT:
    // Keep the full payload coming from the main CRM (policies, statuses, consents, etc.).
    // Earlier versions rebuilt a "clean" object and accidentally DROPPED arrays like existingPolicies/newPolicies.
    const base = (c && typeof c === 'object') ? { ...c } : {};

    // Normalize core fields (without removing any other keys on `base`).
    base.fullName = base.fullName || '';
    base.tz = base.tz || '';
    base.phone = base.phone || '';
    base.email = base.email || '';
    base.dob = base.dob || '';
    base.maritalStatus = base.maritalStatus || '';
    base.occupation = base.occupation || '';
    base.hmo = base.hmo || '';
    base.shaban = base.shaban || '';
    base.address = base.address || '';
    base.deliveryPref = base.deliveryPref || '';
    base.smoking2y = base.smoking2y || '';
    base.preparedBy = base.preparedBy || '';
    base.companies = Array.isArray(base.companies) ? base.companies.slice() : [];
  ...
  function renderStage1(){
    ...
    const existingPolicies = pickPolicies(p, ['existingPolicies','policiesExisting','existing','harHabituachPolicies']);
    const newPolicies = pickPolicies(p, ['newPolicies','policiesNew','proposedPolicies','offerPolicies']);

    const hbConsent = p.hbConsent || '';
    const mainConsideration = p.mainConsideration || '';

    const renderExistingRows = () => {
      if(!existingPolicies.length){
        return `<div class="polEmpty">אין פוליסות קיימות להצגה (טרם נשאב מידע).</div>`;
      }
      return `
        <div class="polTableWrap">
          <table class="polTable" aria-label="פוליסות קיימות">
            <thead>
              <tr>
                <th>חברה</th>
                <th>מוצר / תחום</th>
                <th>פרמיה</th>
                <th>סכום ביטוח / פיצוי</th>
                <th>סטטוס</th>
              </tr>
            </thead>
            <tbody>
              ${existingPolicies.map(pol => {
                const company = pol.company || pol.insurer || pol.companyName || '—';
                const product = pol.product || pol.plan || pol.type || pol.category || '—';
                const premium = pol.premium ?? pol.price ?? pol.monthly ?? pol.payment;
                const sum = pol.coverageAmount ?? pol.coverage ?? pol.sumInsured ?? pol.payout ?? '';
                const status = pol.status ?? pol.action ?? pol.agentStatus ?? '';
                return `
                  <tr>
                    <td>${esc(company)}</td>
                    <td>${esc(product)}</td>
                    <td>${esc(fmtILS(premium))}</td>
                    <td>${esc(sum || '—')}</td>
                    <td>${esc(status || '—')}</td>
                  </tr>
                `;
    ...
    const renderNewRows = () => {
      if(!newPolicies.length){
        return `<div class="polEmpty">אין פוליסות חדשות להצגה (לא מולאו פוליסות בהצעה).</div>`;
      }
      return `
        <div class="polTableWrap">
          <table class="polTable" aria-label="פוליסות חדשות">
            <thead>
              <tr>
                <th>חברה</th>
                <th>מוצר ביטוח</th>
            ...
            <tbody>
              ${newPolicies.map(pol => {
                const company = pol.company || pol.insurer || pol.companyName || '—';
                const product = pol.product || pol.plan || pol.type || pol.category || '—';
                const before = pol.priceBeforeDiscount ?? pol.premiumBefore ?? pol.beforeDiscount ?? pol.priceBefore ?? pol.premium;
                const discount = pol.discount ?? pol.discountPercent ?? pol.discountValue ?? '';
                const after = pol.priceAfterDiscount ?? pol.premiumAfter ?? pol.afterDiscount ?? pol.priceAfter ?? '';
                const pledge = (pol.pledge ?? pol.lien ?? pol.shiabud ?? pol.charge);
                const pledgeTxt = (pledge === true) ? 'כן' : (pledge === false) ? 'לא' : (pledge ? String(pledge) : '—');
                return `
                  <tr>
                    <td>${esc(company)}</td>
                    <td>${esc(product)}</td>
                    <td>${esc(fmtILS(before))}</td>
                    <td>${esc(discount || '—')}</td>
                    <td>${esc(after ? fmtILS(after) : '—')}</td>
                    <td>${esc(pledgeTxt)}</td>
                  </tr>
                `;
              }).join('')}
...
<div class="field">
        ...
        <div class="modalOverlay" id="stage2Modal" style="display:none" role="dialog" aria-modal="true" aria-label="שלב 2">
          <div class="modalCard">
            ...
            <div class="modalBody">
              <div class="stage2Script">
                חשוב לי לעדכן אותך כי בשוק ישנן 8 בבריאות 9 בחיים חברות המשווקות את המוצר.
                חברות ביטוח העיקריות שאנו עובדים איתם בתחום ביטוחי הבריאות כלל ואיילון ובתחום ביטוחי החיים כלל ומגדל..
                אז לאחר שקיבלנו את פנייתך.
              </div>

              <div class="stage2QuestionRed">האם אתה מאשר שאתה זה שאישרת לנו להיכנס עבורך לממשק הר הביטוח ולבצע עבורך בדיקה על מנת להתאים עבורך ביטוח העונה על צרכיך?</div>

              <div class="ynRow" style="margin-top:10px">
                <button class="ynBtn" type="button" id="hbYes">כן</button>
                <button class="ynBtn" type="button" id="hbNo">לא</button>
                <span class="meta" id="hbMeta" style="margin-right:auto">${hbConsent ? ('נבחר: ' + hbConsent) : 'לא סומן'}</span>
              </div>

              <div class="stage2After" id="hbAfter" style="display:${hbConsent === 'כן' ? '' : 'none'}">
                לאחר ביצוע בדיקה באתר הר הביטוח שתקף ל 5 ימי עבודה. להלן הביטוחים הקיימים לך כיום :
              </div>

              <div id="existingPoliciesWrap" style="display:${hbConsent === 'כן' ? '' : 'none'}">
                ${renderExistingRows()}
              </div>

              <div class="stage2After" style="margin-top:14px;">
                בהתאם לביטוחים הקיימים לך כיום הפוליסה שאנו מציעים לך לרכוש היא פוליסת...
              </div>

              <div id="newPoliciesWrap">
                ${renderNewRows()}
              </div>
            </div>
              <div class="stage2After" style="margin-top:14px;">
                השיקולים העיקריים במתן ההמלצה הינם:
              </div>

              <div class="choiceGrid" id="mainConsiderationGrid" style="margin-top:10px">
                <label class="choiceItem">
                  <input type="radio" name="mainConsideration" value="הוזלה" ${mainConsideration==='הוזלה'?'checked':''}>
                  <span>הוזלה</span>
                </label>
                <label class="choiceItem">
                  <input type="radio" name="mainConsideration" value="הרחבה" ${mainConsideration==='הרחבה'?'checked':''}>
                  <span>הרחבה</span>
                </label>
                <label class="choiceItem">
                  <input type="radio" name="mainConsideration" value="רכישת ביטוח חדש" ${mainConsideration==='רכישת ביטוח חדש'?'checked':''}>
                  <span>רכישת ביטוח חדש</span>
                </label>
                <span class="meta" id="mainConsiderationMeta" style="grid-column:1/-1; justify-self:start">${mainConsideration ? ('נבחר: ' + mainConsideration) : 'לא סומן'}</span>
              </div>

              <div class="stage2Script" style="margin-top:12px">
                הפרמיה צמודה למדד ובמידה ולא תהיה תוספת חיתומית או מקצועית ייתכן והגבייה הראשונה תהיה גבייה יחסית או כפולה בהתאם למועד החיוב.
                הגבייה תתבצע במועד התשלום הקבוע של אמצעי התשלום שלך.
                במידה ובעתיד תרצה לעשות שינוי או ביטול תוכל לבצע זאת בכל אחד מהאמצעים שמעמידה לרשותך חברת הביטוח פקס. מייל. מוקד שירות. אזור אישי באתר החברה.
                חשוב לי שתדע שתוכל לבטל את כל אחד מנספחים הכלולים בחבילה בכל עת בתנאי שנותר מוצר הבסיס.
              </div>

              <div class="actionsRow" style="margin-top:14px; justify-content:flex-end">
                <button class="btn btnPrimary" type="button" id="nextToStage3" disabled>הבא — שלב 3 (גילוי נאות כיסויים)</button>
              </div>
            </div>
          </div>

        <div class="modalOverlay" id="stage3Modal" style="display:none" role="dialog" aria-modal="true" aria-label="שלב 3">
          <div class="modalCard">
            <div class="modalHead">
              <div class="modalTitle">שלב 3 — גילוי נאות כיסויים</div>
              <button class="btn btnSoft btnSmall" type="button" id="stage3Close">סגור</button>
            </div>
            <div class="modalBody">
              <div class="stage2Script">
                (שלד) כאן נכניס את מלל "גילוי נאות כיסויים" ואת אישורי הלקוח לפי ההנחיות שלך.
              </div>
            </div>
          </div>
        </div>
  ...
  function bindStage1Events(){
    ...
    const afterLine = document.getElementById('afterConsentLine');
        ...
        // Stage 2 (Har Habituach consent)
        const hbYes = document.getElementById('hbYes');
        const hbNo  = document.getElementById('hbNo');
        const hbMeta = document.getElementById('hbMeta');
        const hbAfter = document.getElementById('hbAfter');
        const existingWrap = document.getElementById('existingPoliciesWrap');

        function syncHbUI(){
          const v = currentProfile.hbConsent || '';
          if(hbYes && hbNo) setYN(hbYes, hbNo, v);
          if(hbMeta) hbMeta.textContent = v ? ('נבחר: ' + v) : 'לא סומן';
          const ok = (v === 'כן');
          if(hbAfter) hbAfter.style.display = ok ? '' : 'none';
          if(existingWrap) existingWrap.style.display = ok ? '' : 'none';
        }

        if(hbYes && hbNo){
          syncHbUI();
          hbYes.addEventListener('click', () => {
            currentProfile.hbConsent = 'כן';
            syncHbUI();
          });
          hbNo.addEventListener('click', () => {
            currentProfile.hbConsent = 'לא';
            syncHbUI();
          });
        }

        // Stage 2 (Main consideration) — must choose 1 of 3 before continuing
        const mcMeta = document.getElementById('mainConsiderationMeta');
        const mcGrid = document.getElementById('mainConsiderationGrid');
        const nextToStage3 = document.getElementById('nextToStage3');
        const stage3Modal = document.getElementById('stage3Modal');
        const stage3Close = document.getElementById('stage3Close');

        function syncMainConsiderationUI(){
          const v = currentProfile.mainConsideration || '';
          if(mcMeta) mcMeta.textContent = v ? ('נבחר: ' + v) : 'לא סומן';
          if(nextToStage3) nextToStage3.disabled = !v;
        }

        if(mcGrid){
          // init checked state is already rendered via template, just wire changes
          mcGrid.addEventListener('change', (e) => {
            const t = e.target;
            if(t && t.name === 'mainConsideration'){
              currentProfile.mainConsideration = t.value;
              syncMainConsiderationUI();
            }
          });
        }
        syncMainConsiderationUI();

        // Stage 3 modal controls
        if(nextToStage3){
          nextToStage3.addEventListener('click', () => {
            if(nextToStage3.disabled) return;
            // close stage2, open stage3
            if(modal) modal.style.display = 'none';
            if(stage3Modal) stage3Modal.style.display = '';
          });
        }
        if(stage3Close){
          stage3Close.addEventListener('click', () => {
            if(stage3Modal) stage3Modal.style.display = 'none';
          });
        }
        if(stage3Modal){
          stage3Modal.addEventListener('click', (e) => {
            if(e.target === stage3Modal) stage3Modal.style.display = 'none';
          });
        }
