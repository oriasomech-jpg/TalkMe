...
(() => {
  ...
  function doLogout_(){
  // Full logout: clear both session + remember-me storage
  ...
  (function bootAuth_(){
    ...
    // Restore session/remembered user — but ONLY if it still exists in the users list and is active.
    const remembered = getStoredUser_();
    const sessionU = getSessionUser_();
    const u = remembered || sessionU;

    const rememberEl = document.getElementById('rememberMe');
    if(rememberEl) rememberEl.checked = !!remembered;

    const users = getUsers_(); // normalized list (and seeded if first run)

    if(u && u.username){
      const uname = String(u.username || '').trim();
      const real = users.find(x => String(x.username||'').trim() === uname);

      // validate existence + active
      if(real && real.active !== false){
        const safe = { username: real.username, role: real.role, displayName: real.displayName };

        state.currentUser = safe;
        state.agentName = real.displayName || 'משתמש';

        // Refresh stored/session identity to match latest role/name (keep remember choice)
        persistUser_(safe, true);
      ...
      // Stale / deleted / disabled user in storage -> force login
      clearPersistedUser_();
