...
function openPolicyDiscountsModal(policy, onSave){
  ...
  const COMPANY_LOGOS = {
    'הראל': 'assets/insurers/harel.png',
    'הפניקס': 'assets/insurers/phoenix.png',
    'מנורה': 'assets/insurers/menora.png',
    'מגדל': 'assets/insurers/migdal.png',
    'הכשרה': 'assets/insurers/hachshara.png',
    'כלל': 'assets/insurers/clal.png'
  };
        ...
        <div class="giCompanyList">
          ${COMPANIES.map(c => `
            <button type="button" class="btn btnSoft giCompanyBtn" data-company="${esc(c)}">
              ${COMPANY_LOGOS[c] ? `<img class="giCompanyLogo" src="${escapeAttr(COMPANY_LOGOS[c])}" alt="">` : ``}
              <span class="giCompanyName">${esc(c)}</span>
            </button>
          `).join('')}
        </div>
