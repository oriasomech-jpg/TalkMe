  ...
  function openWizard({ customerId=null, proposalId=null }){
    ...
    if(statusSelect){
      ...
      statusSelect.onchange = () => {
        ...
        savePill.textContent = (proposal.status==='טיוטה' ? '' : proposal.status);
...
function gotoStep(i){
  ...
  function renderWizard(){
    ...
    if(!wiz) return;
    const stepKey = WIZ_STEPS[wiz.stepIndex].key;
    savePill.textContent = (wiz.proposal.status==='טיוטה' ? '' : wiz.proposal.status);
...
function renderNewPoliciesStep(){
      ...
      <div class="payPanel">
        ${method==='card' ? `
          ...
          <div class="grid3" style="margin-top:10px">
            ...
            <div class="field"><div class="label">ב־מסך סיכום</div><div class="value">יופיע רק **** 4 ספרות אחרונות</div></div>
          </div>
        ` : `
          <div class="grid3">
  ...
  function renderSummaryStep(){
    ...
    wizardMain.innerHTML = `
      ...
      <div class="hr"></div>
      <div class="grid2" style="margin-top:10px">
        <button class="btn btnSoft" id="pdfBtn">הדפס / הורד מסך סיכום</button>
    ...
    if(markDoneBtnEl) markDoneBtnEl.addEventListener('click', () => {
      ...
      savePill.textContent = (p.status==='טיוטה' ? '' : p.status);
      render();
    });
  }
  ...
  // ---------- מסך סיכום (Client-side print to מסך סיכום) ----------
  function openPrintView(proposal){
    ...
    const html = `
      ...
      <body>
        ...
        <div class="muted" style="margin-top:14px">כדי להוריד כ־מסך סיכום: הדפס → Save as מסך סיכום</div>
    ...
    w.document.close();
      ...
      toast('שגיאה', 'לא הצלחתי ליצור מסך סיכום. נסה שוב');
    }
  }
  ...
  function computeCompletion(p){
    ...
    const items = [
      ...
      { key:'pdf', label:'מסך סיכום', ok: okPdf },
    ];
