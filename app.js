...
(() => {
  ...
  const state = {
    ...
    agentName: 'אוריה (דמו)',
    customers: [
      { id:'c1', fullName:'דניאל כהן', tz:'123456789', phone:'050-1234567', email:'daniel@mail.com', status:'פעיל', segment:'פרימיום' },
      { id:'c2', fullName:'נועה לוי', tz:'987654321', phone:'052-7654321', email:'noa@mail.com', status:'חדש', segment:'ליד' },
      { id:'c3', fullName:'יוסי מזרחי', tz:'314159265', phone:'054-5551234', email:'yossi@mail.com', status:'פעיל', segment:'סטנדרט' },
    ],
    proposals: []
  };
  ...
  function buildInsuredTabs(){
    insuredTabsEl.innerHTML = wiz.proposal.insuredList.map(x => (
      `<button class="insuredTab ${x.id===wiz.proposal.activeInsuredId?'active':''}" data-insured="${x.id}">${escapeHtml(x.label)}</button>`
    )).join('');
    $$('[data-insured]', insuredTabsEl).forEach(b => b.addEventListener('click', () => {
      wiz.proposal.activeInsuredId = b.getAttribute('data-insured');
      buildInsuredTabs();
      if(WIZ_STEPS[wiz.stepIndex].key === 'medical'){
        renderWizard(); // rerender questions per insured
      }
    }));
  }
  ...
  function renderCustomerStep(){
    ...
    wizardMain.innerHTML = `
      ...
      <div class="grid2">
        ${field('שם מלא', 'customer_fullName', p.customer.fullName)}
        ${field('ת״ז', 'customer_tz', p.customer.tz)}
        ${field('טלפון', 'customer_phone', p.customer.phone)}
        ${field('אימייל', 'customer_email', p.customer.email)}
      </div>
      <div class="hr"></div>
    ...
    bindInput('customer_fullName', v => { p.customer.fullName = v; p.customerName = v; $('#wizardTitle').textContent = v ? `הצעה – ${v}` : 'הצעה חדשה'; });
    bindInput('customer_tz', v => p.customer.tz = v);
    bindInput('customer_phone', v => p.customer.phone = v);
    bindInput('customer_email', v => p.customer.email = v);
    bindTextarea('customer_notes', v => p.customer.notes = v);
    const pushCustomer = debounce(async () => {
      if(!GOOGLE_SCRIPT_URL) return;
      // upsert a minimal customer record
      const cust = {
        id: p.customerId || ('c_' + p.id),
      ...
      await syncUpCustomer(cust);
    }, 800);
    // attach to inputs
    ['customer_fullName','customer_tz','customer_phone','customer_email','customer_notes'].forEach(fid => {
      const el = document.getElementById(fid);
      if(el) el.addEventListener('input', () => pushCustomer());
    });
  ...
  function renderSummaryStep(){
    ...
    $('#pdfBtn').addEventListener('click', () => openPrintView(p));
