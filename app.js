...
(() => {
  ...
  const USERS_KEY = 'gemel_users_v1';
  ...
  function loadUsers_(){
    try{
      ...
      if(Array.isArray(arr) && arr.length){
        ...
        if(!arr.some(u => u && u.role === 'admin')) return DEFAULT_USERS.slice();
        return arr.map(u => ({
          username: String(u.username||'').trim(),
          password: String(u.password||''),
          role: (u.role === 'admin' ? 'admin' : 'agent'),
          displayName: String(u.displayName||'').trim() || String(u.username||'')
        })).filter(u => u.username);
      }
    }catch(_){}
  ...
  function saveUsers_(users){
    ...
    if(!clean.some(u => u.role === 'admin')) clean.unshift({ username:'admin', password:'3316', role:'admin', displayName:'מנהל מערכת' });
    try{ localStorage.setItem(USERS_KEY, JSON.stringify(clean)); }catch(_){ /* ignore */ }
    USERS = clean;
  }
  ...
  function renderSettings(){
    viewEl.innerHTML = `
      <div class="grid2">
        ...
        <div class="sideCard">
          <div class="sideTitle">ניהול משתמשים (מנהל בלבד)</div>
          <div class="sideHint">משתמשים נשמרים מקומית בדפדפן (Local). חיבור לשרת נוסיף בהמשך בלי לשבור סנכרון.</div>
          ...
          <div class="sideTitle" style="font-size:13px;opacity:.9">הוספת נציג</div>
          <div class="grid2" style="margin-top:8px">
            <div class="field">
              <label>שם מלא</label>
              <input id="newUserDisplay" placeholder="לדוגמה: סתיו כהן"/>
            </div>
            <div class="field">
              <label>שם משתמש</label>
              <input id="newUserUsername" placeholder="stav"/>
            </div>
          </div>
          <div class="grid2" style="margin-top:8px">
            <div class="field">
              <label>סיסמה</label>
              <input id="newUserPassword" placeholder="••••" type="password"/>
            </div>
            <div class="field">
              <label>תפקיד</label>
...
function setSelectValue(id, value){
  ...
  (async () => {
    ...
    // --- Admin users management (local) ---
    if(isAdmin_()){
      const tbody = $('#usersTbody');
      const paint = () => {
        const me = (state.currentUser && state.currentUser.username) ? state.currentUser.username : '';
        tbody.innerHTML = USERS.map(u => {
          const isMe = u.username === me;
          const roleLabel = u.role === 'admin' ? 'מנהל' : 'נציג';
          return `
            <tr>
              ...
              <td style="display:flex;gap:8px;flex-wrap:wrap">
                <button class="btn btnSoft" data-act="edit" data-u="${escapeAttr(u.username)}">ערוך</button>
                <button class="btn btnSoft" data-act="pass" data-u="${escapeAttr(u.username)}">סיסמה</button>
                ${isMe ? '' : `<button class="btn btnDanger" data-act="del" data-u="${escapeAttr(u.username)}">מחק</button>`}
              </td>
            </tr>
          `;
        }).join('') || `<tr><td colspan="4" class="muted">אין משתמשים</td></tr>`;
      };
      paint();
      ...
      addBtn.addEventListener('click', () => {
        ...
        const role = ($('#newUserRole').value || 'agent') === 'admin' ? 'admin' : 'agent';

        if(!username || !password || !displayName){
          toast('חסר פרטים', 'מלא שם מלא, שם משתמש וסיסמה');
          return;
        }
        if(USERS.some(u => u.username === username)){
          toast('קיים כבר', 'שם המשתמש כבר קיים');
          return;
        }
        const next = USERS.slice();
        next.push({ username, password, role, displayName });
        saveUsers_(next);
        // Reset fields
      ...
      tbody.addEventListener('click', (e) => {
        ...
        if(act === 'del'){
          if(u.role === 'admin' && USERS.filter(x => x.role === 'admin').length <= 1){
            toast('לא ניתן', 'חייב להיות לפחות מנהל אחד');
            return;
          }
          const next = USERS.filter(x => x.username !== uname);
          saveUsers_(next);
        ...
        if(act === 'pass'){
          const np = prompt('סיסמה חדשה עבור: ' + (u.displayName || u.username));
          if(np === null) return;
          const newPass = String(np).trim();
          if(!newPass){
            toast('בוטל', 'לא בוצע שינוי');
            return;
          }
          const next = USERS.map(x => x.username === uname ? ({ ...x, password:newPass }) : x);
          saveUsers_(next);
        ...
        if(act === 'edit'){
          const dn = prompt('שם מלא (Display Name)', u.displayName || u.username);
          if(dn === null) return;
          const newDn = String(dn).trim();
          if(!newDn){
            toast('חסר', 'שם מלא לא יכול להיות ריק');
            return;
          }
          const role = prompt('תפקיד: admin / agent', u.role || 'agent');
          if(role === null) return;
          const newRole = (String(role).trim() === 'admin') ? 'admin' : 'agent';
          // Prevent removing last admin
          if(u.role === 'admin' && newRole !== 'admin' && USERS.filter(x => x.role === 'admin').length <= 1){
            toast('לא ניתן', 'חייב להיות לפחות מנהל אחד');
            return;
          }
          const next = USERS.map(x => x.username === uname ? ({ ...x, displayName:newDn, role:newRole }) : x);
          saveUsers_(next);
          toast('עודכן', 'פרטי משתמש עודכנו');
          // If editing current user, refresh badge + permissions
          if(state.currentUser && state.currentUser.username === uname){
            const updated = USERS.find(x => x.username === uname);
            state.currentUser = { username:updated.username, role:updated.role, displayName:updated.displayName };
            state.agentName = updated.displayName;
