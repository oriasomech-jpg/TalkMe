...
function normalizeHarHeader_(s){
    return String(s||'')
      .trim()
      .toLowerCase()
  ...
  function findHarCol_(header, aliases){
    const hNorm = header.map(normalizeHarHeader_);
    const aNorm = (aliases||[]).map(normalizeHarHeader_).filter(Boolean);
    // 1) exact matches
    for(const a of aNorm){
      const idx = hNorm.findIndex(h => h === a);
      if(idx >= 0) return idx;
    }
    // 2) contains matches (fallback)
    for(const a of aNorm){
      const idx = hNorm.findIndex(h => h.includes(a) || a.includes(h));
      if(idx >= 0) return idx;
    }
    return -1;
  }
  ...
  function detectHarHeaderRow_(rows){
    // Find a row that looks like the table header, even if columns are slightly renamed.
    for(let i=0;i<rows.length;i++){
      const r = rows[i] || [];
      const cells = r.map(x => normalizeHarHeader_(x)).filter(Boolean);
      const joined = cells.join('|');
      const hasCompany = joined.includes('חברה');
      const hasPolicy = joined.includes('פוליס'); // covers "מספר פוליסה" / "מס פוליסה" / "מס׳ פוליסה"
      const hasPremium = joined.includes('פרמיה') || joined.includes('דמי ביטוח') || joined.includes('תשלום');
      if(hasCompany && hasPolicy && hasPremium){
        return i;
      }
    }
    return -1;
  ...
  function harRowsToOldPolicies_(rows, insuredId){
    ...
    const cMain    = findHarCol_(header, ['ענף ראשי','ענף']);
    const cSub     = findHarCol_(header, ['ענף משני','ענף (משני)','ענף משני/תחום','תחום']);
    const cCompany = findHarCol_(header, ['חברה','חברת ביטוח','שם חברה']);
    const cPremium = findHarCol_(header, ['פרמיה בש"ח','פרמיה בשח','פרמיה','דמי ביטוח','תשלום חודשי','תשלום','פרמיה חודשית']);
    const cPremType= findHarCol_(header, ['סוג פרמיה','סוג תשלום','תדירות תשלום','תדירות','מחזור חיוב']);
    const cPolicy  = findHarCol_(header, ['מספר פוליסה','מס פוליסה','מס׳ פוליסה','פוליסה','מספר חוזה','מס חוזה']);
    const cProgram = findHarCol_(header, ['סיווג תכנית','סיווג תוכנית','שם תכנית','תכנית','תוכנית']);
    const cPeriod  = findHarCol_(header, ['תקופת ביטוח','תקופה']);
    ...
    for(let i=headerIdx+1;i<rows.length;i++){
      ...
      // Skip obvious non-data / section rows
      const looksLikeHeader = normalizeHarHeader_(companyRaw).includes('חברה') || normalizeHarHeader_(policyNo).includes('פוליס');
      const isSection = normalizeHarHeader_(main).startsWith('תחום -') || normalizeHarHeader_(companyRaw).startsWith('תחום -');
      if(isSection || looksLikeHeader) continue;
        ...
        <table>
          ...
          <tbody>
            ${rows.length ? rows.map(({x,idx}) => {
              let dec = (x.decision !== undefined && x.decision !== null) ? String(x.decision) : String(x.action||'');
              ...
              <tr>
                ...
                <td style="min-width:200px">
                  <select data-old-decision="${idx}">
                    <option value="" ${dec===""?'selected':''}>בחר פעולה</option>
