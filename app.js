...
(() => {
  ...
  function setUsers_(arr){
    try{
      localStorage.setItem(USERS_KEY, JSON.stringify(Array.isArray(arr) ? arr : []));
    }catch(_){}
  }
  function syncTeamAgentsFromUsers_(){
    const agents = getUsers_().filter(u => u && u.role === 'agent').map(u => u.displayName).filter(Boolean);
    if(agents.length) state.teamAgents = agents;
  }
...
function renderKpisSettings(){
  ...
  function renderSettings(){
    viewEl.innerHTML = `
      <div class="grid2">
        ...
        <div class="sideCard">
          ...
          <div class="sideHint">פתיחת יוזרים לנציגים: שם משתמש + קוד (סיסמה) + שם נציג. נשמר מקומית בדפדפן (דמו) — בהמשך נעביר ל־Sheets.</div>

          <div class="grid2" style="margin-top:10px">
            <div class="field">
              <label>שם משתמש</label>
              <input id="newUserUsername" placeholder="agent4"/>
            </div>
            <div class="field">
              <label>קוד / סיסמה</label>
              <input id="newUserPassword" placeholder="1234"/>
            </div>
          </div>

          <div class="field">
            <label>שם הנציג (תצוגה במערכת)</label>
            <input id="newUserDisplay" placeholder="לדוגמה: שחר כהן"/>
          </div>

          <div style="display:flex;gap:8px;flex-wrap:wrap">
            <button class="btn btnPrimary" id="addUserBtn">+ צור נציג</button>
            <button class="btn btnSoft" id="resetUsersBtn" title="מחזיר לרשימת דמו המקורית">איפוס משתמשים (דמו)</button>
          </div>

          <div style="height:10px"></div>
          <div class="tableWrap">
            <table>
              <thead><tr><th>שם משתמש</th><th>שם</th><th>תפקיד</th><th></th></tr></thead>
    ...
    // User management (admin only) — local demo storage
    if(isAdmin_()){
      const tbody = $('#usersTbody');
      const uUser = $('#newUserUsername');
      const uPass = $('#newUserPassword');
      const uDisp = $('#newUserDisplay');

      function renderUsersTable_(){
        if(!tbody) return;
        const users = getUsers_();
        tbody.innerHTML = users.map(u => {
          const canDel = (u.role !== 'admin') && (u.username !== (state.currentUser && state.currentUser.username));
          return `
            <tr>
              ...
              <td style="text-align:left">
                ${canDel ? `<button class="btn btnSoft" data-del-user="${escapeAttr(u.username)}">מחיקה</button>` : `<span style="color:rgba(18,19,25,.45)">—</span>`}
              </td>
            </tr>
          `;
        }).join('') || `<tr><td colspan="4" style="color:rgba(18,19,25,.55)">אין משתמשים.</td></tr>`;

        $$('[data-del-user]', tbody).forEach(b => b.addEventListener('click', () => {
          const username = b.getAttribute('data-del-user');
          const users2 = getUsers_();
          const keep = users2.filter(x => x.username !== username);
          setUsers_(keep);
          syncTeamAgentsFromUsers_();
      ...
      if(addBtn){
        addBtn.addEventListener('click', () => {
          ...
          const displayName = String(uDisp?.value||'').trim();
          if(!username || !password || !displayName){
            toast('חסר מידע', 'מלא שם משתמש, קוד ושם נציג');
            return;
          }
          const users = getUsers_();
          if(users.some(x => (x.username||'').toLowerCase() === username.toLowerCase())){
            toast('שגיאה', 'שם משתמש כבר קיים');
            return;
          }
          users.push({ username, password, role:'agent', displayName });
          setUsers_(users);
          syncTeamAgentsFromUsers_();
          toast('נוצר', 'המשתמש נוסף בהצלחה');
          if(uUser) uUser.value = '';
          if(uPass) uPass.value = '';
          if(uDisp) uDisp.value = '';
          renderUsersTable_();
          render();
        });
      }

      const resetBtn = $('#resetUsersBtn');
      if(resetBtn){
        resetBtn.addEventListener('click', () => {
          if(!confirm('לאפס משתמשים לרשימת הדמו המקורית?')) return;
          setUsers_(DEFAULT_USERS);
          syncTeamAgentsFromUsers_();
          toast('אופס', 'המשתמשים אופסו לדמו');
          renderUsersTable_();
          render();
        });
      }
