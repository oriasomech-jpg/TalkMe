...
(() => {
  ...
  function saveUsers_(arr){
    try{ localStorage.setItem(USERS_STORAGE_KEY, JSON.stringify(arr)); }catch(_){}
  }
  ...
  function addUser_(u){
    ...
    const exists = users.some(x => String(x.username).toLowerCase() === String(u.username).toLowerCase());
  ...
  function resetPassword_(username, newPass){
    const users = loadUsers_();
    const u = users.find(x => x.username === username);
    if(!u) return { ok:false, msg:'משתמש לא נמצא' };
    u.password = newPass;
    saveUsers_(users);
    return { ok:true };
  }
  ...
  function renderSettings(){
    viewEl.innerHTML = `
      <div class="grid2">
        ...
        ${isAdmin_() ? `
        <div class="sideCard">
          <div class="sideTitle">ניהול משתמשים</div>
          <div class="sideHint">פתיחת יוזרים חדשים לנציגים (שם + קוד).</div>
          <div class="grid3">
            ${field('שם מלא', 'um_display', '')}
            ${field('שם משתמש', 'um_username', '')}
            ${field('קוד/סיסמה', 'um_password', '', 'numeric')}
          </div>
          <div style="height:10px"></div>
          <button class="btn btnPrimary" id="um_addBtn">פתח יוזר</button>
          <div class="hr"></div>
          <div id="um_list"></div>
        </div>
        ` : ''}
sideTitle">הערות</div>
    ...
    if(isAdmin_()) renderUserManager_();

  }

  function renderUserManager_(){
    const listEl = document.getElementById('um_list');
    ...
    function drawList(){
      ...
      const rows = users.map(u => {
        ...
        const safeRole = role === 'admin' ? 'מנהל' : 'נציג';
        const actions = (u.username === 'admin') ? '' : `
          <button class="btn btnSoft btnTiny" data-um-reset="${escapeAttr(u.username)}">איפוס קוד</button>
          <button class="btn btnSoft btnTiny" data-um-del="${escapeAttr(u.username)}">מחק</button>
        `;
        return `
          <div class="sideRow" style="display:flex;justify-content:space-between;gap:10px;align-items:center;border:1px solid var(--line);border-radius:12px;padding:10px;margin-bottom:8px;background:#fff">
            <div style="display:flex;flex-direction:column;gap:2px;min-width:0">
              <b style="white-space:nowrap;overflow:hidden;text-overflow:ellipsis">${escapeHtml(u.displayName || u.username)}</b>
              <span style="color:var(--muted2);font-size:12px">יוזר: <span class="mono">${escapeHtml(u.username)}</span> • ${safeRole}</span>
            </div>
      ...
      listEl.innerHTML = rows || `<div class="sideHint">אין משתמשים.</div>`;

      $$('[data-um-del]').forEach(btn => btn.addEventListener('click', () => {
        const username = btn.getAttribute('data-um-del');
        if(!username) return;
        deleteUser_(username);
        toast('נמחק', 'המשתמש הוסר');
        drawList();
      }));

      $$('[data-um-reset]').forEach(btn => btn.addEventListener('click', () => {
        const username = btn.getAttribute('data-um-reset');
        if(!username) return;
        const newPass = prompt('הכנס קוד/סיסמה חדש (4-10 ספרות מומלץ):', '');
        if(!newPass) return;
        const res = resetPassword_(username, String(newPass).trim());
        if(!res.ok){ toast('שגיאה', res.msg || 'לא הצליח'); return; }
        toast('עודכן', 'הקוד עודכן');
      }));
    }

    addBtn.addEventListener('click', () => {
      const display = (document.getElementById('um_display')?.value || '').trim();
      ...
      const res = addUser_({ username, password, role:'agent', displayName: display });
      if(!res.ok){ toast('לא נשמר', res.msg || 'שגיאה'); return; }
      // keep agent pool for assignment drop-downs
      if(display && !state.teamAgents.includes(display)) state.teamAgents.push(display);
      toast('נוצר', 'יוזר חדש נוסף');
  ...
  function validateCurrentStep(){
    ...
    // Extra rule (payer step): אם בחרו כרטיס, נדרוש לפחות 4 ספרות אחרונות + תוקף.
    // המספר המלא לא נשמר, לכן לא נדרוש 12-16 ספרות.
    if(wiz && WIZ_STEPS[wiz.stepIndex] && WIZ_STEPS[wiz.stepIndex].key==='payer'){
      ...
      if(method==='card'){
        ...
        if(numEl && digits.length < 4){
          numEl.classList.add('reqError');
          toast('חסר מידע', 'יש למלא לפחות 4 ספרות אחרונות של הכרטיס + תוקף');
