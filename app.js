...
// Keep wizard title in sync
  ...
  function renderOldPoliciesStep(){
    ...
    wizardMain.innerHTML = `
      ...
      <div class="grid2">
        ${logoSelectField('חברה','old_company',['הראל','כלל','מנורה','הפניקס','מגדל','הכשרה'])}
...
function renderNewPoliciesStep(){
    ...
    wizardMain.innerHTML = `
      ...
      <div class="grid3">
        ${logoSelectField('חברה','new_company',['הראל','כלל','מנורה','הפניקס','מגדל','הכשרה'])}
...
// ---------- Company Logos (safe, isolated) ----------
const GI_COMPANY_LOGOS = {
  'הראל': 'assets/companies/harel.svg',
  'כלל': 'assets/companies/clal.svg',
  'מנורה': 'assets/companies/menora.svg',
  'הפניקס': 'assets/companies/afenix.svg',
  'מגדל': 'assets/companies/migdal.svg',
  'הכשרה': 'assets/companies/achshara.svg',
};

function giNormCompanyName(name){
  return String(name || '').replace(/\s+/g,' ').trim();
}
function giCompanyLogoSrc(name){
  const n = giNormCompanyName(name);
  return GI_COMPANY_LOGOS[n] || '';
}

function companyCell(companyName){
  const name = giNormCompanyName(companyName);
  const src = giCompanyLogoSrc(name);
  // In tables we want a consistent, professional cell:
  // small rounded logo badge + name next to it.
  if(!src){
    return `<div class="giCompanyCell"><span class="giCompanyBadge giCompanyBadgePh" aria-hidden="true"></span><span class="giCompanyName">${escapeHtml(name || '')}</span></div>`;
  }
  return `
    <div class="giCompanyCell">
      <span class="giCompanyBadge" aria-hidden="true">
        <img class="giCompanyCellLogo" src="${src}" alt="">
      </span>
      <span class="giCompanyName">${escapeHtml(name)}</span>
    </div>
  `;
}

function logoSelectField(label, id, options){
  const list = Array.isArray(options) ? options : [];
  const first = giNormCompanyName(list[0] || '');
  const firstSrc = giCompanyLogoSrc(first);
  const optBtns = list.map(o => {
    const name = giNormCompanyName(o);
    const src = giCompanyLogoSrc(name);
    const logo = src ? `<img class="giLogoOptImg" src="${src}" alt="${escapeHtml(name)}"/>` : `<span class="giLogoOptPh" aria-hidden="true"></span>`;
    return `<button type="button" class="giLogoOpt" title="${escapeAttr(name)}" data-value="${escapeHtml(name)}">${logo}<span class="giSrOnly">${escapeHtml(name)}</span></button>`;
  }).join('');
  const btnLogo = firstSrc ? `<img class="giLogoMiniImg" src="${firstSrc}" alt="${escapeHtml(first)}"/>` : `<span class="giLogoMiniPh" aria-hidden="true"></span>`;
  return `
    <div class="field">
      <label>${escapeHtml(label)}</label>
      <div class="giLogoSelect" data-logo-select="${escapeHtml(id)}">
        <button type="button" class="giLogoSelectBtn" aria-haspopup="listbox" aria-expanded="false">
          <span class="giLogoMini">${btnLogo}</span>
          <span class="giLogoText">${escapeHtml(first)}</span>
          <span class="giLogoChevron" aria-hidden="true">▾</span>
        </button>
        <div class="giLogoSelectMenu" hidden>
          ${optBtns}
        </div>
        <input class="giLogoHidden" type="hidden" id="${escapeHtml(id)}" value="${escapeHtml(first)}"/>
      </div>
    </div>
  `;
}

let _giLogoSelectDocBound = false;
function wireLogoSelect(id){
  const wrap = document.querySelector(`.giLogoSelect[data-logo-select="${CSS.escape(id)}"]`);
  if(!wrap) return;
  const btn = wrap.querySelector('.giLogoSelectBtn');
  const menu = wrap.querySelector('.giLogoSelectMenu');
  const hidden = wrap.querySelector('input.giLogoHidden');
  const txt = wrap.querySelector('.giLogoText');
  const mini = wrap.querySelector('.giLogoMini');

  function setBtn(company){
    const name = giNormCompanyName(company);
    const src = giCompanyLogoSrc(name);
    if(btn){ btn.setAttribute('title', name || ''); btn.setAttribute('aria-label', name || ''); }
    if(txt) txt.textContent = name || '';
    if(mini){
      mini.innerHTML = src
        ? `<img class="giLogoMiniImg" src="${src}" alt="${escapeHtml(name)}"/>`
        : `<span class="giLogoMiniPh" aria-hidden="true"></span>`;
    }
  }
  function open(){
    if(!menu) return;
  ...
  if(menu && !menu.dataset.bound){
    ...
    menu.querySelectorAll('button.giLogoOpt[data-value]').forEach(b => {
      b.addEventListener('click', (e) => {
        e.preventDefault();
        const v = giNormCompanyName(b.dataset.value || '');
        if(hidden){
          hidden.value = v;
          setBtn(v);
  ...
  if(!_giLogoSelectDocBound){
    _giLogoSelectDocBound = true;
    document.addEventListener('click', (e) => {
      document.querySelectorAll('.giLogoSelect.open').forEach(w => {
        if(!w.contains(e.target)){
          const m = w.querySelector('.giLogoSelectMenu');
          if(m) m.hidden = true;
          const b = w.querySelector('.giLogoSelectBtn');
...
function openPolicyDiscountsModal(policy, onSave){
  ...
  const COMPANIES = ['הראל','כלל','מנורה','הפניקס','מגדל','הכשרה'];

  function openCompanyDiscountsListModal_(){
    ...
    ov.innerHTML = `
      <div class="giModal giCompanyModal">
        <div class="giModalHead">
          <div class="giModalTitle">מפרט הנחות לפי חברה</div>
          <button class="btn btnSoft" type="button" data-x>סגור</button>
        </div>

        <div class="giCompanyList">
          ${COMPANIES.map(c => `
            <button type="button" class="btn btnSoft giCompanyBtn" data-company="${esc(c)}">
              ${esc(c)}
            </button>
          `).join('')}
        </div>

        <div class="giCompanyHint">בהמשך: לחיצה על חברה תפתח מסך הנחות ייעודי.</div>
      </div>
    `;
    ...
    ov.querySelectorAll('[data-company]').forEach(b => b.addEventListener('click', () => {
      const c = b.getAttribute('data-company') || '';
      toast('בקרוב', 'נפתח מפרט הנחות עבור: ' + c);
    }));
      ...
      <div class="field" style="margin-top:10px">
        ...
        <div class="giCheckPills">
          ${TYPES.map(t => {
            ...
            const extra = (t === 'אחר') ? `
              <button class="btn btnSoft giInlineBtn" type="button" data-company-list>
                מפרט הנחות לפי חברה
              </button>
            ` : '';
  ...
  overlay.querySelector('[data-company-list]')?.addEventListener('click', openCompanyDiscountsListModal_);
