...
// Keep wizard title in sync
  ...
  function renderOldPoliciesStep(){
    ...
    wizardMain.innerHTML = `
      ...
      <div class="grid2">
        ${insurerField('חברה', 'old_company', ['הראל','כלל','מנורה','הפניקס','מגדל','הכשרה'])}
    ...
    // Insurer logos picker
    bindInsurerPicker('old_company');
...
function renderNewPoliciesStep(){
    ...
    wizardMain.innerHTML = `
      ...
      <div class="grid3">
        ${insurerField('חברה', 'new_company', ['הראל','כלל','מנורה','הפניקס','מגדל','הכשרה'])}
...
`;
  ...
  // ---------- Insurer logos (UI only) ----------
  // Keep this mapping in sync with assets/insurers/*
  const INSURER_LOGOS = {
    'הכשרה': 'assets/insurers/achshara.svg.png',
    'הפניקס': 'assets/insurers/afenix.svg.png',
    'כלל': 'assets/insurers/clal.svg.png',
    'הראל': 'assets/insurers/harel.svg.png',
    'מגדל': 'assets/insurers/megdl.svg.png',
    'מנורה': 'assets/insurers/menora.svg.png'
  };

  function insurerLogoSrc(name){
    return INSURER_LOGOS[String(name||'').trim()] || '';
  }

  function insurerField(label, id, companies){
    const opts = (companies||[]).map(c => `<option value="${escapeAttr(c)}">${escapeHtml(c)}</option>`).join('');
    const pills = (companies||[]).map(c => {
      const src = insurerLogoSrc(c);
      const img = src
        ? `<img src="${escapeAttr(src)}" alt="${escapeAttr(c)}" loading="lazy" />`
        : `<span class="insurerFallback">${escapeHtml(String(c).slice(0,2))}</span>`;
      return `
        <button type="button" class="insurerBtn" data-insurer="${escapeAttr(c)}" aria-label="${escapeAttr(c)}">
          ${img}
        </button>
      `;
    }).join('');
    return `
      <div class="field insurerField">
        <label for="${id}">${escapeHtml(label)}</label>
        <select id="${id}" class="insurerSelectHidden">${opts}</select>
        <div class="insurerPicker" id="${id}_picker" role="listbox" aria-label="${escapeAttr(label)}">
          ${pills}
        </div>
      </div>
    `;
  ...
  function bindInsurerPicker(selectId){
    const sel = document.getElementById(selectId);
    const wrap = document.getElementById(selectId + '_picker');
    if(!sel || !wrap) return;

    const sync = () => {
      const val = sel.value;
      wrap.querySelectorAll('.insurerBtn').forEach(btn => {
        btn.classList.toggle('active', btn.getAttribute('data-insurer') === val);
      });
    };

    wrap.addEventListener('click', (e) => {
      const btn = e.target.closest('.insurerBtn');
      if(!btn) return;
      const val = btn.getAttribute('data-insurer') || '';
      sel.value = val;
