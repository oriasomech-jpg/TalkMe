...
(function(){
  if(window.__GEMEL_INVEST_BOOTED__){ return; }
  window.__GEMEL_INVEST_BOOTED__ = true;
})();
...
(() => {
  ...
  // Option A (requested): users are stored locally per browser (localStorage).
  // Later we can move this to Apps Script / Google Sheets as the "server".
  const USERS_LS_KEY = 'gemel_users_v1';

  const DEFAULT_USERS = [
    { username:'admin', password:'3316', role:'admin', displayName:'מנהל מערכת', agencyRole:'מנהל' },
    { username:'agent1', password:'1111', role:'agent', displayName:'אוריה (דמו)', agencyRole:'נציג' },
    { username:'agent2', password:'2222', role:'agent', displayName:'סתיו', agencyRole:'נציג' },
    { username:'agent3', password:'3333', role:'agent', displayName:'דוד', agencyRole:'נציג' },
  ];
  ...
  function loadUsers_(){
      ...
      if(Array.isArray(arr) && arr.length){
        ...
        return arr
          ...
          .map(u => ({
            ...
            role: (String(u.role||'agent') === 'admin') ? 'admin' : 'agent',
            displayName: String(u.displayName||u.username||'').trim() || String(u.username||'').trim(),
            agencyRole: String(u.agencyRole||'').trim(),
            disabled: !!u.disabled
          }))
          .filter(u => u.username);
      }
    ...
    try{ localStorage.setItem(USERS_LS_KEY, JSON.stringify(DEFAULT_USERS)); }catch(_){}
    return DEFAULT_USERS.slice();
  }

  function saveUsers_(arr){
    try{ localStorage.setItem(USERS_LS_KEY, JSON.stringify(arr || [])); }catch(_){}
  }

  let users = loadUsers_();

  function refreshUsers_(){
    users = loadUsers_();
    ensureAtLeastOneAdmin_();
  }
  ...
  function ensureAtLeastOneAdmin_(){
    const admins = (users||[]).filter(u => u && u.role === 'admin' && !u.disabled);
    if(admins.length) return true;
    // fallback: re-add default admin
    users.unshift({ username:'admin', password:'3316', role:'admin', displayName:'מנהל מערכת', agencyRole:'מנהל' });
    saveUsers_(users);
    return true;
  }
  ...
  function applyPermissions_(){
    ...
    const userSettingsBtn = document.querySelector('.navItem[data-route="userSettings"]');
    if(userSettingsBtn){ userSettingsBtn.style.display = isAdmin_() ? '' : 'none'; }
  }
  ...
  function tryLogin_(username, password){
    ...
    const safe = { username:u.username, role:u.role, displayName:u.displayName, agencyRole:u.agencyRole||'' };
  ...
  function setRoute(r){
    // Admin-only routes
    if((r === 'settings' || r === 'userSettings') && !isAdmin_()){
      toast('אין הרשאה', 'רק מנהל יכול לפתוח את האזור הזה');
      r = 'customers';
    }
    route = r;
  ...
  (function bootAuth_(){
    ...
    if(u){
      // Normalize session payload (older sessions might miss displayName)
      const full = u.username ? getUserByUsername_(u.username) : null;
      const displayName = u.displayName || full?.displayName || full?.username || state.agentName;

      state.currentUser = { username: u.username || full?.username || '', role: u.role || full?.role || 'agent', displayName, agencyRole: u.agencyRole || full?.agencyRole || '' };
      state.agentName = displayName || state.agentName;
  ...
  function render(){
    ...
    if(route === 'proposals'){
      ...
      crumbEl.textContent = 'Admin';
      renderKpisUserSettings();
...
function renderKpisSettings(){
  ...
  function renderKpisUserSettings(){
    refreshUsers_();
    const total = (users||[]).length;
    const admins = (users||[]).filter(u => u && u.role === 'admin' && !u.disabled).length;
    const disabled = (users||[]).filter(u => u && u.disabled).length;
    kpisEl.innerHTML = [
      kpiCard('סה״כ משתמשים', total, 'מקומי (דפדפן)'),
      kpiCard('מנהלים', admins, 'עם הרשאת Admin'),
      kpiCard('מושבתים', disabled, 'לא יכולים להתחבר'),
    ].join('');
  }
function renderCustomersTable(rows){
  ...
  function renderSettings(){
    ...
    $('#connectBtn').addEventListener('click', async () => {
      ...
      // If you want dynamic URL, set GOOGLE_SCRIPT_URL in c
  function renderUserSettings(){
    ...
    viewEl.innerHTML = `
      <div class="grid2">
        <div class="sideCard">
          <div class="sideTitle">הוספת משתמש</div>

          <div class="grid2">
            <div class="field">
              <label>שם משתמש</label>
              <input id="us_username" placeholder="לדוגמה: agent5" autocomplete="off"/>
            </div>
            <div class="field">
              <label>סיסמה</label>
              <input id="us_password" placeholder="••••" type="password" autocomplete="new-password"/>
            </div>

            <div class="field">
              <label>שם תצוגה</label>
              <input id="us_display" placeholder="לדוגמה: נציג/ה — רועי" autocomplete="off"/>
            </div>
            <div class="field">
              <label>הרשאה</label>
              <select id="us_role">
                <option value="agent" selected>נציג</option>
                <option value="admin">מנהל</option>
              </select>
            </div>

            <div class="field" style="grid-column:1 / -1">
              <label>תפקיד בסוכנות (טקסט חופשי)</label>
              <input id="us_agencyRole" placeholder="לדוגמה: מנהל צוות / משווק פנסיוני / נציג מכירות" autocomplete="off"/>
            </div>
          </div>

          <div style="height:10px"></div>
          <div style="display:flex;gap:8px;flex-wrap:wrap">
            <button class="btn btnPrimary" id="addUserBtn">הוסף משתמש</button>
            <button class="btn btnSoft" id="resetDemoUsersBtn" title="יחזיר את רשימת הדמו (מקומית בדפדפן)">איפוס לרשימת דמו</button>
          </div>
          <div class="sideHint">📌 אופציה א׳: המשתמשים נשמרים מקומית בדפדפן. בהמשך נחבר לשרת/Sheets ואז זה יהיה משותף לכל הנציגים.</div>
        </div>

        <div class="sideCard">
          <div class="sideTitle">דגשים</div>
          <div class="sideHint">• מנהל רואה את כל המערכת + “הגדרות משתמשים”.</div>
          <div class="sideHint">• נציג רואה רק את הלקוחות שלו (ניישם מיד אחרי שננעל משתמשים).</div>
          <div class="sideHint">• באופציה א׳ אין אבטחת שרת אמיתית (הכל מקומי). כשנעבור לשרת נוסיף הצפנה והרשאות אמיתיות.</div>
        </div>
      </div>
        ...
        <table>
          ...
          <tbody>
            ${rows.length ? rows.map(u => `
              <tr style="${u.disabled ? 'opacity:.65' : ''}">
                <td class="mono"><b>${escapeHtml(u.username||'')}</b></td>
                <td>${escapeHtml(u.displayName||'')}</td>
                <td>${escapeHtml(u.agencyRole||'—')}</td>
                <td>${chip(u.role === 'admin' ? 'מנהל' : 'נציג', u.role === 'admin' ? 'warn' : '')}</td>
                <td>${chip(u.disabled ? 'מושבת' : 'פעיל', u.disabled ? '' : 'ok')}</td>
                <td style="text-align:left;white-space:nowrap">
                  <button class="btn btnTiny" data-us-edit="${escapeAttr(u.username)}">ערוך</button>
                  <button class="btn btnTiny" data-us-pass="${escapeAttr(u.username)}">סיסמה</button>
                  <button class="btn btnTiny" data-us-toggle="${escapeAttr(u.username)}">${u.disabled ? 'הפעל' : 'השבת'}</button>
                  <button class="btn btnSoft" data-us-del="${escapeAttr(u.username)}">מחק</button>
                </td>
              </tr>
            `).join('') : `<tr><td colspan="6" style="color:rgba(18,19,25,.55)">אין משתמשים.</td></tr>`}
          </tbody>
        </table>
      </div>
    `;
    ...
    addBtn?.addEventListener('click', () => {
      const username = String(document.getElementById('us_username')?.value || '').trim();
      const password = String(document.getElementById('us_password')?.value || '').trim();
      const displayName = String(document.getElementById('us_display')?.value || '').trim();
      const role = String(document.getElementById('us_role')?.value || 'agent') === 'admin' ? 'admin' : 'agent';
      const agencyRole = String(document.getElementById('us_agencyRole')?.value || '').trim();

      if(!username){ toast('חסר', 'שם משתמש חובה'); return; }
      if(!password){ toast('חסר', 'סיסמה חובה'); return; }
      if(getUserByUsername_(username)){ toast('קיים', 'שם משתמש כבר קיים'); return; }
      ...
      ensureAtLeastOneAdmin_();
      saveUsers_(users);
      toast('נוסף', 'משתמש נוצר בהצלחה');
      render();
    });

    // Reset demo users
    document.getElementById('resetDemoUsersBtn')?.addEventListener('click', () => {
      if(!confirm('לאפס לרשימת משתמשי דמו? זה ידרוס את המשתמשים המקומיים בדפדפן.')) return;
      users = DEFAULT_USERS.slice();
      saveUsers_(users);
      toast('אופס', 'הוחזרו משתמשי הדמו');
      render();
    });

    // Edit user
    $$('[data-us-edit]').forEach(b => b.addEventListener('click', () => {
      const username = b.getAttribute('data-us-edit');
      const u = getUserByUsername_(username);
      if(!u) return;

      const newDisplay = prompt('שם תצוגה:', u.displayName || u.username) ;
      if(newDisplay === null) return;

      const newAgency = prompt('תפקיד בסוכנות (טקסט חופשי):', u.agencyRole || '') ;
      if(newAgency === null) return;

      const newRole = prompt('הרשאה (admin / agent):', u.role || 'agent');
      if(newRole === null) return;

      u.displayName = String(newDisplay||'').trim() || u.username;
      u.agencyRole = String(newAgency||'').trim();
      u.role = (String(newRole||'agent').trim() === 'admin') ? 'admin' : 'agent';

      ensureAtLeastOneAdmin_();
      saveUsers_(users);

      // If editing current user, refresh session display name
      if(state.currentUser && state.currentUser.username === u.username){
        state.currentUser.displayName = u.displayName;
        state.currentUser.role = u.role;
        state.currentUser.agencyRole = u.agencyRole || '';
        setSessionUser_(state.currentUser);
        state.agentName = u.displayName;
        applyPermissions_();
      }

      toast('עודכן', 'פרטי משתמש עודכנו');
      render();
    }));

    // Change password
    $$('[data-us-pass]').forEach(b => b.addEventListener('click', () => {
      const username = b.getAttribute('data-us-pass');
      const u = getUserByUsername_(username);
      if(!u) return;
      const pw = prompt('סיסמה חדשה עבור ' + username + ':', '');
      if(pw === null) return;
      if(!String(pw).trim()){ toast('חסר', 'סיסמה לא יכולה להיות ריקה'); return; }
      u.password = String(pw).trim();
      saveUsers_(users);
      toast('עודכן', 'סיסמה עודכנה');
    }));

    // Toggle disable
    $$('[data-us-toggle]').forEach(b => b.addEventListener('click', () => {
      const username = b.getAttribute('data-us-toggle');
      const u = getUserByUsername_(username);
      if(!u) return;
      ...
      if(u.role === 'admin' && !u.disabled){
        ...
        if(activeAdmins.length <= 1){
          toast('לא ניתן', 'חייב להישאר לפחות מנהל אחד פעיל');
          return;
        }
      }
      ...
      // If disabling yourself, logout
      if(u.disabled && state.currentUser && state.currentUser.username === u.username){
        toast('עודכן', 'המשתמש הושבת — מתנתק…');
        setTimeout(() => doLogout_(), 400);
        return;
      }

      toast('עודכן', u.disabled ? 'המשתמש הושבת' : 'המשתמש הופעל');
      render();
    }));

    // Delete
    $$('[data-us-del]').forEach(b => b.addEventListener('click', () => {
      const username = b.getAttribute('data-us-del');
      const u = getUserByUsername_(username);
      if(!u) return;
      ...
      if(u.role === 'admin'){
        const activeAdmins = (users||[]).filter(x => x.role === 'admin');
        if(activeAdmins.length <= 1){
          toast('לא ניתן', 'חייב להישאר לפחות מנהל אחד');
          return;
        }
      }
      ...
      if(state.currentUser && state.currentUser.username === username){
        toast('נמחק', 'המשתמש נמחק — מתנתק…');
        setTimeout(() => doLogout_(), 300);
        return;
      }

      toast('נמחק', 'המשתמש הוסר');
      render();
    }));
  }
ode (recommended).
...
function gotoStep(i){
  ...
  function renderStepper(){
    ...
    stepperEl.innerHTML = WIZ_STEPS.map((s, idx) => {
      const lbl = (s.key === 'old' && insuredId !== 'main') ? `פוליסות קיימות – ${insuredLabel}` : s.label;
...
function renderNewPoliciesStep(){
    ...
    wizardMain.innerHTML = `
      <div class="sectionTitle">פוליסות חדשות</div>
      <div class="grid3">
