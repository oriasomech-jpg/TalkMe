...
(() => {
  ...
  // ---------- AUTH (LocalStorage users; can be moved to Google Sheets later) ----------
  const USERS_LS_KEY = 'gemel_users_v2';

  function seedDefaultUsers_(){
    return [
      { username:'admin', password:'3316', role:'admin', displayName:'מנהל מערכת' },
      { username:'agent1', password:'1111', role:'agent', displayName:'אוריה' },
  ...
  function loadUsers_(){
    ...
    const seeded = seedDefaultUsers_();
    try{ localStorage.setItem(USERS_LS_KEY, JSON.stringify(seeded)); }catch(_){}
    return seeded;
  }

  function saveUsers_(users){
    try{ localStorage.setItem(USERS_LS_KEY, JSON.stringify(users || [])); }catch(_){}
  }
  ...
  function getUsers_(){
    ...
    if(!state.users.length) state.users = loadUsers_();
    return state.users;
  }

  function upsertUser_(u){
    const users = getUsers_().slice();
    const uname = normalizeUsername_(u.username);
    if(!uname) return { ok:false, msg:'חסר שם משתמש' };
    const ix = users.findIndex(x => normalizeUsername_(x.username) === uname);
    if(ix >= 0){
      users[ix] = { ...users[ix], ...u, username: uname };
    }else{
      users.push({ ...u, username: uname });
    }
    state.users = users;
    saveUsers_(users);
    return { ok:true };
  ...
  function deleteUser_(username){
    ...
    const users = getUsers_().filter(x => normalizeUsername_(x.username) !== uname);
    state.users = users;
    saveUsers_(users);
  }

  function bulkGenerateAgents_(count, prefix='agent', startNum=1){
    const users = getUsers_().slice();
    const n = Math.max(0, Math.min(2000, Number(count)||0)); // safety cap
    const p = String(prefix||'agent').trim() || 'agent';
    let start = Number(startNum)||1;
    start = Math.max(1, start);

    for(let i=0;i<n;i++){
      const idx = start + i;
      const username = normalizeUsername_(p + idx);
      if(users.some(x => normalizeUsername_(x.username) === username)) continue;

      const pw = String(idx).padStart(4,'0'); // 0001, 0002 ... 0530
      users.push({
        username,
        password: pw,
        role: 'agent',
        displayName: 'סוכן ' + String(idx).padStart(2,'0')
      });
    }
  ...
  function downloadText_(filename, text, mime='text/plain'){
    try{
  ...
  function exportUsersCSV_(){
    const users = getUsers_();
    const rows = [['username','password','role','displayName']]
      .concat(users.map(u => [u.username||'', u.password||'', u.role||'', u.displayName||'']));
    const csv = rows.map(r => r.map(v => {
      const s = String(v).replace(/"/g,'""');
      return /[",\n]/.test(s) ? '"' + s + '"' : s;
    }).join(',')).join('\n');
    downloadText_('gemel_users.csv', csv, 'text/csv');
  }

  function exportUsersJSON_(){
    downloadText_('gemel_users.json', JSON.stringify(getUsers_(), null, 2), 'application/json');
  }
  ...
  function applyPermissions_(){
    ...
    if(badgeNameEl){
      badgeNameEl.textContent = (isAdmin_() ? 'מנהל: ' : 'נציג: ') + (state.agentName || 'אורח');
    }
...
// ---------- UI references ----------
  ...
  function tryLogin_(username, password){
    const u = getUsers_().find(x => normalizeUsername_(x.username) === normalizeUsername_(username) && String(x.password||'') === String(password));
    if(!u) return { ok:false, msg:'שם משתמש או סיסמה שגויים' };
    const safe = { username: normalizeUsername_(u.username), role: (u.role||'agent'), displayName: (u.displayName||u.username) };
  ...
  // ---------- Render ----------
  function render(){
    agentNameEl.textContent = (isAdmin_() ? 'מנהל: ' : 'נציג: ') + state.agentName;
              ...
              <tr>
                ...
                <td>
                  <div style="display:flex;gap:8px;align-items:center;flex-wrap:wrap">
                    ...
                    ${isAdmin_() ? `<button class="btn btnTiny" data-assign="${escapeAttr(c.id)}">שייך</button>` : ``}
                  </div>
                </td>
  ...
  function renderSettings(){
    viewEl.innerHTML = `
      <div class="grid2">
        ...
        ${isAdmin_() ? `
        <div class="sideCard">
          <div class="sideTitle">ניהול משתמשים (מנהל)</div>

          <div class="field">
            <label>יצירת משתמש חדש</label>
            <div style="display:grid;grid-template-columns: 1fr 1fr;gap:10px">
              <input id="uNewUsername" placeholder="שם משתמש (לועזי)" />
              <input id="uNewPassword" placeholder="סיסמה" />
            </div>
            <div style="height:8px"></div>
            <input id="uNewDisplay" placeholder="שם תצוגה (למשל: אוריה / סוכן 07)" />
            <div class="sideHint">ברירת מחדל: משתמש רגיל (סוכן/נציג). סיסמא מומלצת: 4 ספרות.</div>
            <div style="height:10px"></div>
            <div style="display:flex;gap:8px;flex-wrap:wrap">
              <button class="btn btnPrimary" id="uCreateBtn">צור משתמש</button>
              <button class="btn btnSoft" id="uExportCsvBtn">הורד CSV</button>
              <button class="btn btnSoft" id="uExportJsonBtn">הורד JSON</button>
            </div>
          </div>
          ...
          <div class="field">
            <label>יצירה אוטומטית (Bulk) — לדוגמה 530 סוכנים</label>
            <div style="display:grid;grid-template-columns: 1fr 1fr 1fr;gap:10px">
              <input id="uBulkCount" placeholder="כמות (למשל 530)" />
              <input id="uBulkPrefix" placeholder="Prefix (למשל agent)" value="agent"/>
              <input id="uBulkStart" placeholder="מספר התחלה (למשל 1)" value="1"/>
            </div>
            <div class="sideHint">נוצר: agent1..agent530. סיסמה = מספר 4 ספרות (0001..0530). אם משתמש כבר קיים — מדלג.</div>
            <div style="height:10px"></div>
            <div style="display:flex;gap:8px;flex-wrap:wrap">
              <button class="btn btnPrimary" id="uBulkBtn">צור משתמשים</button>
              <button class="btn btnSoft" id="uResetUsersBtn">איפוס לרשימת דמו</button>
            </div>
          </div>
          ...
          <div class="sideHint">מחיקה לא מוחקת לקוחות — רק את המשתמש. (לקוחות עדיין משויכים לפי שם תצוגה).</div>
        </div>
        ` : ``}
    ...
    if(isAdmin_()){
      const renderUsersTable_ = () => {
        const tbody = document.getElementById('usersTbody');
        if(!tbody) return;
        const users = getUsers_().slice().sort((a,b) => String(a.username||'').localeCompare(String(b.username||'')));
        tbody.innerHTML = users.map(u => `
          <tr>
            <td><b>${escapeHtml(u.username||'')}</b></td>
            <td>${escapeHtml(u.displayName||'')}</td>
            <td>${escapeHtml(u.role||'agent')}</td>
            <td style="text-align:left;white-space:nowrap">
              ${normalizeUsername_(u.username) === 'admin' ? '' : `<button class="btn btnTiny" data-u-del="${escapeAttr(u.username)}">מחק</button>`}
            </td>
          </tr>
        `).join('') || `<tr><td colspan="4" style="color:rgba(18,19,25,.55)">אין משתמשים.</td></tr>`;

        Array.from(tbody.querySelectorAll('[data-u-del]')).forEach(btn => {
          btn.addEventListener('click', (e) => {
            e.preventDefault(); e.stopPropagation();
            const uname = btn.getAttribute('data-u-del');
            if(!uname) return;
            if(!confirm('למחוק את המשתמש ' + uname + '?')) return;
            deleteUser_(uname);
            renderUsersTable_();
            toast('עודכן', 'משתמש נמחק');
          });
        });
      };
      ...
      const uCreateBtn = document.getElementById('uCreateBtn');
      uCreateBtn?.addEventListener('click', (e) => {
        e.preventDefault();
        const username = normalizeUsername_(document.getElementById('uNewUsername')?.value || '');
        const password = String(document.getElementById('uNewPassword')?.value || '').trim();
        const displayName = String(document.getElementById('uNewDisplay')?.value || '').trim();
        if(!username){ toast('חסר', 'שם משתמש חסר'); return; }
        if(!password){ toast('חסר', 'סיסמה חסרה'); return; }
        if(!displayName){ toast('חסר', 'שם תצוגה חסר'); return; }
        const res = upsertUser_({ username, password, role:'agent', displayName });
        if(!res.ok){ toast('שגיאה', res.msg || 'לא ניתן ליצור משתמש'); return; }
        document.getElementById('uNewUsername').value = '';
        document.getElementById('uNewPassword').value = '';
        document.getElementById('uNewDisplay').value = '';
        renderUsersTable_();
        toast('נשמר', 'משתמש נוסף');
      });

      document.getElementById('uExportCsvBtn')?.addEventListener('click', (e) => {
        e.preventDefault();
        exportUsersCSV_();
      });
      document.getElementById('uExportJsonBtn')?.addEventListener('click', (e) => {
        e.preventDefault();
        exportUsersJSON_();
      });

      document.getElementById('uBulkBtn')?.addEventListener('click', (e) => {
        e.preventDefault();
        const count = Number(document.getElementById('uBulkCount')?.value || 0);
        const prefix = String(document.getElementById('uBulkPrefix')?.value || 'agent').trim() || 'agent';
        const start = Number(document.getElementById('uBulkStart')?.value || 1);
        if(!count || count < 1){ toast('חסר', 'הכנס כמות'); return; }
        bulkGenerateAgents_(count, prefix, start);
        renderUsersTable_();
        toast('נוצר', 'נוספו משתמשים');
      });

      document.getElementById('uResetUsersBtn')?.addEventListener('click', (e) => {
        e.preventDefault();
        if(!confirm('לאפס לרשימת דמו? (זה ימחק את כל המשתמשים שנוצרו)')) return;
        const seeded = seedDefaultUsers_();
        state.users = seeded;
        saveUsers_(seeded);
        renderUsersTable_();
        toast('אופס', 'רשימת משתמשים אופסה');
      });
    }
