...
<div class="grid2">
    ...
    // Cache last selected file per insured (file inputs can reset on re-render in some browsers)
    wiz.harFiles = wiz.harFiles || {};
    if(harFileEl){
      harFileEl.addEventListener('change', () => {
        const f = (harFileEl.files && harFileEl.files[0]) ? harFileEl.files[0] : null;
        if(f){
          wiz.harFiles[insuredId] = f;
          if(harPrev){
            harPrev.innerHTML = `נבחר קובץ: <b>${escapeHtml(f.name)}</b>`;
    ...
    if(harBtn){
      harBtn.addEventListener('click', async () => {
        try{
          const fileFromInput = harFileEl && harFileEl.files ? harFileEl.files[0] : null;
          const file = fileFromInput || (wiz.harFiles ? wiz.harFiles[insuredId] : null);
          ...
          // Clear cached file after successful import
          if(wiz.harFiles) delete wiz.harFiles[insuredId];
