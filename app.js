...
(() => {
  ...
  // ---------- Boot: no-login (login removed) ----------

  // ---------- No-Login boot (Login screen removed) ----------
  (function bootNoLogin_(){
    // Always run as admin in this build (no authentication layer)
    state.currentUser = { username:'admin', role:'admin', displayName:'מנהל מערכת' };
    state.agentName = state.currentUser.displayName;
    try{ document.body.classList.remove('isLoggedOut'); }catch(_){}
    applyPermissions_();
