...
(() => {
  ...
  // ---------- Auth session ----------
  const SESSION_KEY = 'gemel_session_token_v1';
  const SESSION_USER_KEY = 'gemel_session_user_v1';
  function getSessionToken_(){
    try{ return String(localStorage.getItem(SESSION_KEY)||''); }catch(_){ return ''; }
  }
  function setSession_(token, user){
    try{
      if(token) localStorage.setItem(SESSION_KEY, String(token));
      if(user) localStorage.setItem(SESSION_USER_KEY, JSON.stringify(user));
    }catch(_){ }
  }
  function clearSession_(){
    try{ localStorage.removeItem(SESSION_KEY); localStorage.removeItem(SESSION_USER_KEY); }catch(_){ }
  }
  function loadSessionUser_(){
    try{ const raw = localStorage.getItem(SESSION_USER_KEY); return raw? JSON.parse(raw) : null; }catch(_){ return null; }
  }
  ...
  async function apiCall(action, payload){
    ...
    const res = await fetch(url, {
      ...
      body: JSON.stringify({ payload: Object.assign({}, payload || {}, { auth: { token: getSessionToken_() } }) })
    });
  ...
  // App root (hide until authenticated)
  ...
  // ---------- Boot: Auth (Modal) ----------
  function hideApp_(){ if(appRootEl) appRootEl.style.display = 'none'; }
  function showApp_(){ if(appRootEl) appRootEl.style.display = ''; }

  async function initAuth_(){
    // If we have a session token, try resume it from server (Live) first.
    const token = getSessionToken_();
    if(token && GOOGLE_SCRIPT_URL){
      const s = await apiCall('authSession', { token });
      if(s && s.ok && s.user){
        state.currentUser = s.user;
        state.agentName = s.user.displayName || s.user.username || 'אורח';
        showApp_();
        setSession_(s.token || token, s.user);
        applyPermissions_();
        if(AUTO_CONNECT) await syncDown();
    ...
    // Fallback: if we stored user locally (for demo), reuse it only in demo mode
    if(!GOOGLE_SCRIPT_URL){
      const u = loadSessionUser_();
      if(u && u.username){
        state.currentUser = u;
        state.agentName = u.displayName || u.username || 'אורח';
        showApp_();
        applyPermissions_();
        if(!location.hash) setRoute('customers');
        return;
    ...
    // No valid session => show modal login
    openAuthModal_(GOOGLE_SCRIPT_URL ? 'התחבר עם משתמש שהוגדר בשרת' : 'מצב דמו (ללא שרת) • התחבר עם משתמש מקומי');
  }

  function openAuthModal_(subText){
    if(!authOverlayEl) return;
    if(authSubEl) authSubEl.textContent = subText || 'אנא הזן שם משתמש וסיסמה';
    if(authErrorEl){ authErrorEl.style.display = 'none'; authErrorEl.textContent = ''; }
    authOverlayEl.style.display = 'flex';
    setTimeout(() => { try{ authUserEl && authUserEl.focus(); }catch(_){ } }, 50);
  }
  ...
  function setAuthError_(msg){
    if(!authErrorEl) return;
    authErrorEl.textContent = msg || 'שגיאה';
    authErrorEl.style.display = '';
  }

  async function doLogin_(){
    const username = String(authUserEl?.value || '').trim();
    const password = String(authPassEl?.value || '').trim();
    if(!username || !password){
      setAuthError_('חסר שם משתמש או סיסמה');
      return;
    }

    // Live auth via Apps Script
    if(GOOGLE_SCRIPT_URL){
      try{
        authLoginBtn && (authLoginBtn.disabled = true);
        const r = await apiCall('authLogin', { username, password });
        if(r && r.ok && r.user && r.token){
          state.currentUser = r.user;
          state.agentName = r.user.displayName || r.user.username || 'אורח';
          showApp_();
          setSession_(r.token, r.user);
          applyPermissions_();
          if(AUTO_CONNECT) await syncDown();
          closeAuthModal_();
          toast('ברוך הבא', state.agentName || 'התחברת');
          if(!location.hash) setRoute('customers');
          render();
          return;
        }
        setAuthError_(r && r.error ? ('שגיאה: ' + r.error) : 'שם משתמש או סיסמה שגויים');
      }catch(e){
        setAuthError_('שגיאת תקשורת. בדוק חיבור לשרת');
      }finally{
        authLoginBtn && (authLoginBtn.disabled = false);
      }
      return;
    }

    // Demo local auth (optional)
    const users = getUsers_();
    const u = users.find(x => x.username === username && String(x.password||'') === password && x.active !== false);
    if(!u){
      setAuthError_('שם משתמש או סיסמה שגויים');
      return;
    }
    const safe = { username:u.username, role:u.role, displayName:u.displayName };
    state.currentUser = safe;
    state.agentName = safe.displayName || safe.username;
    setSession_('', safe);
    applyPermissions_();
    closeAuthModal_();
    toast('ברוך הבא', state.agentName || 'התחברת');
  ...
  function logout_(){
    ...
    applyPermissions_();
    openAuthModal_(GOOGLE_SCRIPT_URL ? 'נותקת. התחבר מחדש' : 'מצב דמו (ללא שרת) • התחבר עם משתמש מקומי');
  }

  // Wire auth modal UI
  if(authLoginBtn) authLoginBtn.addEventListener('click', doLogin_);
  if(authPassEl) authPassEl.addEventListener('keydown', (e) => { if(e.key === 'Enter') doLogin_(); });
  if(authUserEl) authUserEl.addEventListener('keydown', (e) => { if(e.key === 'Enter') doLogin_(); });

  // Quick logout: double click the agent badge
  const agentBadgeEl = document.getElementById('agentBadge');
  if(agentBadgeEl){
    agentBadgeEl.addEventListener('dblclick', () => {
      const ok = confirm('להתנתק ולהחליף משתמש?');
      if(ok) logout_();
    });
  }
...
// Keep wizard title in sync
  ...
  function renderOldPoliciesStep(){
    ...
    wizardMain.innerHTML = `
      ...
      <div class="grid2">
        ${companyPickerField('חברה', 'old_company', ['הראל','כלל','מנורה','הפניקס','מגדל','הכשרה'])}
          ...
          <tbody>
            ${rows.length ? rows.map(({x,idx}) => {
              ...
              if(dec==='לביטול') dec='cancel_full';
                            return `
              <tr>
                <td>${companyCellLogo_(x.company||'')}</td>
...
function renderNewPoliciesStep(){
    ...
    wizardMain.innerHTML = `
      ...
      <div class="grid3">
        ${companyPickerField('חברה', 'new_company', ['הראל','כלל','מנורה','הפניקס','מגדל','הכשרה'])}
...
`;
  ...
  // ---------- Company Logos (UI only) ----------
  // We try multiple filename candidates because repos may use different naming conventions.
  const COMPANY_LOGO_MAP = {
    'הראל': ['assets/companies/harel.svg','assets/companies/harel.png','assets/companies/הראל.svg','assets/companies/הראל.png'],
    'כלל': ['assets/companies/clal.svg','assets/companies/clal.png','assets/companies/כלל.svg','assets/companies/כלל.png'],
    'מנורה': ['assets/companies/menora.svg','assets/companies/menora.png','assets/companies/מנורה.svg','assets/companies/מנורה.png'],
    'הפניקס': ['assets/companies/phoenix.svg','assets/companies/phoenix.png','assets/companies/הפניקס.svg','assets/companies/הפניקס.png'],
    'מגדל': ['assets/companies/migdal.svg','assets/companies/migdal.png','assets/companies/מגדל.svg','assets/companies/מגדל.png'],
    'הכשרה': ['assets/companies/hachshara.svg','assets/companies/hachshara.png','assets/companies/הכשרה.svg','assets/companies/הכשרה.png'],
  };

  function companyLogoCandidates_(name){
    const n = String(name||'').trim();
    const list = COMPANY_LOGO_MAP[n] ? COMPANY_LOGO_MAP[n].slice() : [];
    // fallback: try direct name under assets/companies
    if(n){
      list.push(`assets/companies/${n}.svg`, `assets/companies/${n}.png`);
    }
    // de-dupe
    return Array.from(new Set(list.filter(Boolean)));
  }

  function companyLogoImg_(name, cls){
    const n = String(name||'').trim();
    const srcs = companyLogoCandidates_(n);
    const first = srcs[0] || '';
    // We set data-srcs and handle fallbacks in JS (no inline handlers).
    return `<img class="coLogo ${cls||''}" alt="${escapeAttr(n||'')}" src="${escapeAttr(first)}" data-srcs="${escapeAttr(JSON.stringify(srcs))}" loading="lazy" decoding="async">`;
  }

  function companyCellLogo_(name){
    const n = String(name||'').trim();
    if(!n) return '<span class="coCell"></span>';
    return `<span class="coCell" title="${escapeAttr(n)}" aria-label="${escapeAttr(n)}">${companyLogoImg_(n,'coLogoCell')}</span>`;
  }

  function companyPickerField(label, id, options){
    const opts = options.map(o => {
      const [val, text] = String(o).includes('|') ? String(o).split('|') : [o,o];
      return `<option value="${escapeAttr(val)}">${escapeHtml(text)}</option>`;
    }).join('');

    const logos = options.map(o => {
      const [val, text] = String(o).includes('|') ? String(o).split('|') : [o,o];
      return `
        <button type="button" class="coBtn" data-co="${escapeAttr(val)}" data-target="${escapeAttr(id)}" title="${escapeAttr(text)}" aria-label="${escapeAttr(text)}">
          ${companyLogoImg_(text,'coLogoPick')}
        </button>
      `;
    }).join('');
    ...
    return `
      <div class="field">
        ...
        <div class="coPickRow" role="group" aria-label="${escapeAttr(label)}">${logos}</div>
        <select id="${id}" class="srOnlySelect" aria-hidden="true" tabindex="-1">${opts}</select>
      </div>
    `;
  }

  function wireCompanyPickers_(){
    // Sync logo buttons with hidden selects
    $$('.coPickRow').forEach(row => {
      const buttons = $$('.coBtn', row);
      if(!buttons.length) return;
      const targetId = buttons[0].dataset.target;
      const sel = targetId ? document.getElementById(targetId) : null;
      if(!sel) return;

      const sync = () => {
        const v = String(sel.value||'');
        buttons.forEach(b => b.classList.toggle('active', String(b.dataset.co||'') === v));
      };

      buttons.forEach(b => b.addEventListener('click', () => {
        const v = String(b.dataset.co||'');
        sel.value = v;
      ...
      // default selection
      if(!sel.value && buttons[0]?.dataset.co){
        sel.value = String(buttons[0].dataset.co);
      }
      sync();
    });

    // Logo fallback chaining: if an img fails, try the next candidate
    $$('img.coLogo[data-srcs]').forEach(img => {
      if(img.dataset._wired) return;
      img.dataset._wired = '1';
      img.addEventListener('error', () => {
        try{
          const arr = JSON.parse(img.dataset.srcs||'[]');
          const i = parseInt(img.dataset.errI||'0', 10) || 0;
          if(i+1 < arr.length){
            img.dataset.errI = String(i+1);
            img.src = arr[i+1];
          }
        }catch(_){}
      });
