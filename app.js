(() => {
  ...
  let lastPdfName = '';
  let currentStep = 0; // wizard step index
  ...
  function renderWizard(){
    ...
    const steps = [
      { key:'intro', title:'פתיח ואישור' },
      { key:'personal', title:'פרטי הלקוח' },
      { key:'extra', title:'פרטים משלימים' },
    ];

    const safeStep = Math.max(0, Math.min(steps.length - 1, Number(currentStep) || 0));
    currentStep = safeStep;

    const stepper = `
      <div class="tmWizardHead">
        <div class="tmWizardTitle">שיקוף שיחה — שלבים</div>
        <div class="tmWizardSub">${esc(p._sourceLabel || 'CRM')}</div>
      </div>
      <div class="tmStepper" role="tablist" aria-label="שלבי שיקוף">
        ${steps.map((s, idx) => `
          <button type="button"
            class="tmStepBtn ${idx===safeStep?'on':''}"
            role="tab"
            aria-selected="${idx===safeStep?'true':'false'}"
            data-step="${idx}">
            <span class="tmStepNum">${idx+1}</span>
            <span class="tmStepTxt">${esc(s.title)}</span>
          </button>
        `).join('')}
      </div>
    `;
    ...
    function introStep(){
      return `
        <div class="tmStepCard">
          <div class="tmScript">
            שלום, אני מדבר/ת עם <b>${esc(p.fullName || '—')}</b>?<br/>
            מדבר/ת <b>${esc('אוריה')}</b> ואני נציג/ת מכירות מטעם סוכנות <b>${esc('גרגורי')}</b>
            משווק הביטוחים של חברת <b>${esc(companiesText || '—')}</b>.<br/>
            אני יוצר/ת איתך קשר בהמשך לפנייתך ולשיחתך עם <b>${esc(p.preparedBy || '—')}</b>
            במטרה להציע לך לרכוש ביטוח.<br/>
          ...
          <div class="tmGrid tmGrid2" style="margin-top:12px">
            <div class="field full">
              <span class="lbl">חברות שנרכשו (ניתן לעריכה)</span>
              <input class="input inputSm" id="f_companies" value="${esc(companiesText)}" placeholder="למשל: הראל, הפניקס" />
            </div>
            <div class="field full">
              <span class="lbl">שם הנציג שהכין את ההצעה (ניתן לעריכה)</span>
              <input class="input inputSm" id="f_preparedBy" value="${esc(p.preparedBy)}" placeholder="שם נציג" />
            </div>
          </div>

          <div class="smallWarn">הערכים כאן הם נקודת פתיחה בלבד — אפשר לתקן לפי מה שהלקוח אומר.</div>
        </div>
      `;
    }
    ...
    const body = (safeStep === 0) ? introStep() : (safeStep === 1) ? personalStep() : extraStep();

    const footer = `
      <div class="tmWizardFooter">
        <button class="btn btnSoft btnSmall" type="button" id="tmPrev" ${safeStep===0?'disabled':''}>הקודם</button>
        <div class="tmProgress">${safeStep+1} / ${steps.length}</div>
        <button class="btn btnSmall" type="button" id="tmNext" ${safeStep===steps.length-1?'disabled':''}>הבא</button>
      </div>
    `;

    return `<div class="tmWizard">${stepper}<div class="tmWizardBody">${body}</div>${footer}</div>`;
  }
  ...
  function bindWizardEvents(){
    ...
    // step switching
    const stepBtns = [...document.querySelectorAll('.tmStepBtn[data-step]')];
    stepBtns.forEach(b => {
    ...
    if(prev){
      prev.addEventListener('click', () => {
        currentStep = Math.max(0, (Number(currentStep)||0) - 1);
        currentStep = 0;
    renderDetails({ __profile: true });
      });
    }
    if(next){
      next.addEventListener('click', () => {
        currentStep = Math.min(2, (Number(currentStep)||0) + 1);
        currentStep = 0;
    renderDetails({ __profile: true });
      });
    }
    ...
    if(cYes && cNo){
      ...
      setYN(cYes, cNo, v);
      if(cMeta) cMeta.textContent = v ? ('נבחר: ' + v) : 'לא סומן';

      cYes.addEventListener('click', () => {
        currentProfile.consent = 'כן';
        setYN(cYes, cNo, 'כן');
        if(cMeta) cMeta.textContent = 'נבחר: כן';
      });
      cNo.addEventListener('click', () => {
        currentProfile.consent = 'לא';
        setYN(cYes, cNo, 'לא');
        if(cMeta) cMeta.textContent = 'נבחר: לא';
      });
    }
    ...
    const compEl = document.getElementById('f_companies');
    if(compEl){
      compEl.addEventListener('input', () => {
        const raw = String(compEl.value||'');
        currentProfile.companies = raw.split(',').map(s => norm(s)).filter(Boolean);
      });
    }
  }
  ...
  function renderDetails(c){
    ...
    detailsBody.innerHTML = renderWizard();
