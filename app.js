...
(() => {
  ...
  // מערכת פתוחה: אין מסך כניסה / שם משתמש / סיסמה.
  // אם תרצה בעתיד להחזיר הרשאות — נחזיר בצורה מסודרת.
  state.currentUser = { username:'admin', role:'admin', displayName:'מנהל מערכת' };
  state.agentName = 'מנהל מערכת';

  function isAdmin_(){ return true; }

  function applyPermissions_(){
    // update badge name
    const badgeNameEl = document.getElementById('agentName');
    if(badgeNameEl){
      badgeNameEl.textContent = 'נציג: ' + (state.agentName || 'מנהל מערכת');
    }
    // show admin-only items (since open access)
  ...
  // Boot: no auth required
  (function bootOpen_(){
    try{ applyPermissions_(); }catch(_){}
    // route to customers by default
