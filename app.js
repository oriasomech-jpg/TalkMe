...
// ---------- CONFIG (Google Sheets via Apps Script Web App) ----------
  ...
  // Call Reflection (Shikuf Sicha) module
  const reflectBtn = document.querySelector('.navItem[data-action="callReflect"]');
  const reflectOverlay = $('#reflectOverlay');
  const reflectCloseBtn = $('#reflectCloseBtn');
  const reflectFrame = $('#reflectFrame');
  const reflectFallback = $('#reflectFallback');
  // Legacy internal reflect UI (kept for future iteration; elements may be absent when using iframe)
  const reflectSearch = $('#reflectSearch');
  const reflectSearchBtn = $('#reflectSearchBtn');
  const reflectResults = $('#reflectResults');
  const reflectStepper = $('#reflectStepper');
  const reflectStage = $('#reflectStage');
  const reflectPrevBtn = $('#reflectPrevBtn');
  const reflectNextBtn = $('#reflectNextBtn');


  // Set your GOFI module URL (GitHub Pages) here when ready
  // Example: const GOFI_URL = 'https://oriasomech-jpg.github.io/TalkMe/';

  // Set your 'Shikuf Sicha' module URL (GitHub Pages / external server) here when ready
  // Example: const REFLECT_CALL_URL = 'https://your-user.github.io/your-reflect-repo/';
  const REFLECT_CALL_URL = '';
...
function renderKpisSettings(){
  ...
  // Call Reflection overlay wiring
  if(reflectCloseBtn){
    reflectCloseBtn.addEventListener('click', closeReflect_);
  }
  if(reflectOverlay){
    reflectOverlay.addEventListener('click', (e) => { if(e.target === reflectOverlay) closeReflect_(); });
  }
  if(reflectSearchBtn){
    reflectSearchBtn.addEventListener('click', () => {
      const q = reflectSearch ? reflectSearch.value : '';
      renderReflectResults_(q);
    });
  }
  if(reflectSearch){
    reflectSearch.addEventListener('keydown', (e) => {
      if(e.key === 'Enter'){
        e.preventDefault();
        renderReflectResults_(reflectSearch.value);
      }
    });
  }
  if(reflectPrevBtn){
    reflectPrevBtn.addEventListener('click', () => {
      if(!reflectState.customer) return;
      reflectState.stepIndex = Math.max(0, reflectState.stepIndex - 1);
      renderReflect_();
    });
  }
  if(reflectNextBtn){
    reflectNextBtn.addEventListener('click', () => {
      if(!reflectState.customer) return;
      if(reflectState.stepIndex >= REFLECT_STEPS.length - 1){
        toast('הסתיים', 'שיקוף שיחה הושלם (דמו)');
        closeReflect_();
        return;
      }
      reflectState.stepIndex = Math.min(REFLECT_STEPS.length - 1, reflectState.stepIndex + 1);
      renderReflect_();
    });
  }
...
// ==========================
// Call Reflection (שיקוף שיחה)
// ==========================
let __reflectPrevBodyOverflow = null;
const REFLECT_STEPS = [
  { key:'verify',  label:'אימות זיהוי' },
  { key:'old',     label:'קיים היום' },
  { key:'new',     label:'פוליסות חדשות' },
  { key:'medical', label:'שאלון רפואי' },
  { key:'payer',   label:'משלם' }
];
...
const reflectState = {
  ...
  proposal: null,
  stepIndex: 0,
  fields: {} // small per-step notes/approvals (demo-local)
};

function ensureReflectFullscreen_(){
  try{
    if(__reflectPrevBodyOverflow === null){
      __reflectPrevBodyOverflow = document.body.style.overflow || '';
    }
    document.body.style.overflow = 'hidden';
  }catch(_){ }
}

function restoreReflectFullscreen_(){
  try{
    if(__reflectPrevBodyOverflow !== null){
      document.body.style.overflow = __reflectPrevBodyOverflow;
      __reflectPrevBodyOverflow = null;
    }else{
      document.body.style.overflow = '';
    }
  }catch(_){ }
...
function openReflect_(){
  if(!reflectOverlay) return;
  ensureReflectFullscreen_();

  // Load external module inside iframe (new repo / server)
  const url = (typeof REFLECT_CALL_URL === 'string') ? REFLECT_CALL_URL.trim() : '';
  if(reflectFrame){
    if(url){
      const sep = url.includes('?') ? '&' : '?';
      // cache-bust using BUILD when available
      const v = (typeof BUILD !== 'undefined') ? String(BUILD) : String(Date.now());
      reflectFrame.src = url + sep + 'v=' + encodeURIComponent(v);
      if(reflectFallback) reflectFallback.classList.add('hidden');
    }else{
      reflectFrame.removeAttribute('src');
      if(reflectFallback) reflectFallback.classList.remove('hidden');
    }
  }else{
    if(reflectFallback) reflectFallback.classList.remove('hidden');
  }

  reflectOverlay.classList.remove('hidden');
  reflectOverlay.setAttribute('aria-hidden','false');
}

function closeReflect_(){
  if(!reflectOverlay) return;
  reflectOverlay.classList.add('hidden');
  reflectOverlay.setAttribute('aria-hidden','true');
  restoreReflectFullscreen_();
}

function normalizeQ_(q){
  return String(q||'')
    .trim()
    .toLowerCase()
    .replace(/\s+/g,' ');
...
function findAllCustomers_(q){
  const qq = normalizeQ_(q);
  const list = Array.isArray(state.customers) ? state.customers : [];
  if(!qq) return list.slice();
  return list.filter(c => {
    const name = normalizeQ_(c.fullName);
    const tz = normalizeQ_(c.tz);
    const phone = normalizeQ_(c.phone);
    return name.includes(qq) || tz.includes(qq) || phone.includes(qq);
  });
}

function latestProposalForCustomer_(customerId){
  const list = (state.proposals||[])
    .filter(p => p && (p.customerId === customerId || (p.customer && p.customer.id === customerId)))
    .slice();
...
function renderReflectResults_(q){
  if(!reflectResults) return;
  const found = findAllCustomers_(q);
  if(!found.length){
    reflectResults.innerHTML = `<div class="sideHint">לא נמצאו תוצאות. נסה שם / ת״ז / טלפון.</div>`;
    return;
  }

  reflectResults.innerHTML = found.map(c => {
    const name = escapeHtml(c.fullName||'');
    const tz = escapeHtml(c.tz||'');
    const phone = escapeHtml(c.phone||'');
    const agent = escapeHtml(c.assignedAgent||'');
    return `
      <div class="reflectRow">
        <div style="min-width:0">
          <b>${name || 'ללא שם'}</b>
          <div class="meta">ת״ז: ${tz || '—'} • טלפון: ${phone || '—'}${agent ? ` • משויך: ${agent}` : ''}</div>
        </div>
        <div style="white-space:nowrap">
          <button class="btn btnTiny" data-reflect-pick="${escapeAttr(c.id)}">בחר</button>
        </div>
      </div>
    `;
  }).join('');
  ...
  // Wire pick buttons
  $$('[data-reflect-pick]', reflectResults).forEach(b => b.addEventListener('click', () => {
    const id = b.dataset.reflectPick;
    const c = (state.customers||[]).find(x => x.id === id) || null;
    if(!c) return;
    reflectState.customer = c;
    reflectState.proposal = latestProposalForCustomer_(c.id);
    reflectState.stepIndex = 0;
    renderReflect_();
  }));
}

function renderReflect_(){
  // header
  if(reflectSub){
    const c = reflectState.customer;
    reflectSub.textContent = c ? (`לקוח: ${c.fullName || '—'} • שלב ${reflectState.stepIndex+1}/${REFLECT_STEPS.length}`) : 'בחירת לקוח → שלבים';
  }

  renderReflectStepper_();
  renderReflectStage_();

  // buttons state
  if(reflectPrevBtn) reflectPrevBtn.disabled = !reflectState.customer || reflectState.stepIndex === 0;
  if(reflectNextBtn){
    reflectNextBtn.disabled = !reflectState.customer;
    reflectNextBtn.textContent = (!reflectState.customer)
      ? 'בחר לקוח כדי להתחיל'
      : (reflectState.stepIndex >= REFLECT_STEPS.length-1 ? 'סיום' : 'מאשר והמשך');
  }
}

function renderReflectStepper_(){
  if(!reflectStepper) return;
  reflectStepper.innerHTML = REFLECT_STEPS.map((s, i) => {
    const cls = (reflectState.customer && i === reflectState.stepIndex) ? 'rStep active' : 'rStep';
    return `<div class="${cls}" data-rstep="${escapeAttr(String(i))}">${escapeHtml(String(i+1))}. ${escapeHtml(s.label)}</div>`;
  }).join('');

  // allow step click only after customer selected
  $$('[data-rstep]', reflectStepper).forEach(el => el.addEventListener('click', () => {
    if(!reflectState.customer) return;
    const i = Number(el.dataset.rstep||0);
    if(!Number.isFinite(i)) return;
    reflectState.stepIndex = Math.max(0, Math.min(REFLECT_STEPS.length-1, i));
    renderReflect_();
  }));
}

function smallFieldBlock_(stepKey){
  const base = reflectState.fields[stepKey] || (reflectState.fields[stepKey] = { ok:'', note:'' });
  const ok = escapeAttr(base.ok||'');
  const note = escapeHtml(base.note||'');
  return `
    <div class="panel" style="padding:12px;border-radius:16px;margin-top:12px">
      <div style="font-weight:900;margin-bottom:8px">אישור הנציג (דמו)</div>
      <div style="display:grid;grid-template-columns: 220px 1fr;gap:10px;align-items:end">
        <div>
          <div class="lbl" style="margin-bottom:6px">הלקוח אישר?</div>
          <select class="input" data-rfield-ok="${escapeAttr(stepKey)}">
            <option value="" ${!ok?'selected':''}>—</option>
            <option value="yes" ${ok==='yes'?'selected':''}>כן</option>
            <option value="no" ${ok==='no'?'selected':''}>לא</option>
          </select>
        </div>
        <div>
          <div class="lbl" style="margin-bottom:6px">הערה קצרה</div>
          <input class="input" type="text" value="${escapeAttr(base.note||'')}" data-rfield-note="${escapeAttr(stepKey)}" placeholder="אופציונלי" />
        </div>
      </div>
    </div>
  `;
...
function renderReflectStage_(){
  if(!reflectStage) return;
  if(!reflectState.customer){
    reflectStage.innerHTML = `
      <div class="sideHint">כדי להתחיל — חפש לקוח משמאל ולחץ <b>בחר</b>. השיקוף ימשוך נתונים מתוך ההצעה האחרונה של הלקוח (במצב דמו זה לוקאל).</div>
    `;
    return;
  }

  const c = reflectState.customer;
  const p = reflectState.proposal;
  const step = REFLECT_STEPS[reflectState.stepIndex];

  const noProposal = !p;
  const proposalHint = noProposal
    ? `<div class="sideHint">לא נמצאה הצעה שמורה ללקוח הזה במצב דמו. אחרי שתיצור/תשמור הצעה — כאן יופיעו הנתונים לשיקוף.</div>`
    : '';

  const title = `<div style="font-weight:900;font-size:16px;margin-bottom:6px">${escapeHtml(step.label)}</div>`;

  if(step.key === 'verify'){
    reflectStage.innerHTML = `
      ${title}
      <div class="panel" style="padding:12px;border-radius:16px">
        <div class="miniStat"><span>שם מלא</span><b>${escapeHtml(c.fullName||'—')}</b></div>
        <div class="miniStat"><span>ת״ז</span><b>${escapeHtml(c.tz||'—')}</b></div>
        <div class="miniStat"><span>טלפון</span><b>${escapeHtml(c.phone||'—')}</b></div>
        <div class="miniStat"><span>אימייל</span><b>${escapeHtml(c.email||'—')}</b></div>
      </div>
      ${proposalHint}
      ${smallFieldBlock_('verify')}
    `;
    wireReflectSmallFields_();
    return;
  }

  if(step.key === 'old'){
    const rows = (p && Array.isArray(p.oldPolicies)) ? p.oldPolicies : [];
    reflectStage.innerHTML = `
      ${title}
      <div class="sideHint">שיקוף מצב קיים — מה משולם היום, ומה נשאר/מבוטל.</div>
      ${proposalHint}
      <div class="tableWrap" style="margin-top:10px">
        <table class="table">
          <thead><tr>
            <th>חברה</th><th>מוצר</th><th>מס׳ פוליסה</th><th>פרמיה</th><th>סטטוס</th>
          </tr></thead>
          <tbody>
            ${rows.length ? rows.map(x => `
              <tr>
                ...
                <td>${escapeHtml(x.policyNumber||x.policyNo||'')}</td>
                <td>${escapeHtml(String(x.premium||x.price||''))}</td>
                <td>${escapeHtml(x.status||x.keepCancel||'')}</td>
              </tr>
            `).join('') : `<tr><td colspan="5" style="opacity:.7">אין פוליסות קיימות להצגה</td></tr>`}
          </tbody>
        </table>
      </div>
      ${smallFieldBlock_('old')}
    `;
    wireReflectSmallFields_();
    return;
  }
  ...
  if(step.key === 'new'){
    const rows = (p && Array.isArray(p.newPolicies)) ? p.newPolicies : [];
    reflectStage.innerHTML = `
      ${title}
      <div class="sideHint">שיקוף פוליסות חדשות — מה נרכש אצלנו, סוג, פרמיה, הנחות.</div>
      ${proposalHint}
      <div class="tableWrap" style="margin-top:10px">
        <table class="table">
          <thead><tr>
            <th>חברה</th><th>מוצר</th><th>כיסוי/סכום</th><th>פרמיה</th><th>הנחה</th>
          </tr></thead>
          <tbody>
            ${rows.length ? rows.map(x => `
              <tr>
                <td>${renderCompanyCell(x.company||'')}</td>
                <td>${escapeHtml(x.product||'')}</td>
                <td>${escapeHtml(String(x.sumInsured||x.cover||x.compensation||''))}</td>
                <td>${escapeHtml(String(x.premium||x.price||''))}</td>
                <td>${escapeHtml(String(x.discount||x.discountPct||''))}</td>
              </tr>
            `).join('') : `<tr><td colspan="5" style="opacity:.7">אין פוליסות חדשות להצגה</td></tr>`}
          </tbody>
        </table>
      </div>
      ${smallFieldBlock_('new')}
    `;
    wireReflectSmallFields_();
    return;
  }
  ...
  if(step.key === 'medical'){
    // we keep it high-level; actual medical UI already exists in wizard
    const m = (p && p.medical) ? p.medical : null;
    const hasMedical = !!(m && Object.keys(m).length);
    reflectStage.innerHTML = `
      ${title}
      <div class="sideHint">כאן נציג משקף את התשובות שנאספו בשאלון הרפואי בהצעה.</div>
      ${proposalHint}
      <div class="panel" style="padding:12px;border-radius:16px">
        <div class="miniStat"><span>קיים שאלון רפואי בהצעה?</span><b>${hasMedical ? 'כן' : 'לא / ריק'}</b></div>
        <div style="margin-top:10px;font-size:12px;opacity:.75">
          בשלב הבא נלטש את זה: נציג כאן את השאלות/תשובות לפי המבוטח הפעיל (מבוטח ראשי/ילדים), בדיוק כמו שמולא בטופס.
        </div>
      </div>
      ${smallFieldBlock_('medical')}
    `;
    wireReflectSmallFields_();
    return;
  }
  ...
  if(step.key === 'payer'){
    const payer = (p && (p.payer || p.payerInfo || p.payment)) ? (p.payer || p.payerInfo || p.payment) : null;
    reflectStage.innerHTML = `
      ${title}
      <div class="sideHint">שיקוף פרטי משלם — כפי שהוקלדו במסך משלם.</div>
      ${proposalHint}
      <div class="panel" style="padding:12px;border-radius:16px">
        <div class="miniStat"><span>שם משלם</span><b>${escapeHtml((payer && (payer.fullName || payer.name)) || '—')}</b></div>
        <div class="miniStat"><span>ת״ז משלם</span><b>${escapeHtml((payer && (payer.tz || payer.id)) || '—')}</b></div>
        <div class="miniStat"><span>אמצעי תשלום</span><b>${escapeHtml((payer && (payer.method || payer.payMethod)) || '—')}</b></div>
      </div>
      ${smallFieldBlock_('payer')}
    `;
    wireReflectSmallFields_();
    return;
  }

  reflectStage.innerHTML = `${title}<div class="sideHint">שלב לא ידוע.</div>`;
}

function wireReflectSmallFields_(){
  if(!reflectStage) return;
  // ok select
  $$('[data-rfield-ok]', reflectStage).forEach(sel => sel.addEventListener('change', () => {
    const k = sel.dataset.rfieldOk;
    if(!k) return;
    reflectState.fields[k] = reflectState.fields[k] || { ok:'', note:'' };
    reflectState.fields[k].ok = String(sel.value||'');
  }));
  // note
  $$('[data-rfield-note]', reflectStage).forEach(inp => inp.addEventListener('input', () => {
    const k = inp.dataset.rfieldNote;
    if(!k) return;
    reflectState.fields[k] = reflectState.fields[k] || { ok:'', note:'' };
    reflectState.fields[k].note = String(inp.value||'');
  }));
}
