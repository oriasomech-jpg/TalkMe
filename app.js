...
(() => {
  ...
  function renderMyProcesses(){
    ...
    // Upcoming / overdue processes
    const now = Date.now();
    const due = (state.processes||[])
      .filter(p => (p.status||'open') === 'open' && String(p.assignedTo||'') === String(agent))
      .map(p => ({...p, overdue: isOverdue_(p.dueAt)}));
    due.sort((a,b) => {
    ...
    const dueHtml = due.length ? `
      <div class="card" style="margin-bottom:14px">
        <div style="display:flex;justify-content:space-between;align-items:center;gap:10px;margin-bottom:10px">
          ...
          <div style="font-size:12px;color:rgba(18,19,25,.65)">${due.length} פריטים</div>
        </div>
        <div class="tableWrap" style="border:none;box-shadow:none;padding:0">
          <table class="table">
            <thead>
              <tr>
                <th>לקוח</th>
            ...
            <tbody>
              ${due.map(p => {
                const c = customerById_(p.customerId);
                const nm = (c && c.fullName) ? c.fullName : (p.customerName||'—');
                const dueTxt = formatDt_(p.dueAt);
                const badge = p.overdue ? '<span class="chip chipBad">חורג</span>' : '<span class="chip chipWarn">מתקרב</span>';
                return `
                  <tr>
                    <td><b>${escapeHtml(nm)}</b><div style="font-size:12px;color:rgba(18,19,25,.55)">${escapeHtml((c&&c.phone)||'')}</div></td>
                    <td>${escapeHtml(p.type||'—')} ${badge}</td>
                    <td>${escapeHtml(dueTxt)}</td>
                    <td>${escapeHtml(p.note||'')}</td>
                    <td style="white-space:nowrap">
                      <button class="btn btnTiny" data-open-proposal="${escapeAttr(p.customerId)}">פתח</button>
    ...
    ` : `
      <div class="card" style="margin-bottom:14px">
        <div style="font-weight:900;margin-bottom:6px">תהליכים שמגיעים עכשיו</div>
        <div style="color:rgba(18,19,25,.65);font-size:13px">אין כרגע תהליכים שמוגדרים עבורך.</div>
      </div>
    `;
...
function renderKpisSettings(){
  ...
  function renderCustomersTable(rows){
          ...
          <tbody>
            ${rows.map(c => `
              <tr>
                ...
                <td><div style="display:flex;gap:8px;justify-content:flex-end;flex-wrap:wrap"><button class="btn btnSoft" data-open-proposal="${c.id}">צור הצעה</button><button class="btn btnSoft" data-create-process="${c.id}">צור תהליך</button></div></td>
              </tr>
            `).join('')}
          </tbody>
        </table>
    ...
    $$('[data-proc-done]').forEach(btn => {
      btn.addEventListener('click', () => {
        const id = btn.getAttribute('data-proc-done');
        markProcessDone_(id);
      });
    });

    $$('[data-create-process]').forEach(btn => {
      btn.addEventListener('click', () => {
        const cid = btn.getAttribute('data-create-process');
        openProcessModal(cid);
      });
    });
...
function renderNewPoliciesStep(){
  ...
  function renderSummaryStep(){
    ...
    wizardMain.innerHTML = `
      ...
      <div class="grid2" style="margin-top:10px">
        <div class="field">
          ...
          <input id="summaryRepInput" placeholder="הקלד שם נציג" value="${escapeHtml(String((p.meta&&p.meta.repName)||p.assignedAgent||''))}"/>
        </div>
        <div class="field">
          <label>סוכן</label>
          <input id="summaryBrokerInput" placeholder="הקלד שם סוכן" value="${escapeHtml(String((p.meta&&p.meta.brokerName)||''))}"/>
        </div>
      </div>
    ...
    // Summary header inputs (Rep / Broker)
    const repIn = document.getElementById('summaryRepInput');
    if(repIn){
      repIn.addEventListener('input', (e)=>{
        p.meta = p.meta || {};
        p.meta.repName = String(e.target.value || '').trim();
      });
      repIn.addEventListener('change', (e)=>{
        p.meta = p.meta || {};
        p.meta.repName = String(e.target.value || '').trim();
      });
    }
    const broIn = document.getElementById('summaryBrokerInput');
    if(broIn){
      broIn.addEventListener('input', (e)=>{
        p.meta = p.meta || {};
        p.meta.brokerName = String(e.target.value || '').trim();
      });
      broIn.addEventListener('change', (e)=>{
        p.meta = p.meta || {};
        p.meta.brokerName = String(e.target.value || '').trim();
      });
    }
    ...
    if(pdfBtnEl) pdfBtnEl.addEventListener('click', () => {
      // Open the print window FIRST (keeps it in the user-gesture context, avoids popup blockers)
      const w = window.open('', '_blank');
      if(!w){ toast('חסום', 'הדפדפן חסם חלון קופץ'); return; }

      p.pdfGenerated = true;

      // Render print view into the opened window
      openPrintView(p, w);

      // Optional: also download the HTML (backup / archiving)
      downloadSummaryHtml_(p);
  ...
  function buildSummaryHtml_(proposal){
    const p = proposal;

    function esc(v){ return escapeHtml(String(v||'').trim()||'—'); }

    const insuredList = (p.insuredList||[]).filter(ins => insuredHasAnyData(p, ins.id));
    const repName = String((p.meta && (p.meta.repName || p.meta.rep || p.meta.agentName)) || p.assignedAgent || (typeof state!=='undefined' ? state.agentName : '') || '').trim();
    const brokerName = String((p.meta && (p.meta.brokerName || p.meta.broker || p.meta.agentBroker)) || p.brokerName || (p.customer && (p.customer.brokerName || p.customer.broker || p.customer.agentBroker)) || '').trim();
    const generatedAt = new Date().toLocaleString('he-IL', { weekday:'long', year:'numeric', month:'2-digit', day:'2-digit', hour:'2-digit', minute:'2-digit' });
        ...
        return `
          <div class="ibox">
            ...
            <table>
              ...
              <tbody>
                ${rows.map(x => {
                  const decision = (x.decision || x.status || '').toString();
                  const statusLabel = (decision === 'keep') ? 'נשאר ללא שינוי' : (decision === 'cancel') ? 'לביטול' : (x.decisionLabel || x.decisionText || '');
                  return `<tr>
                    <td>${esc(x.company)}</td>
                    <td>${esc(x.product)}</td>
                    <td>${money(x.premium||0)}</td>
                    ${showStatus ? `<td>${esc(statusLabel || '—')}</td>` : ``}
                  </tr>`;
                }).join('')}
              </tbody>
            </table>
    ...
    function medicalTables(){
      ...
      for(const ins of insuredList){
        ...
        for(const [k,v] of Object.entries(bucket)){
          ...
          const isYes = (valRaw === 'כן');
          rows.push(`<tr class="${'${isYes ? "yesRow" : ""}'}"><td>${esc(label)}</td><td>${esc(valRaw + extra)}</td></tr>`);
        }
        ...
        out.push(`
          <div class="ibox">
            ...
            <div class="muted" style="margin:-2px 0 8px">
              <b>שם מלא:</b> ${esc(d.fullName)} &nbsp; • &nbsp; <b>ת״ז:</b> ${esc(d.tz)}
            </div>
            <table>
              <thead><tr><th>שאלה</th><th>תשובה</th></tr></thead>
              <tbody>${rows.length ? rows.join('') : `<tr><td colspan="2">אין תשובות</td></tr>`}</tbody>
    ...
    const html = `
      ...
      <head>
        ...
        <style>
          ...
          .yesRow td{background:#eefaf1; font-weight:700}
      ...
      <body>
        <h1>סיכום עסקה</h1>
        <div class="sub">נציג מטפל: <b>${esc(repName) }</b> &nbsp; | &nbsp; סוכן: <b>${esc(brokerName) }</b><br/>תאריך הפקה: ${esc(generatedAt)}</div>
        ...
        <script>
          // Give the browser a tick to paint before opening print dialog
  ...
  function downloadSummaryHtml_(proposal){
    try{
      const html = buildSummaryHtml_(proposal);
      const fname = 'סיכום עסקה - ' + sanitizeFilename_((proposal.customer && proposal.customer.fullName) || proposal.customerName || 'לקוח') + '.html';
  ...
  function openPrintView(proposal, existingWindow=null){
  let w = existingWindow || null;
  if(!w){
    w = window.open('', '_blank');
    if(!w){ toast('חסום', 'הדפדפן חסם חלון קופץ'); return null; }
  }

  let html = '';
  try{
    html = buildSummaryHtml_(proposal);
  }catch(err){
    console.error('buildSummaryHtml_ failed:', err);
    toast('שגיאה', 'שגיאה ביצירת סיכום. פתח קונסול לפרטים.');
    const msg = String(err && err.message ? err.message : err);
    html = `<!doctype html><html lang="he" dir="rtl"><head><meta charset="utf-8"><title>שגיאה</title>
      <style>body{font-family:Arial;direction:rtl;padding:24px} .box{border:1px solid #ddd;border-radius:12px;padding:16px;background:#fff}</style>
      </head><body><div class="box"><h2>שגיאה ביצירת סיכום</h2><div>${escapeHtml(msg)}</div><div style="margin-top:12px;color:#666">נסה שוב לאחר רענון, או פתח דרך הצעה → סיכום.</div></div></body></html>`;
  }
  ...
  try{
    ...
    console.error('write print window failed:', err);
    toast('שגיאה', 'לא הצלחנו לפתוח תצוגת הדפסה.');
    return null;
  }
  return w;
}

  // ---------- Completion (What is missing) (What is missing) ----------
...
function setSelectValue(id, value){
  ...
  // ---------- Process UI ----------
  function customerById_(cid){
    return state.customers.find(c => String(c.id) === String(cid)) || null;
  }
  ...
  function upsertProcess_(p){
    state.processes = Array.isArray(state.processes) ? state.processes : [];
    const i = state.processes.findIndex(x => x.id === p.id);
    if(i >= 0) state.processes[i] = p;
    else state.processes.push(p);
    saveProcesses_();
  }

  function markProcessDone_(id){
    const p = (state.processes||[]).find(x => x.id === id);
    if(!p) return;
    p.status = 'done';
    p.doneAt = new Date().toISOString();
    upsertProcess_(p);
    toast('עודכן', 'התהליך סומן כבוצע');
    render();
  }

  function openProcessModal(customerId){
    const c = customerById_(customerId);
    if(!c){ toast('שגיאה','לא נמצא לקוח'); return; }

    // Ensure a default datetime (30 minutes from now)
    ...
    overlay.style.cssText = 'position:fixed;inset:0;background:rgba(5,7,10,.55);backdrop-filter: blur(6px);display:flex;align-items:center;justify-content:center;z-index:9999;padding:18px;';
    overlay.addEventListener('click', (e) => { if(e.target===overlay) overlay.remove(); });

    const card = document.createElement('div');
    card.style.cssText = 'width:min(680px,100%);background:rgba(255,255,255,.92);border:1px solid rgba(18,19,25,.12);border-radius:18px;box-shadow:0 24px 70px rgba(0,0,0,.35);padding:18px;';
    card.innerHTML = `
      <div style="display:flex;justify-content:space-between;gap:12px;align-items:center;margin-bottom:10px">
        <div>
          <div style="font-weight:900;font-size:16px;color:#121319">צור תהליך</div>
          <div style="color:rgba(18,19,25,.65);font-size:12px;margin-top:2px">לקוח: <b>${escapeHtml(c.fullName)}</b> • ת״ז: ${escapeHtml(c.tz||'')}</div>
        </div>
      ...
      <div style="display:grid;grid-template-columns:1fr 1fr;gap:12px">
        <div class="field">
          <label>בחירת תהליך</label>
          <select id="procType" class="input">
        ...
        <div class="field">
          ...
          <input id="procDue" class="input" type="datetime-local" value="${dtLocal}">
        </div>

        <div class="field" style="grid-column:1/-1">
          <label>הערה (אופציונלי)</label>
          <input id="procNote" class="input" type="text" placeholder="לדוגמה: לחזור אחרי שיחה / לבדוק מסמכים...">
        </div>
      </div>
    ...
    card.querySelector('[data-save]').addEventListener('click', () => {
      const type = card.querySelector('#procType').value;
      const dueLocal = card.querySelector('#procDue').value;
      const note = card.querySelector('#procNote').value || '';
      if(!dueLocal){
        toast('חסר מידע','בחר תאריך ושעה');
        return;
      }
      const dueAt = new Date(dueLocal).toISOString();
      const proc = {
        id: 'pr_' + Math.random().toString(16).slice(2) + '_' + Date.now(),
        customerId: c.id,
